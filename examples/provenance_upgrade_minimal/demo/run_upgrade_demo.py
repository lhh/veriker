...
from examples.provenance_upgrade_minimal.predicate_check import (  # noqa: E402
    UpgradePredicateCheck,
)
...
def main() -> int:
    ...
    # 0 — HONEST: the real verifier-signed upgrade is admitted, AND the
    #     predicate it was granted for re-derives TRUE from committed bytes.
    ...
    rp = UpgradePredicateCheck().check(_BUNDLE, _manifest_from(copy.deepcopy(base)))
    print(f"[0b] predicate re-derivation         -> ok={rp.ok} {rp.reason_code}")
    print(f"      {rp.detail}")
    failures += 0 if (rp.ok and rp.reason_code == "PASS") else 1
    ...
    # 5 — PREDICATE TAMPER: the extraction value no longer equals the source
    #     sum. The upgrade signature still verifies (it covers the record
    #     body, not the payload bytes) — the NEW predicate re-derivation is
    #     what refuses. Before this check existed, this tamper rode through
    #     every leg (the by-catch gap this scenario exists to keep closed).
    import shutil
    import tempfile

    with tempfile.TemporaryDirectory() as td:
        tampered = Path(td) / "bundle"
        shutil.copytree(_BUNDLE, tampered)
        xp = tampered / "payload" / "extractions.json"
        x = json.loads(xp.read_bytes())
        for c in x["claims"]:
            if c.get("claim_id") == "extract_total":
                c["value_usd"] = c["value_usd"] + 1
        xp.write_bytes(json.dumps(x, indent=2).encode("utf-8"))
        r5 = UpgradePredicateCheck().check(
            tampered, _manifest_from(copy.deepcopy(base))
        )
    expect = (not r5.ok) and r5.reason_code == "UPGRADE_PREDICATE_REFUTED"
    print(
        f"[5] tampered extraction value        -> ok={r5.ok} {r5.reason_code}  "
        "(expect PREDICATE_REFUTED)"
    )
    failures += 0 if expect else 1
    ...
    if failures == 0:
        print(
            ...
            "(1 honest PASS + predicate re-derivation + 5 tamper rejections)"
        )
        return 0
