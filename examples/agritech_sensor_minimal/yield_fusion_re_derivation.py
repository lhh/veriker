...
def _verify(bundle_dir: Path) -> str | None:
    ...
    # ------------------------------------------------------------------
    # Attribution binding: the forecast's claimed field_id, sensor_id, and
    # window_start/window_end must match the SAME fields already carried by
    # inputs/sensor_stream.json. Without this, a producer could honestly
    # compute yield_score from field A's real data but attribute/timestamp
    # the forecast to field B (or a different reporting window) — a
    # scope-misbinding gap that the numeric re-derivation below cannot
    # catch, since the score itself would be untouched. Fail closed if
    # either side omits a binding field: an un-attributable forecast is
    # not re-derived.
    # ------------------------------------------------------------------
    for field in ("field_id", "sensor_id", "window_start", "window_end"):
        if field not in stream_doc:
            return (
                f"YIELD_ATTRIBUTION_MISMATCH: inputs/sensor_stream.json missing "
                f"binding field '{field}' — forecast cannot be attributed"
            )
        if field not in forecast:
            return (
                f"YIELD_ATTRIBUTION_MISMATCH: payload/yield_forecast.json missing "
                f"binding field '{field}' — forecast cannot be attributed"
            )
        stream_val = stream_doc[field]
        forecast_val = forecast[field]
        if stream_val != forecast_val:
            return (
                f"YIELD_ATTRIBUTION_MISMATCH: '{field}' mismatch: "
                f"sensor_stream={stream_val!r}, forecast claims={forecast_val!r} "
                f"— forecast is not attributable to the bundled sensor stream"
            )
    ...
    # Compare sample count. The builder always commits num_samples ==
    # len(samples) (see _build_bundle.py), so this is a required field with
    # exact-equality semantics, not an optional sanity check: a forecast
    # that omits it, or claims a different count, fails closed rather than
    # being silently skipped.
    bundled_count = forecast.get("num_samples")
    if bundled_count is None:
        return "payload/yield_forecast.json missing 'num_samples'"
    if isinstance(bundled_count, bool) or not isinstance(bundled_count, int):
        return f"payload/yield_forecast.json 'num_samples' is not an integer: {bundled_count!r}"
    if bundled_count != len(samples):
        return (
...
def main() -> int:
    ...
    if error is None:
        print("RE_DERIVED: re-derivation matched bundled forecast")
        return 0
