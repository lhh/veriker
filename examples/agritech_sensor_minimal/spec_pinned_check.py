"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for agritech_sensor.

Per-dir migration of the agritech_sensor pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + YieldFusionReDerivationCheck.py +
yield_fusion_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/agritech_sensor.spec.json).
  - The verifier re-derives the representative output (scalar yield_score)
...
Usage:
    python examples/agritech_sensor_minimal/spec_pinned_check.py
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("agritech_sensor__legacy_build_bundle", _HERE / "_build_bundle.py")
    _m = _ilu.module_from_spec(_s)
    _s.loader.exec_module(_m)
    return _m.build

_build_legacy_bundle = _load_legacy_build()
from agritech_sensor_recompute import compute_yield_score  # noqa: E402

_OUTPUT_ID = "agritech_yield_score"
_TYPE_KEY = "agritech_yield_score"
_SPEC_SRC = _HERE / "spec_pinned" / "agritech_sensor.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned agritech_sensor bundle in out_dir. Reuses the legacy
    ...
    stream_doc = json.loads(
        (out_dir / "inputs" / "sensor_stream.json").read_bytes()
    )
    weights = json.loads(
        (out_dir / "payload" / "fusion_weights.json").read_bytes()
    )
    claimed = compute_yield_score(stream_doc["samples"], weights)
...
def make_verifier(anchor=None):
    ...
    from agritech_sensor_recompute import AgritechSensorRecompute  # noqa: PLC0415

    register_primitive(AgritechSensorRecompute())
    return BundleVerifier(plugins=[FileIntegrityManySmall()], spec_anchor=anchor)


def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the agritech_sensor exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  agritech_sensor  (spec-pinned dispatch)")
            return 0
        print("FAIL  agritech_sensor  (spec-pinned dispatch)", file=sys.stderr)
