"""verify.py — agritech_sensor_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  YieldFusionReDerivationCheck   §C6 — agritech yield-score re-derivation

Usage:
    python examples/agritech_sensor_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/agritech_sensor_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# YieldFusionReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Agritech sensor-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
