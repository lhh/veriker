"""verify.py — caselaw_citation_gate_minimal domain pilot bundle verifier.
...
Registers the CaselawGateRecompute primitive via the verifier registry API
(NOT by editing audit_bundle/rederivation/primitives/). The primitive is
defined in the pilot module and registered here, after audit_bundle is on
sys.path.
...
Spec-pinned dispatch (Axis-2):
  ...
  (spec_pinned/caselaw_gate.spec.json) — NOT from the bundle's spec/ copy.
  A producer who ships a weaker spec in the bundle gets a SHA the anchor does
  not list, so it is not authoritative and dispatch fails closed.

Usage:
    python examples/caselaw_citation_gate_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/caselaw_citation_gate_minimal/verify.py -> parents[2] = pkg root.
...
# Register the primitive AFTER audit_bundle is importable. The primitive module
# keeps its RecomputedValue import deferred, so it stays importable standalone in
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "caselaw_citation_gate_minimal audit bundle verifier "
