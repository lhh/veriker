...
Covers five surfaces:
  ...
  2. Hide-the-fabrication: flip outputs/gate_verdict.json decision to
     ...
     ONLY thing that fires is the re-derivation -> RE_DERIVATION_MISMATCH. This is
  ...
  3. Hide-one-misquote (granularity): flip ONLY C-03's claimed status from
     ...
     recompute disagrees per-citation -> RE_DERIVATION_MISMATCH. Proves the
...
def test_hidden_fabrication_fails_rederivation(tmp_path):
    ...
    the lie -> RE_DERIVATION_MISMATCH, and NOT a file-SHA failure."""
    bundle_dir = tmp_path / "bundle"
    _build(bundle_dir)
...
def test_hidden_single_misquote_fails_rederivation(tmp_path):
    ...
    The recompute disagrees on that one record -> RE_DERIVATION_MISMATCH. Proves
...
def test_verifier_path_is_independent_of_the_producer_helper():
    ...
    """
    corpus = [
        {"reporter_cite": "1 X. 1", "holding_text": "the holding says alpha beta"},
    ]
    assertions = [
