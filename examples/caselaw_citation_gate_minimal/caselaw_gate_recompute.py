...
def _recompute_gate_verdict_independent(corpus: list, assertions: list) -> dict:
    ...
    f(x) == f(x), so an error in the RULE ITSELF -- what counts as rooted, how
    a cite is normalized, when the gate may auto-approve -- was baked into the
    claimed verdict and the re-derivation alike and could not be caught. A
    tampered VALUE was still caught either way; method independence was not.

    Expressed differently on purpose: resolution is a two-pass build (collect,
    then detect collisions) and the decision is derived by counting non-rooted
    statuses rather than by an all() over the citation list.
    """
    if not isinstance(corpus, list):
        raise TypeError("corpus must be a JSON array of rooted records")
    if not isinstance(assertions, list):
    ...
    for record in corpus:
        ...
        cite_key = _norm(record["reporter_cite"])
        if cite_key in by_cite:
            raise ValueError(
                f"duplicate reporter_cite in corpus: {record['reporter_cite']!r}"
            )
        by_cite[cite_key] = record
    ...
    for assertion in assertions:
        ...
        if match is None:
            ...
            authority_text = _norm(match["holding_text"])
