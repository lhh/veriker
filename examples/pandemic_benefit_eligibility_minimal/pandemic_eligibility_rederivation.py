...
FIX-E (claim-set coverage sweep): the bundled decision also carries period_week and
disbursement_system_id. Both fields were previously covered only by the record's own HMAC
signature — but the signing key is disclosed in the bundle (spec/disbursement_hmac_key.hex,
by design, see _DISBURSEMENT_KEY_HEX in _build_bundle.py), so a valid signature only proves
internal self-consistency, not that either field is honest. Both are now bound:
  - period_week is bound by exact equality to the SAME committed input that determines it —
    the applicant's own attested eligibility_period_week (_build_bundle.py sets
    period_week = applicant["eligibility_period_week"]; there is no other source for it).
  - disbursement_system_id is bound against a verifier-held pin (never read from the bundle),
    mirroring the `_LEGITIMATE_RECORDER_IDENTITIES` / `_AUTHORIZED_APPROVER_HOSTS` pattern
    used elsewhere in this codebase: this synthetic
    deployment has exactly one provisioned disbursement system, so the verifier pins its
    identity as a module constant rather than trusting whatever the bundle claims.

Fail-closed: a decision is RE_DERIVED only if (applicant known AND rule
set present AND re-derivation matches both verdict and amount AND signature valid AND
period_week matches the applicant's attested eligibility_period_week AND disbursement_system_id
is in the verifier-held roster). Any mismatch => RE_DERIVATION_MISMATCH,
PERIOD_WEEK_MISMATCH, or UNAUTHORIZED_DISBURSEMENT_SYSTEM respectively.

§C6 (domain-agnostic re-derivation) + §C16 spirit (verifier decides,
...
# --- verifier-held disbursement-system roster (auditor-anchored; NEVER read from the bundle) ---
# Mirrors the verifier-held roster pattern other pilots use for recorder and
# approver identities (`_LEGITIMATE_RECORDER_IDENTITIES`, `_PARTICIPANT_HOSTS`):
# a hardcoded set of legitimate actor identities the *verifier* pins,
# which the producer (the disbursement system itself) cannot weaken by claiming a different
# one. This synthetic deployment has exactly one provisioned disbursement system (see
# `_DISBURSEMENT_SYSTEM_ID` in _build_bundle.py) — a production posture would populate this
# from the benefit program's real system-provisioning registry, not a literal.
_LEGITIMATE_DISBURSEMENT_SYSTEM_IDS = frozenset(
    {
        "system://cra-pandemic-benefit-disbursement-v1",
    }
)
...
def _verify(bundle_dir: Path) -> tuple[str | None, list[dict]]:
    ...
    if rules.get("schema") != "pandemic-benefit-rules-v1":
        return (
            f"RULES_SCHEMA: expected 'pandemic-benefit-rules-v1', got {rules.get('schema')!r}",
            [],
        )
    ...
    applicants = {
        str(a["applicant_id"]): a for a in applicants_raw if "applicant_id" in a
    }
    ...
    if not isinstance(decisions, list):
        return (
            "DECISIONS_SCHEMA: payload/disbursement_decisions.json must be a JSON array",
            [],
        )
    ...
    for idx, dec in enumerate(decisions):
        try:
            ...
            claimed_period_week = int(dec["period_week"])
        ...
        # 1a. disbursement_system_id must be the verifier-held, deployment-pinned identity —
        # the HMAC (step 1) only proves internal self-consistency of the record, not that the
        # claimed system is the one actually provisioned (the signing key is disclosed in the
        # bundle by design, so a producer holding it can forge a valid signature over any
        # system_id). Fail-closed on any identity outside the verifier-held roster.
        if disbursement_system_id not in _LEGITIMATE_DISBURSEMENT_SYSTEM_IDS:
            return (
                f"UNAUTHORIZED_DISBURSEMENT_SYSTEM: {applicant_id}: disbursement_system_id "
                f"{disbursement_system_id!r} is not in the verifier-held roster of provisioned "
                f"disbursement systems {sorted(_LEGITIMATE_DISBURSEMENT_SYSTEM_IDS)} (fail-closed)",
                ledger,
            )
        ...
        # 2a. period_week must equal the applicant's own attested eligibility_period_week — the
        # only committed input that determines it (_build_bundle.py sets
        # period_week = applicant["eligibility_period_week"]; there is no other source). The
        # signature (step 1) does not establish this relationship, only that the record is
        # internally self-consistent under whatever value the producer chose to sign.
        try:
            attested_period_week = int(applicant.get("eligibility_period_week", 0))
        except (TypeError, ValueError) as exc:
            return (f"REDERIVATION_ERROR: {applicant_id}: {exc} (fail-closed)", ledger)
        if claimed_period_week != attested_period_week:
            return (
                f"PERIOD_WEEK_MISMATCH: {applicant_id}: disbursement decision claims "
                f"period_week={claimed_period_week} but the applicant's attested "
                f"eligibility_period_week is {attested_period_week}",
                ledger,
            )
        ...
        if claimed_verdict != verifier_verdict:
            return (
                f"RE_DERIVATION_MISMATCH: {applicant_id}: "
        ...
        ledger.append(
            {
                ...
                "period_week": claimed_period_week,
            }
        )
...
def main() -> int:
    ...
    if args.emit_ledger:
        ...
        for row in ledger:
            ...
            print(
                f"  {row['applicant_id']:<12} [{row['verifier_verdict']:>8}]  {amt_s}"
            )
