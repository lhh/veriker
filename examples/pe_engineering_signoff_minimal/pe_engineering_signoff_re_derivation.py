...
def _load_analyses_inputs(bundle_dir: Path) -> list[dict] | None:
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] inputs/analyses.json: "
...
def _load_analysis_outputs(bundle_dir: Path) -> list[dict] | None:
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] payload/engineering_analyses.json: "
            f"JSON parse error: {exc}",
            file=sys.stderr,
        )
        return None
...
def main() -> int:
    ...
    for analysis in analyses_inputs:
        ...
        if aid is None:
            print(
                "[RE_DERIVATION_MISMATCH] analysis missing analysis_id",
                file=sys.stderr,
            )
            return 1
        ...
        if bundled is None:
            print(
                f"[RE_DERIVATION_MISMATCH] analysis_id={aid!r} "
