"""verify.py — pe_engineering_signoff_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  PeEngineeringSignoffReDerivationCheck       §C6  — cantilever-beam re-derivation +
                                                      PE-stamp HMAC re-verification
  DispatchRecordWellformedCheck               §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"FEA_SOLVE", "PE_STAMP", "COMPUTE"})
     admits the domain-specific op kinds introduced by this pilot)

Usage:
    python examples/pe_engineering_signoff_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/pe_engineering_signoff_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# PeEngineeringSignoffReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from PeEngineeringSignoffReDerivationCheck import PeEngineeringSignoffReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "PE engineering signoff minimal audit bundle verifier "
            "(AUDIT_BUNDLE_CONTRACT §C5 / NCEES Model Law PE stamp liability)"
        )
    )
    parser.add_argument(
        "--bundle-dir",
