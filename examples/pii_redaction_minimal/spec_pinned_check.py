"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for pii_redaction_minimal.

Per-dir migration of the pii_redaction_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + PIIRedactionReDerivationCheck.py +
pii_redaction_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/pii_redaction.spec.json).
  - The verifier re-derives the representative output (the aggregated redaction
    spans — the unordered set of [token_start, token_end, category] tuples)
    IN-PROCESS via the registered primitive (constrained-Viterbi over the committed
    BIOES logits + bias), and compares with the generic `set` comparator
    (order-independent). No subprocess, no bundle-supplied code.
...
Usage:
    python examples/pii_redaction_minimal/spec_pinned_check.py
...
# Legacy builder (lays down payload/manifest) + the shared canonical compute
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "pii_redaction_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from pii_redaction_recompute import compute_redaction_spans  # noqa: E402

_OUTPUT_ID = "pii_redaction_spans"
_TYPE_KEY = "pii_redaction_spans"
_SPEC_SRC = _HERE / "spec_pinned" / "pii_redaction.spec.json"
...
def _honest_spans(out_dir: Path) -> list[list]:
    """Auditor-side canonical recompute of the honest redaction spans from the built
    bundle's committed BIOES logits + transition-bias vector (Viterbi decode)."""
    logits_obj = json.loads(
        (out_dir / "payload" / "bioes_logits.json").read_text(encoding="utf-8")
    )
    output_obj = json.loads(
        (out_dir / "payload" / "redaction_output.json").read_text(encoding="utf-8")
    )
    return compute_redaction_spans(
        logits_obj["logits"], output_obj["bias_vector"], output_obj["categories"]
    )
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned pii_redaction_minimal bundle in out_dir. Reuses the legacy
    builder for payload, then overlays the beta shape: an auditor spec under spec/, a
...
def make_verifier(anchor=None):
    ...
    from pii_redaction_recompute import PiiRedactionRecompute  # noqa: PLC0415

    register_primitive(PiiRedactionRecompute())
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the pii_redaction_minimal pilot"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  pii_redaction_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  pii_redaction_minimal  (spec-pinned dispatch)", file=sys.stderr)
