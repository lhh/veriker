...
class PIIRedactionReDerivationCheck:
    ...
    def check(self, bundle_dir: Path, manifest) -> PluginResult:
        ...
        # Parse structured JSON error from stdout if present. The pack emits a
        # dedicated per-defect "error" code (PII_REDACTION_BIAS_VECTOR_MISMATCH,
        # PII_REDACTION_MODEL_SHA_ABSENT, or the generic
        # RE_DERIVATION_MISMATCH) — surface it as a detail prefix.
        # The verifier wraps ALL plugin failures at reason_code="plugin_failed"
        # and only forwards .detail upward, so the per-defect diagnosis has to
        # live in detail, not in PluginResult.reason_code, to reach the auditor.
        detail = (
            stdout or (result.stderr or b"").decode("utf-8", errors="replace")[:512]
        )
        error_code = "RE_DERIVATION_MISMATCH"
        try:
            err_obj = json.loads(stdout)
            error_code = err_obj.get("error", error_code)
