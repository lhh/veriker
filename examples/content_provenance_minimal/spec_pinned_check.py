"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for content_provenance.

Per-dir migration of the content_provenance pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + ContentProvenanceReDerivationCheck.py +
content_provenance_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/content_provenance.spec.json).
  - The verifier re-derives the representative output (the content SHA-256 hex
    digest) IN-PROCESS via the registered primitive, and compares it with the
...
Usage:
    python examples/content_provenance_minimal/spec_pinned_check.py
...
# Legacy builder (lays down artifact/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("content_provenance__legacy_build_bundle", _HERE / "_build_bundle.py")
    _m = _ilu.module_from_spec(_s)
    _s.loader.exec_module(_m)
    return _m.build

_build_legacy_bundle = _load_legacy_build()
from content_provenance_recompute import compute_content_sha  # noqa: E402

_OUTPUT_ID = "content_sha"
_TYPE_KEY = "content_sha"
_SPEC_SRC = _HERE / "spec_pinned" / "content_provenance.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned content_provenance bundle in out_dir. Reuses the legacy
    builder for artifact/payload, then overlays the β shape: an auditor spec under
    ...
    content_bytes = (out_dir / "artifact" / "content.txt").read_bytes()
    claimed = compute_content_sha(content_bytes)
    ...
    # primitive re-derives the content SHA on the outputs/ claim, and the
    # legacy re-derivation plugin (the check that reads and compares
    # payload/provenance_result.json's fields) is deliberately not wired here.
    # Carrying the declaration would be a false coverage story, so the variant
    # drops it and the verdict carries the honest "claimset: not declared"
    # disclosure. The declared pilot path is exercised by
    # tests/test_content_provenance_minimal.py.
...
def make_verifier(anchor=None):
    ...
    from content_provenance_recompute import ContentProvenanceRecompute  # noqa: PLC0415

    register_primitive(ContentProvenanceRecompute())
    return BundleVerifier(plugins=[FileIntegrityManySmall()], spec_anchor=anchor)


def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the content_provenance exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  content_provenance  (spec-pinned dispatch)")
            return 0
        print("FAIL  content_provenance  (spec-pinned dispatch)", file=sys.stderr)
