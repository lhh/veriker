...
def main() -> int:
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] payload structure malformed: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] re-derivation raised exception: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if len(rederived_ledger) != bundled_enum_count:
        print(
            f"[RE_DERIVATION_MISMATCH] enumerated_count mismatch: "
        ...
        for field in _COMPARE_FIELDS:
            ...
            if field == "pk":
                ...
                if rd_val is None or bd_val is None:
                    print(
                        f"[RE_DERIVATION_MISMATCH] ledger[{i}].pk: "
            ...
            if isinstance(rd_val, float) and isinstance(bd_val, float):
                if abs(rd_val - bd_val) > 1e-9:
                    print(
                        f"[RE_DERIVATION_MISMATCH] ledger[{i}].{field}: "
                        f"re-derived={rd_val!r}, bundled={bd_val!r}",
                        file=sys.stderr,
                    )
                    return 1
    ...
    if rederived_passed != bundled_passed_count:
        print(
            f"[RE_DERIVATION_MISMATCH] passed_count mismatch: "
