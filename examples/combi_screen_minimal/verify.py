"""verify.py — combi_screen_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  CombiScreenReDerivationCheck      §C6  — combinatorial screening re-derivation (local pack)
  DispatchRecordWellformedCheck     §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"DOCK_SCREEN","COMPUTE"}) admits the new
     domain-specific DOCK_SCREEN kind introduced by this pilot)

Usage:
    python examples/combi_screen_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/combi_screen_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# CombiScreenReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from CombiScreenReDerivationCheck import CombiScreenReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Combi-screen-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
