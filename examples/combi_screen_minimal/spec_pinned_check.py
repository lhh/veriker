"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for combi_screen_minimal.

Per-dir migration of the combi_screen_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + CombiScreenReDerivationCheck.py +
combi_screen_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/combi_screen.spec.json).
  - The verifier re-derives the representative output (the advanced (top-K) compound
    id set in payload/combi_screen_result.json) IN-PROCESS via the registered
    primitive (re-enumerate -> Lipinski -> seeded score -> rank -> advance top-K),
    and compares with the generic `set` comparator (order-independent). No
...
Usage:
    python examples/combi_screen_minimal/spec_pinned_check.py
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "combi_screen_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from combi_screen_recompute import compute_advanced_set  # noqa: E402

_OUTPUT_ID = "combi_screen_advanced_set"
_TYPE_KEY = "combi_screen_advanced_set"
_SPEC_SRC = _HERE / "spec_pinned" / "combi_screen.spec.json"
...
def _honest_advanced_set(out_dir: Path) -> list[str]:
    """Auditor-side canonical recompute of the honest advanced (top-K) compound id
    set from the built bundle's committed building blocks + configs + advanced_count."""
    bb = json.loads((out_dir / "inputs" / "building_blocks.json").read_text("utf-8"))
    filter_cfg = json.loads(
        (out_dir / "inputs" / "filter_config.json").read_text("utf-8")
    )
    scoring_cfg = json.loads(
        (out_dir / "inputs" / "scoring_config.json").read_text("utf-8")
    )
    result = json.loads(
        (out_dir / "payload" / "combi_screen_result.json").read_text("utf-8")
    )
    return compute_advanced_set(
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned combi_screen_minimal bundle in out_dir. Reuses the legacy
    ...
    claimed = _honest_advanced_set(out_dir)
...
def make_verifier(anchor=None):
    ...
    from combi_screen_recompute import CombiScreenRecompute  # noqa: PLC0415

    register_primitive(CombiScreenRecompute())
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the combi_screen_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  combi_screen_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  combi_screen_minimal  (spec-pinned dispatch)", file=sys.stderr)
