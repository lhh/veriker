"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for auto_ubi.

Per-dir migration of the auto_ubi_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + AutoUBIReDerivationCheck.py +
auto_ubi_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/auto_ubi.spec.json).
  - The verifier re-derives the representative output (the ordered per-policyholder
    categorical tier list — [{policyholder_id, tier}]) IN-PROCESS via the
    registered primitive, and compares with the generic `exact` comparator. No
    subprocess, no bundle-supplied code.
  - Honest bundle -> PASS; tampered claimed list or tampered input (a trip mutated
    so a tier flips) -> FAIL (RE_DERIVATION_MISMATCH); no auditor anchor /
    substituted spec -> fail-closed (AnchorViolation).

The legacy manifest carries dispatch_records + fragment_anchors (the dispatch-record
/ fragment-anchor surface from the original build). The overlay leaves those intact and only ADDS
outputs[]/spec_files/files[outputs/…] and OVERRIDES typed_checks: the spec-pinned
verifier runs FileIntegrityManySmall + step-5 dispatch only, so typed_checks must
name exactly the registered plugin set -> ["file_integrity_many_small"].

Usage:
    python examples/auto_ubi_minimal/spec_pinned_check.py
...
# Legacy builder (lays down telematics/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "auto_ubi_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from auto_ubi_recompute import compute_tier_list  # noqa: E402

_OUTPUT_ID = "auto_ubi_tier_list"
_TYPE_KEY = "auto_ubi_tier_list"
_SPEC_SRC = _HERE / "spec_pinned" / "auto_ubi.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned auto_ubi bundle in out_dir. Reuses the legacy builder
    for telematics/payload/manifest, then overlays the β shape: an auditor spec
    ...
    trips_path = out_dir / "telematics" / "trips.jsonl"
    rate_table = json.loads((out_dir / "payload" / "rate_table.json").read_bytes())
    trips: list[dict] = []
    for line in trips_path.read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if line:
            trips.append(json.loads(line))
    claimed = compute_tier_list(trips, rate_table)
    ...
    # primitive re-derives the categorical tier list on the outputs/ claim,
    # and the legacy re-derivation plugin (the check that reads and compares
    # payload/rating_decisions.json's and payload/rate_table.json's fields)
    # is deliberately not wired here. Carrying the declaration would be a
    # false coverage story, so the variant drops it and the verdict carries
    # the honest "claimset: not declared" disclosure. The declared pilot
    # path is exercised by tests/test_auto_ubi_minimal.py.
...
def make_verifier(anchor=None):
    ...
    from auto_ubi_recompute import AutoUbiRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the auto_ubi exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  auto_ubi  (spec-pinned dispatch)")
            return 0
        print("FAIL  auto_ubi  (spec-pinned dispatch)", file=sys.stderr)
