...
def build(out_dir: Path) -> None:
    ...
    rate_table_bytes = (
        json.dumps(_RATE_TABLE, indent=2, sort_keys=True) + "\n"
    ).encode("utf-8")
    ...
    decisions_bytes = (
        json.dumps(rating_decisions, indent=2, sort_keys=True) + "\n"
    ).encode("utf-8")
    ...
    content = BundleContent(
        ...
        extra_manifest_fields={
            ...
            # Claim-field coverage declaration: every observed field of
            # BOTH declared payload files must be read-and-compared by a
            # wired check or excused with a committed reason — a field
            # nothing reads fails the verdict closed instead of riding a
            # green PASS. All 9 fields of payload/rating_decisions.json are
            # bound by auto_ubi_re_derivation.py (per-policyholder feature
            # tolerance checks, exact tier/adjustment_pct/trip_count match,
            # and a policyholder_id bijection against telematics/trips.jsonl).
            # All 12 fields of payload/rate_table.json are residual: the rate
            # table is used as CONFIG that feeds the rating_decisions
            # comparison above, but nothing else in this bundle carries an
            # independent second copy of it to check its own values against
            # (unlike a signed model artifact) — schema_version is a version
            # tag on the structure the comparator does read
            # (INSPECTION_ONLY); the three tier descriptions are free-text
            # prose needing judgment to check (NOT_MECHANICALLY_CHECKABLE);
            # the five tier_thresholds fields and the three tier
            # discount_pct/surcharge_pct fields have no binding target in
            # this bundle (NO_BINDING_TARGET): the five tier thresholds, which
            # are inequality operands that select a branch, never compared
            # values (annual_mileage_high_min is not read at all). The three
            # tier adjustment values (surcharge_pct / discount_pct) are NOT
            # residual: each is the other operand of the per-row
            # adjustment_pct comparison and is reported covered under
            # payload/rate_table.json once a row of its tier has compared
            # (wave-2 review finding — they were excused while compared).
            "claimset": {
                "claim_files": {
                    "rate_table": "payload/rate_table.json",
                    "rating_decisions": "payload/rating_decisions.json",
                },
                "residuals": {
                    "rate_table:schema_version": "INSPECTION_ONLY",
                    "rate_table:tiers.high_risk_surcharge.description": "NOT_MECHANICALLY_CHECKABLE",
                    "rate_table:tiers.low_mileage_discount.description": "NOT_MECHANICALLY_CHECKABLE",
                    "rate_table:tiers.standard.description": "NOT_MECHANICALLY_CHECKABLE",
                    "rate_table:tier_thresholds.annual_mileage_high_min": "NO_BINDING_TARGET",
                    "rate_table:tier_thresholds.annual_mileage_low_max": "NO_BINDING_TARGET",
                    "rate_table:tier_thresholds.hard_brake_per_mile_surcharge_threshold": "NO_BINDING_TARGET",
                    "rate_table:tier_thresholds.harsh_accel_per_mile_surcharge_threshold": "NO_BINDING_TARGET",
                    "rate_table:tier_thresholds.late_night_fraction_surcharge_threshold": "NO_BINDING_TARGET",
                },
            },
        },
    )
