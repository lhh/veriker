"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for chat_log_redaction.

Per-dir migration of the chat_log_redaction pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + ChatLogRedactionReDerivationCheck.py +
re_derive/ pack) and its verify.py / test are untouched, and no committed manifest
gains an `outputs` array — the spec-pinned bundle is built to a fresh directory
here, so the substrate's "0 committed manifests declare outputs" inertness
invariant is preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/chat_log_redaction.spec.json).
  - The verifier re-derives the representative output (redacted_output_sha, the
    SHA-256 hex digest of the redacted transcript) IN-PROCESS via the registered
    primitive, re-running the regex + entity-dict redaction over the committed
    inputs, and compares with the generic `exact` comparator. No subprocess, no
...
Usage:
    python examples/chat_log_redaction_minimal/spec_pinned_check.py
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "chat_log_redaction_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from chat_log_redaction_recompute import compute_redacted_output_sha  # noqa: E402

_OUTPUT_ID = "redacted_output_sha"
_TYPE_KEY = "redacted_output_sha"
_SPEC_SRC = _HERE / "spec_pinned" / "chat_log_redaction.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned chat_log_redaction bundle in out_dir. Reuses the legacy
    ...
    # --- Honest claimed value = the auditor's own canonical recompute over the
    #     bundled inputs (transcript + policy). ---
    transcript = (out_dir / "inputs" / "transcript.txt").read_text(encoding="utf-8")
    policy = json.loads(
        (out_dir / "inputs" / "redaction_policy.json").read_text(encoding="utf-8")
    )
    claimed = compute_redacted_output_sha(transcript, policy)
...
def make_verifier(anchor=None):
    ...
    from chat_log_redaction_recompute import ChatLogRedactionRecompute  # noqa: PLC0415

    register_primitive(ChatLogRedactionRecompute())
    return BundleVerifier(plugins=[FileIntegrityManySmall()], spec_anchor=anchor)


def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the chat_log_redaction exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  chat_log_redaction  (spec-pinned dispatch)")
            return 0
        print("FAIL  chat_log_redaction  (spec-pinned dispatch)", file=sys.stderr)
