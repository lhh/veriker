"""verify.py — chat_log_redaction_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  ReDerivationInvocationCheck                §C6 — invokes stdlib re-derivation pack
  ChatLogRedactionReDerivationCheck          §C6 — pilot-specific subprocess wrapper
...
re_derive/chat_log_redaction_pack.py and runs the same re-derivation. The two
verifier shells are equivalent — both invoke the identical stdlib pack.

Usage:
    python examples/chat_log_redaction_minimal/verify.py --bundle-dir <path>
...
# same defence in the top-level verify CLI.
sys.dont_write_bytecode = True

from pathlib import Path  # noqa: E402

# §C5 auditor-independence: locate pkg root relative to this file.
# Layout: examples/chat_log_redaction_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# ChatLogRedactionReDerivationCheck lives alongside this script.
...
from ChatLogRedactionReDerivationCheck import ChatLogRedactionReDerivationCheck  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "chat_log_redaction_minimal audit bundle verifier "
