"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for bom_minimal.

Per-dir migration of the bom_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + BomReDerivationCheck.py + bom_re_derivation.py)
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/bom.spec.json).
  - The verifier re-derives the representative output (the full resolved dependency
    tree: a dict {root, nodes, resolution_order}) IN-PROCESS via the registered
    primitive, and compares with the generic `exact` comparator (structural
    equality on parsed JSON objects). No subprocess, no bundle-supplied code.
...
Usage:
    python examples/bom_minimal/spec_pinned_check.py
...
# Legacy builder (lays down lockfile/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "bom__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from bom_recompute import compute_resolved_tree  # noqa: E402

_OUTPUT_ID = "bom_resolved_tree"
_TYPE_KEY = "bom_resolved_tree"
_SPEC_SRC = _HERE / "spec_pinned" / "bom.spec.json"
...
def _honest_resolved_tree(out_dir: Path) -> dict:
    """Auditor-side canonical recompute of the honest resolved_tree from the
    built bundle's committed lockfile."""
    lockfile = json.loads((out_dir / "lockfile" / "lockfile.json").read_bytes())
    return compute_resolved_tree(lockfile)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned bom bundle in out_dir. Reuses the legacy builder for
    lockfile/payload, then overlays the beta shape: an auditor spec under spec/, a
    ...
    claimed = _honest_resolved_tree(out_dir)
...
def make_verifier(anchor=None):
    ...
    from bom_recompute import BomRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the bom_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  bom_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  bom_minimal  (spec-pinned dispatch)", file=sys.stderr)
