"""verify.py — prior_auth_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  PriorAuthReDerivationCheck      §C6 — prior-auth decision re-derivation +
                                        provider attestation HMAC re-verification
  DispatchRecordWellformedCheck   §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"MEDICAL_NECESSITY_EVAL", "PROVIDER_ATTEST", "COMPUTE"})
     admits the domain-specific op kinds introduced by this pilot)

Usage:
    python examples/prior_auth_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/prior_auth_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# PriorAuthReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from PriorAuthReDerivationCheck import PriorAuthReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Prior-auth-minimal audit bundle verifier "
            "(AUDIT_BUNDLE_CONTRACT §C5 / CO Reg 10-1-1 § 5.A.5)"
        )
    )
    parser.add_argument(
        "--bundle-dir",
