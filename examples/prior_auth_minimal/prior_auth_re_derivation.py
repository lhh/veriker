...
payload/attestation_key.hex. The HMAC message binds provider_role
(e.g. medical_director vs. attending_physician) alongside provider_id,
decision_id, provider_verdict, and attestation_timestamp — WHO signed off
is legally significant under CO Reg 10-1-1 §5.A.5 and was previously
unbound, letting the role be forged freely with no other copy in the
...
Exit codes:
  ...
  1  mismatch found — see stderr for [RE_DERIVATION_MISMATCH] or
...
def _load_findings(bundle_dir: Path) -> list[dict] | None:
    ...
    for i, line in enumerate(p.read_text(encoding="utf-8").splitlines()):
        ...
        try:
            ...
            print(
                f"[RE_DERIVATION_MISMATCH] clinical/findings.jsonl line {i}: "
...
def _load_rules(bundle_dir: Path) -> list[dict] | None:
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] clinical/plan_rules.json: "
            f"JSON parse error: {exc}",
            file=sys.stderr,
        )
        return None
...
def _load_decisions(bundle_dir: Path) -> list[dict] | None:
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] payload/prior_auth_decisions.json: "
            f"JSON parse error: {exc}",
            file=sys.stderr,
        )
        return None
...
def _compute_attestation_hmac(
    ...
    provider_role). provider_role is bound so WHO signed off (medical_director vs.
    attending_physician — CO Reg 10-1-1 §5.A.5) cannot be forged independently of
    the rest of the attestation; the field has no other copy in the bundle."""
    msg = f"{provider_id}|{decision_id}|{provider_verdict}|{timestamp}|{provider_role}".encode(
        "utf-8"
    )
...
def main() -> int:
    ...
    for request in findings:
        ...
        if req_id is None:
            print(
                "[RE_DERIVATION_MISMATCH] request missing request_id",
                file=sys.stderr,
            )
            return 1
        ...
        if bundled is None:
            print(
                f"[RE_DERIVATION_MISMATCH] request_id={req_id!r} "
    ...
    for row in provenance:
        ...
        if not _hmac.compare_digest(expected_hmac, stored_hmac):
            print(
                ...
                f"provider_role={provider_role!r}: "
                f"HMAC mismatch — stored={stored_hmac[:16]!r}... "
                f"expected={expected_hmac[:16]!r}... "
                "(provider_verdict, timestamp, or provider_role may have been "
                "tampered)",
                file=sys.stderr,
            )
            return 1
