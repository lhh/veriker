...
Two checks in one plugin:
  ...
  2. Provider attestation HMAC re-verification: for every row in
     ...
     provider_role) under the bundle's committed attestation key and asserts it
     matches the stored attestation_hmac. provider_role (e.g. medical_director
     vs. attending_physician) is bound here — under CO Reg 10-1-1 §5.A.5, WHO
     signed off on an AI-assisted UM decision is legally significant, and this
     field has no other copy anywhere in the bundle to cross-check against.

Reason codes:
  RE_DERIVED                              — both invariants passed
  RE_DERIVATION_MISMATCH                  — decision-tree mismatch
