...
for each row in payload/decision_provenance.jsonl:
    expected_hmac = HMAC-SHA256(
        ...
        msg="{provider_id}|{decision_id}|{provider_verdict}|{attestation_timestamp}|{provider_role}"
    )
    assert expected_hmac == row.attestation_hmac
```

`provider_role` (e.g. `medical_director` vs. `attending_physician`) is bound into the
HMAC message. Under CO Reg 10-1-1 §5.A.5, WHO signed off — not just that someone
did — is legally significant; this field has no other copy anywhere in the bundle
to cross-check against, so the HMAC binding is the only thing standing between it
and free forgery.
...
Per this repo's signing vocabulary ("sign", §C-series): an `_hmac` scheme
is **symmetric tamper-evidence within a trust domain — any key holder can mint —
NOT non-repudiation**. Because the key travels with the bundle, the trust domain
here is "anyone who has the bundle," which includes the bundle's own producer.
Re-verifying the HMAC (now over `provider_id`, `decision_id`, `provider_verdict`,
`attestation_timestamp`, and `provider_role` together) proves the five fields are
**internally self-consistent as shipped** and catches a downstream party who edits
the record without also recomputing its signature (Tamper B/C below). It does
**not** give non-repudiation, and it does **not** protect against an adversarial
*producer*: a producer who fabricates the entire bundle — including a false
`provider_role` — holds the same key the verifier uses and can mint a
self-consistent forgery from scratch. Closing that gap requires an exogenous,
verifier-wired key delivered out-of-band (the fix already landed for
`examples/gxp_part11_minimal`'s Pattern-2 regression — see
`tests/test_gxp_rederivation_exogenous_key.py`); this minimal pilot has not been
migrated to that architecture, so do not read a green `PASS` here as proof that a
named provider actually reviewed the case — only that the shipped record has not
been altered in transit without also updating its own signature.
...
**Tamper C — provider role (HMAC mismatch):**
Mutate `provider_role` in a provenance row without recomputing its HMAC (e.g. an
attending physician's sign-off relabeled as a medical director's — the escalation
CO Reg 10-1-1 §5.A.5 cares about):
...
p = Path('/tmp/prior_auth_bundle/payload/decision_provenance.jsonl')
lines = p.read_text().splitlines()
row = json.loads(lines[2])            # PA-2026-003, attending_physician
row['provider_role'] = 'medical_director'   # relabel without recomputing HMAC
...
Re-run `verify.py` — expect exit 1, reason `PRIOR_AUTH_PROVIDER_ATTESTATION_INVALID`.
Before this fix, `provider_role` was excluded from the HMAC message and had no
other copy in the bundle, so this exact mutation passed silently — the confirmed
verifier defect this pilot now closes.
