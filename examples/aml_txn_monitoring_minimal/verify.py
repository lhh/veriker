"""verify.py — aml_txn_monitoring_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  AmlTxnMonitoringReDerivationCheck   §C6 — AML COAF-report re-derivation (local pack)
  DispatchRecordWellformedCheck       §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"RULE_TREE_EVAL", "COMPUTE"}) admits the new
     domain-specific RULE_TREE_EVAL kind introduced by this pilot)

Usage:
    python examples/aml_txn_monitoring_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/aml_txn_monitoring_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# AmlTxnMonitoringReDerivationCheck lives alongside this script (AB4).
...
from AmlTxnMonitoringReDerivationCheck import AmlTxnMonitoringReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="AML transaction monitoring audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
