...
Per-dir migration of the aml_txn_monitoring_minimal pilot onto the disclosed
spec-pinned, auditor-anchored, in-process recompute-then-compare method (S0). This
is ADDITIVE: the legacy bundle (_build_bundle.py + AmlTxnMonitoringReDerivationCheck.py
+ aml_txn_monitoring_re_derivation.py) and its verify.py / test are untouched, and
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/aml_txn_monitoring.spec.json).
  - The verifier re-derives the representative output (the per-customer
    coaf_report_triggered decision list) IN-PROCESS via the registered primitive,
    and compares with the generic `exact` comparator (element-wise list equality).
...
Usage:
    python examples/aml_txn_monitoring_minimal/spec_pinned_check.py
...
# Legacy builder (lays down transactions/rule_tree/baselines/payload/manifest) +
# the shared canonical compute function (imported standalone — no audit_bundle needed).
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "aml_txn_monitoring_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from aml_txn_monitoring_recompute import compute_coaf_decisions  # noqa: E402

_OUTPUT_ID = "aml_coaf_report_decisions"
_TYPE_KEY = "aml_coaf_report_decisions"
_SPEC_SRC = _HERE / "spec_pinned" / "aml_txn_monitoring.spec.json"
...
def _honest_decisions(out_dir: Path) -> list[dict]:
    """Auditor-side canonical recompute of the honest per-customer COAF-report
    decision list from the built bundle's committed inputs."""
    txns: list[dict] = []
    txt = (out_dir / "transactions" / "transactions.jsonl").read_text(encoding="utf-8")
    for line in txt.splitlines():
        line = line.strip()
        if line:
            txns.append(json.loads(line))
    rule_tree = json.loads((out_dir / "rule_tree" / "rule_tree.json").read_bytes())
    baselines = json.loads(
        (out_dir / "baselines" / "customer_baselines.json").read_bytes()
    )
    return compute_coaf_decisions(txns, rule_tree, baselines)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned aml bundle in out_dir. Reuses the legacy builder for
    transactions/rule_tree/baselines/payload, then overlays the beta shape: an
    auditor spec under spec/, a producer claimed-value file under outputs/, a
    manifest.outputs entry, manifest.spec_files, and a typed_checks set matching
    ...
    claimed = _honest_decisions(out_dir)
...
def make_verifier(anchor=None):
    ...
    from aml_txn_monitoring_recompute import AmlTxnMonitoringRecompute  # noqa: PLC0415

    register_primitive(AmlTxnMonitoringRecompute())
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the aml_txn_monitoring_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  aml_txn_monitoring_minimal  (spec-pinned dispatch)")
            return 0
        print(
            "FAIL  aml_txn_monitoring_minimal  (spec-pinned dispatch)", file=sys.stderr
        )
