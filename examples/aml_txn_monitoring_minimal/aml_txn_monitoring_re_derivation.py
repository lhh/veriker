...
[RE_DERIVATION_MISMATCH] on stderr.
...
def main() -> int:
    ...
    if rule_tree is None:
        print(
            "[RE_DERIVATION_MISMATCH] rule_tree.json missing or unreadable",
            file=sys.stderr,
        )
        return 1
    ...
    if baselines is None:
        print(
            "[RE_DERIVATION_MISMATCH] customer_baselines.json missing or unreadable",
            file=sys.stderr,
        )
        return 1
    ...
    if bundled is None:
        print(
            "[RE_DERIVATION_MISMATCH] payload/coaf_reports.json missing or unreadable",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] malformed input structure: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    for txn in transactions:
        ...
        if not cid:
            print(
                "[RE_DERIVATION_MISMATCH] transaction missing customer_id",
                file=sys.stderr,
            )
            return 1
    ...
    for cid, bundled_rec in sorted(bundled_customers.items()):
        ...
        if bundled_velocity != velocity:
            print(
                f"[RE_DERIVATION_MISMATCH] customer {cid}: "
