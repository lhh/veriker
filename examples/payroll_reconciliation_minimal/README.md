...
The builder also emits `data/disbursements.csv` — a committed, SHA-pinned
record of what was actually disbursed to each paid employee, written from the
same source `payload/paychecks.json`'s `issued_net_cents` is computed from.
The checker binds the two together (see Step 2, `payroll_re_derivation`); see
"Scope honesty" for what that binding does and does not establish.
...
| `file_integrity_many_small`  | §C9      | inputs (incl. the disbursement ledger), ledger, coverage row, event log, and proof obligation are untampered |
| `payroll_re_derivation`      | §C6      | every paycheck re-derives from rules+inputs; `issued_net_cents` binds to the committed `data/disbursements.csv` ledger; every clawback amount re-derives from that bound issued amount |
...
`RE_DERIVATION_MISMATCH … clawback_cents mismatch`. **This is the Phoenix
...
`test_tamper_clawback_caught_by_rederivation` test.)
...
**(d) Joint-shift `issued_net_cents`/`clawback_cents`, SHA-consistently
(FIX-E)** — shift E0008's `issued_net_cents` **and** `clawback_cents` in
`payload/paychecks.json` by the same delta (e.g. +100) and re-stamp the
manifest SHA. Before FIX-E this was undetectable: `clawback_cents` was only
checked against `issued_net_cents`, both self-reported in the same file, so a
joint constant shift left the internal check satisfied. Now `issued_net_cents`
is additionally bound to the committed `data/disbursements.csv` record, which
the attack left untouched — the verifier exits `1` with
`PAYROLL_DISBURSEMENT_UNBOUND … issued_net_cents mismatch`. (See
`test_tamper_issued_net_and_clawback_joint_shift_caught_by_disbursement_binding`.)

**(e) Verifier-vs-claim discharge divergence (S2 "retain-and-still-reject")** —
...
├── _build_bundle.py            # synthesizes fixtures, computes the ledger + disbursement ledger, signs the S2 discharge
...
├── payroll_re_derivation.py    # stdlib re-derivation pack (integer-cents pay engine; binds issued_net_cents to data/disbursements.csv)
├── pilot.json                  # pilot registry entry
├── README.md
└── tests/
    ├── test_payroll_reconciliation_minimal.py   # happy path + tamper tests (incl. FIX-E disbursement-binding)
...
`data/disbursements.csv`, `payload/paychecks.json`,
`coverage/period_2026_05.json`, `status_change_log.jsonl`,
`proofs/payroll_conservation.smt2`, `manifest.json`.
...
**The disbursement ledger is producer-committed evidence, not a bank-side
attestation.** `data/disbursements.csv` closes the specific hole where
`payload/paychecks.json`'s `issued_net_cents` and `clawback_cents` were a free
payload assertion — moving both by the same constant delta previously left the
internal reconciliation check satisfied, because nothing in the bundle
recorded what was actually paid independently of that same payload. Binding
`issued_net_cents` to a separately SHA-pinned committed file kills that
specific payload↔evidence inconsistency: a producer can no longer doctor the
reconciliation ledger alone. It does **not** establish the disbursement
ledger's own wall-clock truth — this pilot's builder writes both files from
the same synthetic source, so a producer willing to forge *both* committed
files consistently is not caught by this leg. The stronger leg — out of scope
here — is an independently-sourced attestation of the actual disbursement
(e.g. a bank-side payment-rail record, analogous to the C19.B cross-org
attestation premium leg), which this pilot does not carry.
