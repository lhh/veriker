...
§C6 (re-derivation pack — domain-agnostic substrate).
...
Reads:
  ...
  data/disbursements.csv      — committed per-employee disbursement ledger (what
                                 was actually paid out; input snapshot, FIX-E)
...
For each bundled paycheck:
  ...
  2. FIX-E — bind issued_net_cents to committed evidence: the paycheck's
     employee must have a row in data/disbursements.csv, and the bundled
     issued_net_cents must equal that committed value. Before this fix,
     issued_net_cents was a free payload assertion — data/pay_events.csv
     carried no column for the disbursed amount, so payload/paychecks.json's
     issued_net_cents and clawback_cents could be moved together by any
     constant delta and still pass (clawback_cents was only checked against
     issued_net_cents, both self-reported in the same file). Binding
     issued_net_cents to a separately SHA-pinned input file closes that:
     moving the pair in payload/paychecks.json without also moving
     data/disbursements.csv now diverges and is caught here.
     HONESTY RAIL: this binds payload<->evidence consistency, not the
     disbursement ledger's own truth — a colluding producer that forges both
     committed files consistently is not caught by this leg (see README
     "Scope honesty"; a bank-side disbursement attestation would be the
     stronger leg).
  3. Re-derive clawback_cents = issued_net_cents - net_cents (correct net).
...
the first mismatch. A FIX-E disbursement-binding failure is additionally
prefixed with the dedicated PAYROLL_DISBURSEMENT_UNBOUND code inside the
description, so it is grep-able in the detail text even when a caller (e.g.
the verifier's generic exception wrapper) only surfaces reason_code
"plugin_failed".
...
# FIX-E (claim-set coverage sweep): dedicated reason code for the
# disbursement-binding leg. PayrollReDerivationCheck wraps every subprocess
# failure under the single reason_code "RE_DERIVATION_MISMATCH" (and
# the verifier's own generic exception path separately surfaces
# reason_code="plugin_failed" for unhandled errors) — either way only
# `detail` is guaranteed to carry the specific failure, so the dedicated code
# is embedded as a literal prefix in the returned description, grep-able in
# stderr/detail regardless of which reason_code wraps it.
_DISBURSEMENT_UNBOUND_CODE = "PAYROLL_DISBURSEMENT_UNBOUND"
...
def _verify(bundle_dir: Path) -> str | None:
    ...
    if not disbursements_path.exists():
        return (
            f"{_DISBURSEMENT_UNBOUND_CODE}: data/disbursements.csv absent from "
            f"bundle_dir {bundle_dir} — issued_net_cents has no committed evidence "
            "to bind to"
        )
    ...
    try:
        rows = list(
            csv.DictReader(events_path.read_bytes().decode("utf-8").splitlines())
        )
    ...
    # FIX-E: index the committed disbursement ledger by employee_id. This is
    # the "actually disbursed" evidence — a committed input, independent of
    # payload/paychecks.json's own issued_net_cents claim.
    try:
        disb_rows = list(
            csv.DictReader(disbursements_path.read_bytes().decode("utf-8").splitlines())
        )
    except Exception as exc:  # noqa: BLE001 — stdlib pack, surface any parse error
        return f"failed to parse data/disbursements.csv: {exc}"
    try:
        disbursements: dict[str, int] = {
            r["employee_id"]: int(r["issued_net_cents"]) for r in disb_rows
        }
    except (KeyError, TypeError, ValueError) as exc:
        return f"{_DISBURSEMENT_UNBOUND_CODE}: malformed data/disbursements.csv row — {exc}"
    ...
    for idx, rec in enumerate(ledger):
        ...
        if ev is None:
            return (
                f"paycheck[{idx}] {emp}: no matching pay event in data/pay_events.csv"
            )
        ...
        # FIX-E: bind issued_net_cents to the committed disbursement ledger
        # (the "actually disbursed" evidence) instead of trusting the payload's
        # own claim. Fail closed if the employee has no disbursement record —
        # a paycheck with no bundled evidence of what was actually paid is not
        # verifiable, not silently accepted.
        disbursed = disbursements.get(emp)
        if disbursed is None:
            return (
                f"paycheck[{idx}] {emp}: {_DISBURSEMENT_UNBOUND_CODE}: no matching "
                "disbursement record in data/disbursements.csv — issued_net_cents "
                "has no committed evidence to bind to"
            )
        if disbursed != issued_net:
            return (
                f"paycheck[{idx}] {emp}: {_DISBURSEMENT_UNBOUND_CODE}: "
                f"issued_net_cents mismatch — payload/paychecks.json claims "
                f"{issued_net}, data/disbursements.csv commits {disbursed}"
            )
