...
_REFINEMENT = "(and (= gross (+ base uplift retro)) (= net (- (- gross tax) pension)))"
_LOGIC = "QF_LIA"
...
def _compute_pay(
    home, acting_cls, acting_days, retro_old, retro_new, retro_periods
) -> dict:
...
def _events_to_csv_bytes() -> bytes:
    ...
    for row in _EVENTS:
        ...
        lines.append(
            ",".join(
                str(v)
                for v in (emp, home, acting, adays, rold, rnew, rper, status, wreason)
            )
        )
    return "\n".join(lines).encode("utf-8")


_DISBURSEMENT_COLUMNS = ["employee_id", "issued_net_cents"]


def _disbursements_to_csv_bytes(disbursements: list[tuple[str, int]]) -> bytes:
    """FIX-E (claim-set coverage sweep): the disbursement ledger is a committed
    bundle INPUT — the same producer-side source `issued_net_cents` was
    previously only *asserted* in payload/paychecks.json. It is written here
    from the exact same source (`correct_net + clawback_offset_cents`) the
    ledger's `issued_net_cents` is fabricated from, so the checker has a
    second, independently-hashed record to bind the payload claim to.

    HONESTY RAIL: this is producer-committed evidence — it kills a
    payload<->evidence inconsistency (moving issued_net_cents and
    clawback_cents together in payload/paychecks.json without also moving
    this file). It does NOT establish the ledger's own wall-clock truth
    (that a colluding producer forges both files consistently); a bank-side
    disbursement attestation would be the stronger leg. See README §Scope
    honesty."""
    lines = [",".join(_DISBURSEMENT_COLUMNS)]
    for emp, issued_net_cents in disbursements:
        lines.append(f"{emp},{issued_net_cents}")
    return "\n".join(lines).encode("utf-8")
...
def build(out_dir: Path) -> None:
    ...
    disbursements: list[
        tuple[str, int]
    ] = []  # FIX-E: committed disbursement ledger rows
    ...
    for emp, home, acting, adays, rold, rnew, rper, status, wreason, offset in _EVENTS:
        ...
        # FIX-E: the SAME issued_net that fabricates payload/paychecks.json's
        # issued_net_cents is also committed here, as a separate SHA-pinned
        # input file — "actually disbursed" becomes evidence the verifier
        # reads, not a free payload assertion (see _disbursements_to_csv_bytes).
        disbursements.append((emp, issued_net))

    payload_bytes = json.dumps(ledger, indent=2).encode("utf-8")
    disbursements_bytes = _disbursements_to_csv_bytes(disbursements)
    ...
    log_bytes = (
        "\n".join(
            json.dumps(e, sort_keys=True, separators=(",", ":")) for e in events_log
        )
        + "\n"
    ).encode("utf-8")
    ...
    content = BundleContent(
        ...
        files={
            ...
            "data/disbursements.csv": disbursements_bytes,
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir", required=True, type=Path, help="Destination directory"
    )
