...
All three types bind to one `primitive_id` under an **identical** `rational_band`
comparator (epsilon=1e-9 — the recompute is the exact rational per metric;
epsilon covers only the producer's unrounded float division) — which is
exactly what the substrate's monotone-strictness invariant requires (a
primitive bound by ≥2 types must carry an identical comparator). The
...
1. **Per-metric mutation** — inflate one claimed metric → `RE_DERIVATION_MISMATCH`.
...
4. **Weaker-spec substitution** — producer ships a same-kind (`rational_band`)
   ε = 1e30 spec → `AnchorViolation`.
...
primitive, three `rational_band` outputs, the coverage invariant + cardinality
