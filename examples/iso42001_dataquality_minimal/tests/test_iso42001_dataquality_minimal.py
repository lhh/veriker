...
ISO/IEC 42001 A.7 data-quality re-derivation (MULTI-OUTPUT). Surfaces:
  ...
  2. Per-metric mutation: inflate the claimed data_completeness_pct (+2.0),
     re-align its SHA -> RE_DERIVATION_MISMATCH. One doctored metric of three
...
def test_omit_output_entry_fails_coverage(tmp_path):
    ...
    m = json.loads(mp.read_bytes())
    m["outputs"] = [
        o for o in m["outputs"] if o["output_id"] != "data_positive_rate_pct"
    ]
...
def test_weak_spec_substitution_fails(tmp_path):
    ...
    # Same kind (rational_band) as the committed spec — a weak-same-kind
    # substitution, not a kind swap.
    weak = json.dumps(
        {
...
# ---------------------------------------------------------------------------
# 6. rational_band edge (positive_rate poster child) — near-edge claims land
#    on the exact side
# ---------------------------------------------------------------------------


def test_rational_band_edge_positive_rate(tmp_path):
    """data_positive_rate_pct's honest value on the frozen fixture is the
    non-terminating R = 1000/19 (10 positives of 19 labelled records — see
    inputs/dataset.json). The migrated comparator decides |R - c| <= 1e-9
    EXACTLY on Fractions. Pin one claim just inside the band (PASS) and one
    just outside (RE_DERIVATION_MISMATCH), asserting each side's expected
    placement with the same exact arithmetic the comparator uses.
    """
    R = Fraction(1000, 19)
    eps = Fraction(1, 10**9)
    inside_claim = 52.631578948  # |Fraction(inside) - R| ~ 6.3e-10 < 1e-9
    outside_claim = 52.631578949  # |Fraction(outside) - R| ~ 1.6e-9 > 1e-9
    ...
    for claim, expect_ok in ((inside_claim, True), (outside_claim, False)):
        ...
        (bundle_dir / rel).write_bytes(claim_bytes)
        mp = bundle_dir / "manifest.json"
        m = json.loads(mp.read_bytes())
        m["files"][rel] = hashlib.sha256(claim_bytes).hexdigest()
        mp.write_bytes(json.dumps(m, indent=2).encode("utf-8"))
