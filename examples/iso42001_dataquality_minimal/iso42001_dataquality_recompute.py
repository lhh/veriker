...
primitive_id under the SAME rational_band comparator — which satisfies the
substrate's monotone-strictness invariant (a primitive bound by >=2 types must
carry an identical comparator).

Comparator: rational_band {"epsilon": 1e-9} (all three types, identical params
— §4a.3 stays satisfied). The recompute returns the EXACT rational for each
metric as {"kind": "rational", "num", "den"} via a SEPARATE
`_METRIC_FNS_EXACT` dispatch table (int-only arithmetic; never calls the
float `compute_*` helpers below). The producer's claims stay the UNROUNDED
floats from those `compute_*` helpers (no round() anywhere in the builder);
epsilon=1e-9 covers only the producer's float division(s) (percentages are
<= 100, so binary64 representation error is <= ~1.5e-14 — 1e-9 is generous
headroom), nothing on the recompute side, which is exact. Note the
`data_positive_rate_pct` denominator is the labelled-record count, NOT the
full record count — a different denominator from the other two metrics.
...
def compute_completeness_pct(records: list) -> float:
    ...
    complete = sum(
        1 for r in records if all(r.get(f) is not None for f in _REQUIRED_FIELDS)
    )
...
# ---------------------------------------------------------------------------
# Exact-rational computations (NEW, recompute-only — never called by the
# builder or by the float compute_* fns above; tautology-free by
# construction). Same aggregation rules, int-only arithmetic throughout.
# ---------------------------------------------------------------------------


def _completeness_exact(records: list) -> Fraction:
    complete = sum(
        1 for r in records if all(r.get(f) is not None for f in _REQUIRED_FIELDS)
    )
    return Fraction(100 * complete, len(records))


def _duplicate_rate_exact(records: list) -> Fraction:
    tuples = [tuple(r.get(f) for f in _REQUIRED_FIELDS) for r in records]
    n_distinct = len(set(tuples))
    return Fraction(100 * (len(records) - n_distinct), len(records))


def _positive_rate_exact(records: list) -> Fraction:
    labelled = [r for r in records if r.get("label") is not None]
    if not labelled:
        raise ValueError("no labelled records — positive rate undefined")
    positives = sum(1 for r in labelled if int(r["label"]) == 1)
    return Fraction(100 * positives, len(labelled))


# type -> exact-rational fn (recompute() dispatches HERE, never into
# _METRIC_FNS above).
_METRIC_FNS_EXACT = {
    _COMPLETENESS: _completeness_exact,
    _DUPLICATE: _duplicate_rate_exact,
    _POSITIVE: _positive_rate_exact,
}
...
class Iso42001DataQualityRecompute:
    ...
    def recompute(self, inputs, pack_section: dict):
        ...
        Returns the EXACT rational {"kind": "rational", "num", "den"} via
        `_METRIC_FNS_EXACT` — never the float `_METRIC_FNS` table the
        builder uses for the claim.
        """
        from audit_bundle.plugin import RecomputedValue  # noqa: PLC0415

        type_key = pack_section.get("type")
        fn = _METRIC_FNS_EXACT.get(type_key)
        if fn is None:
            raise ValueError(
        ...
        return RecomputedValue(
            ...
            detail=(
                f"re-derived {type_key} over {len(records)} records; "
                f"exact R~{float(R):.6f}"
            ),
        )
