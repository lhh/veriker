...
text (corpus/rooted_records.json, captured by _root_corpus.py with provenance):
  ...
  2. Hide-the-fabrication: flip outputs/gate_verdict.json decision to AUTO_APPROVE
     ...
     fires is the re-derivation -> RE_DERIVATION_MISMATCH.
  ...
  3. Hide-one-misquote (granularity): flip ONLY the MISQUOTE assertion's claimed
     ...
     disagrees per-citation -> RE_DERIVATION_MISMATCH.
  ...
  8. signed_at re-dating: build honestly, then back-date
     attestation/gate_attestation.json.signed_at (leave decision, citations, and
     evidence untouched) and reseal ONLY the attestation file's manifest SHA, so
     file-integrity and re-derivation both pass. signed_at is now part of the
     canonical signed payload, so the stale signature no longer verifies against
     the mutated timestamp -> CASELAW_GATE_ATTESTATION_INVALID, isolated (mirrors
     test 6/7's isolation shape for a distinct attack vector on the same surface).
...
# RE_DERIVATION_MISMATCH directly. Harvest both (belt-and-suspenders idiom).
_DETAIL_CODES = (
...
# ---------------------------------------------------------------------------
# Test 2 — Hide the fabrication: claim AUTO_APPROVE -> RE_DERIVATION_MISMATCH
# ---------------------------------------------------------------------------
...
def test_hidden_fabrication_fails_rederivation(tmp_path):
    ...
    lie -> RE_DERIVATION_MISMATCH, and NOT a file-SHA failure."""
    bundle_dir = tmp_path / "bundle"
    _build(bundle_dir)
    ...
    assert "RE_DERIVATION_MISMATCH" in rc, f"Expected RE_DERIVATION_MISMATCH; got {rc!r}"
...
# ---------------------------------------------------------------------------
# Test 3 — Granularity: hide ONE misquote -> RE_DERIVATION_MISMATCH
# ---------------------------------------------------------------------------
...
def test_hidden_single_misquote_fails_rederivation(tmp_path):
    ...
    SHA. The recompute disagrees on that one record -> RE_DERIVATION_MISMATCH.
...
def test_attestation_signature_tamper_isolated(tmp_path):
    ...
    assert "RE_DERIVATION_MISMATCH" not in rc, (
...
# ---------------------------------------------------------------------------
# Test 8 — signed_at re-dating: back-date the attestation, reseal its own SHA
# ---------------------------------------------------------------------------


def test_signed_at_tamper_fails_attestation(tmp_path):
    """Strongest-producer scenario for the FIX-C finding: build honestly, then
    back-date attestation/gate_attestation.json.signed_at ONLY (verdict and
    evidence files untouched), resealing manifest.files for the rewritten
    attestation file so file-integrity passes. Before the fix, signed_at was not
    part of the signed payload, so a re-dated attestation verified fine (the
    finding this test closes). signed_at is now bound into
    canonical_gate_verdict_payload, so the stale signature over the mutated
    timestamp fails to verify -> CASELAW_GATE_ATTESTATION_INVALID, isolated (the
    re-derivation and file-integrity both still pass -- only the signed receipt
    fires, same isolation shape as tests 6/7 for a distinct attack vector)."""
    bundle_dir = tmp_path / "bundle"
    _build(bundle_dir)

    rel = "attestation/gate_attestation.json"
    doc = json.loads((bundle_dir / rel).read_bytes())
    honest_signed_at = doc["signed_at"]
    doc["signed_at"] = "2020-01-01T00:00:00Z"  # back-dated
    assert doc["signed_at"] != honest_signed_at, (
        "tamper precondition: signed_at changed"
    )
    new_bytes = (
        json.dumps(doc, indent=2, sort_keys=True, ensure_ascii=False) + "\n"
    ).encode()
    ...
    result = _verifier().verify(bundle_dir)
    assert not result.ok, "Expected FAIL after back-dating signed_at"
    rc = _reason_codes(result)
    assert "CASELAW_GATE_ATTESTATION_INVALID" in rc, (
        f"Expected attestation INVALID after signed_at tamper; got {rc!r}"
    )
    # Isolation: re-derivation and file-integrity must still pass (only the signed
    # receipt fires) -- proving signed_at is now genuinely part of the binding.
    assert "RE_DERIVATION_MISMATCH" not in rc, (
        f"verdict untouched, re-derivation should pass; {rc!r}"
    )
    assert "bad_file_sha" not in rc, (
...
def test_honest_signed_at_still_passes(tmp_path):
    """Sanity complement to test 8: the honest bundle's untouched signed_at
    (now part of the signed payload) still verifies -- the fix does not break
    the honest path."""
    bundle_dir = tmp_path / "bundle"
    _build(bundle_dir)
    result = _verifier().verify(bundle_dir)
    assert result.ok, [(f.check_name, f.reason_code, f.detail) for f in result.failures]
    doc = json.loads(
        (bundle_dir / "attestation" / "gate_attestation.json").read_bytes()
    )
    assert "signed_at" in doc, "attestation must still carry signed_at"


def test_verifier_path_is_independent_of_the_producer_helper():
    """MUTANT CONTROL for the producer/verifier tautology (fixed 2026-08-29).

    Until this fix the producer computed its claimed verdict with
    `compute_gate_verdict` and the registered primitive re-derived with THE
    SAME function, so the check was f(x) == f(x): an error in the gate RULE was
    baked into claim and re-derivation alike. A tampered VALUE was still
    caught; method independence was not demonstrated.

    Measured then: corrupting the producer's verdict so it echoes NORMALIZED
    reporter cites -- a mutation that survives both of `_build_bundle.py`'s own
    fixture self-checks, since the statuses and the ROUTE_TO_HUMAN decision are
    unchanged -- verified CLEAN at exit 0. With the verifier on its own
    implementation the same bundle is RE_DERIVATION_MISMATCH.

    This test breaks the producer helper and asserts the verifier path is
    unaffected. It could not fail while the two were one function.
    """
    corpus = [
        {"reporter_cite": "1 X. 1", "rooted_text": "the holding says alpha beta"},
    ]
    assertions = [
        {"id": "C-01", "reporter_cite": "1 X. 1", "quoted_holding": "alpha beta"},
    ]
    truth = _prim_mod._recompute_gate_verdict_independent(corpus, assertions)
    assert truth["decision"] == _prim_mod.AUTO_APPROVE, truth
    assert [c["status"] for c in truth["citations"]] == [_prim_mod.ROOTED], truth

    original = _prim_mod.compute_gate_verdict
    try:
        _prim_mod.compute_gate_verdict = lambda *a, **k: {
            "decision": _prim_mod.ROUTE_TO_HUMAN,
            "citations": [],
        }
        again = _prim_mod._recompute_gate_verdict_independent(corpus, assertions)
    finally:
        _prim_mod.compute_gate_verdict = original
    assert again == truth, (
        "the verifier path changed when the PRODUCER helper was replaced -- "
        "the two are not independent"
    )
    assert (
        _prim_mod._recompute_gate_verdict_independent is not _prim_mod.compute_gate_verdict
    )
