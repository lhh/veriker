...
# is the PRODUCER's definition; the verifier re-derives with a SEPARATE
# implementation, so a rule error cannot hide in a shared function.
...
def build(out_dir: Path) -> None:
    ...
    # Bind the signature to the verdict, the evidence SHAs, AND the attestation's
    # own signed_at, so swapping the verdict, an evidence file, or back/forward-
    # dating signed_at all invalidate it. The verifier holds only the public key
    # (committed below) and cannot re-sign -> forge-resistant. (signed_at binding
    # proves it cannot be altered post-signing, not that it was truthful at mint
    # -- see caselaw_verdict_binding.canonical_gate_verdict_payload.)
    ...
    print(
        f"  gate verdict     : {verdict['decision']} ({n_rooted}/{len(assertions)} ROOTED)"
    )
    ...
    print(
        f"  attestation      : ed25519 over verdict+evidence  sig={signature_hex[:16]}..."
    )
    print(
        f"  verify pubkey    : {signing_key.public_key().to_hex()[:16]}...  (synthetic in-bundle anchor)"
    )
