...
bound to the decision, every per-citation status (in order), the SHA-256 of
the corpus and assertions, **and the attestation's own `signed_at` timestamp**
— with an Ed25519 private key; `verify.py`'s `CaselawGateAttestationCheck`
verifies it against the **public-only** key committed at
`attestation/gate_verifier_pubkey.hex`. This adds a property the re-derivation
...
signature (test 6), a swapped trust anchor (test 7), or a back-dated `signed_at`
(test 8) fires `CASELAW_GATE_ATTESTATION_INVALID` *in isolation* —
re-derivation and file-integrity still pass — proving the signed receipt is an
independent surface.
...
>
> **Honest scope of `signed_at`:** binding it into the signed payload proves it
> **cannot be altered after signing** — a re-dated attestation fails the same
> way any other field-mutation does. It does **not** prove the timestamp was
> **truthful when the gate authority signed** — a gate authority that signs a
> false `signed_at` on day one is a trust-root concern, the same category as
> corpus genuineness below, not something a signature over a self-reported
> field can close.
...
python -m pytest examples/caselaw_citation_gate/tests/ -q                          # -> 9 passed
```
...
- `_build_bundle.py` / `verify.py` / `tests/` — build, verify, and the 9 attack/tamper tests
  (7 tamper vectors + 1 honest-signed_at sanity check + the happy path).
