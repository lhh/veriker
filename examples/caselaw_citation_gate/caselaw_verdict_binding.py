...
committed evidence (a tampered claimed verdict -> RE_DERIVATION_MISMATCH). The
...
the SHA-256 of the rooted corpus and the assertions, AND the attestation's own
`signed_at` timestamp. Binding the evidence SHAs means the signed statement is
precisely: "the gate authority attests this verdict was computed over THIS
rooted corpus and THESE assertions." Swapping either evidence file (even with a
re-aligned manifest SHA) invalidates the signature. Binding `signed_at` means a
value changed AFTER signing is caught, the same way any other field-mutation
is caught -- see the HONEST FRAMING note on `canonical_gate_verdict_payload`
for what this does and does not prove (it does not prove the timestamp was
truthful when the gate authority signed).
...
def canonical_gate_verdict_payload(
    *,
    ...
    evidence the verdict was computed over. `signed_at` is the attestation's own
    claimed signing timestamp (`attestation/gate_attestation.json.signed_at`),
    bound so it cannot be back- or forward-dated post-signing without
    invalidating the signature (see HONEST FRAMING below).

    Any mutation to the decision, any per-citation status, the citation order,
    either evidence file, or `signed_at` invalidates the signature. The verifier
    holds only the public key and therefore cannot re-sign -- that asymmetry is
    the forge-resistance property.

    HONEST FRAMING: binding `signed_at` into the signed payload proves it cannot
    be ALTERED after signing (tamper-evidence), not that it was truthful AT mint
    -- the gate authority could still sign a verdict with a false timestamp on
    day one. That is a trust-root concern (same category as corpus genuineness),
    not something a signature over a self-reported field can close.
    """
    body = {
