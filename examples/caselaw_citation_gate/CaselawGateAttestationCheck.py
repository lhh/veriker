...
Any mutation to the decision, a per-citation status, the citation order, either
evidence file (even with a re-aligned manifest SHA), or the attestation's own
`signed_at` timestamp fires CASELAW_GATE_ATTESTATION_INVALID.

HONEST FRAMING (signed_at): binding `signed_at` into the signed payload proves
it cannot be ALTERED after signing -- a back- or forward-dated `signed_at`
written in after the fact no longer verifies. It does NOT prove the timestamp
was truthful AT mint; a gate authority signing its own false timestamp on day
one is a trust-root concern this signature cannot close (same category as
corpus genuineness -- see the HONEST FRAMING note on
caselaw_verdict_binding.canonical_gate_verdict_payload and the README).
...
class CaselawGateAttestationCheck:
    ...
    def check(self, bundle_dir: Path, manifest) -> PluginResult:
        ...
        # Reconstruct the canonical signed bytes from the verdict's own fields,
        # the evidence SHAs computed from the committed files, and the
        # attestation's own claimed signed_at. Mutating the verdict, an evidence
        # file, or signed_at changes these bytes -> signature won't verify.
        ...
        if not verify_payload(signed_bytes, signature_hex, verify_key):
            return PluginResult(
                ...
                (
                    ...
                    "statuses / order), an evidence file (rooted corpus / "
                    "assertions), or the attestation's signed_at was tampered, or "
                    "it was signed by a different key. The verifier holds only the "
                    "public key and cannot re-sign (forge-resistant)."
                ),
        ...
        return PluginResult(
            ...
            (
                ...
                f"bound to corpus+assertions SHAs and signed_at="
                f"{attestation.get('signed_at')!r}"
            ),
            (str(verdict_path), str(attest_path), str(pubkey_path)),
        )
