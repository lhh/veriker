...
python examples/legal_contract_minimal/verify.py --bundle-dir examples/legal_contract_minimal/    # PASS
...
| `LegalContractReDerivationCheck.py` | TypedCheck plugin wrapping the subprocess call to the re-derivation pack (pilot-local; the top-level verify CLI route uses the substrate's `ReDerivationInvocationCheck` against the same pack). |
...
`ReDerivationInvocationCheck` when invoked via the top-level verify CLI).
