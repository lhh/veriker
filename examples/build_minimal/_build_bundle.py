...
def build(out_dir: Path) -> None:
    ...
    content = BundleContent(
        ...
        # Claim-field coverage: the only claim is the artifact itself — a whole-
        # file (opaque) declaration; BuildReDerivationCheck reports it covered
        # only after the pack byte-compares the re-executed recipe's output.
        extra_manifest_fields={
            "claimset": {"opaque_claim_files": {"artifact": "payload/artifacts/combined.txt.gz"}}
        },
    )
