"""verify.py — build_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  BuildReDerivationCheck    §C6 — deterministic build/recipe re-derivation (local pack)

Usage:
    python examples/build_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/build_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# BuildReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from BuildReDerivationCheck import BuildReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="build-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
