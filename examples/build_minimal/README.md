...
`RE_DERIVATION_MISMATCH` because the gzip output bytes diverge from the
bundled artifact.

## Claim-field coverage (claimset)

The bundle declares its one claim — the artifact itself — as a whole-file
(opaque) claim: `manifest.claimset.opaque_claim_files = {"artifact":
"payload/artifacts/combined.txt.gz"}`. `BuildReDerivationCheck` reports that
key covered ONLY when the pack exited 0 and printed the `[COMPARED]` line
naming that file (the pack re-executes the recipe and byte-compares the
result); the verdict then carries `claimset: … n_universe=1 n_covered=1 …
n_opaque=1`. If the pack cannot run (`NO_PACK` / `NO_PAYLOAD`), nothing is
reported and the verdict is could-not-conclude (exit 2) naming `'artifact'` —
never a silent PASS. `tests/test_build_minimal.py` runs the per-field tamper
battery: a byte flipped at the first, middle and last position, a truncation
and an extension of the artifact must each flip the verdict through the
pack's own `[BUILD_REDER_FAIL]` reason. The spec-pinned overlay lane
(`spec_pinned_check.py`) drops the declaration because its plugin set does
not wire this byte comparison.
