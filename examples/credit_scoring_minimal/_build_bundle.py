...
def build(out_dir: Path) -> None:
    ...
    content = BundleContent(
        ...
        extra_manifest_fields={
            ...
            # payload/credit_decisions.json must be read-and-compared by a
            # wired check or excused with a committed reason — a field
            # nothing reads fails the verdict closed instead of riding a
            # green PASS. Seven fields are bound by credit_scoring_re_derivation
            # (per-decision applicant_id / pd / tier / decision / apr_pct
            # against the replayed scorecard + threshold table + applicants/,
            # and scorecard_model / scorecard_version against the bundled
            # scorecard's own identity). The one residual: `schema` is a
            # schema label carrying no claim — the pack reads a fixed key set
            # regardless of the label; it is not a value re-derived from
            # anything, and pinning it to a constant would be an assertion.
            "claimset": {
                "claim_files": {"credit_decisions": "payload/credit_decisions.json"},
                "residuals": {"credit_decisions:schema": "INSPECTION_ONLY"},
            },
        },
    )
