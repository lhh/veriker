...
What it demonstrates:
  ...
  - Honest bundle -> PASS; tampered claimed list or tampered input (an applicant
    attribute mutated so a verdict flips) -> FAIL (RE_DERIVATION_MISMATCH); no
...
def build_spec_pinned(
    ...
    *,
    ...
    # primitive re-derives the categorical verdict list on the outputs/ claim,
    # and the legacy re-derivation plugin (the check that reads and compares
    # payload/credit_decisions.json's fields) is deliberately not wired here.
    ...
    # tests/test_credit_scoring_minimal.py.
