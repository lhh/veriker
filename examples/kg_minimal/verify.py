"""verify.py — kg_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  KgReDerivationCheck             §C6 — KG path-query re-derivation (local pack)
  DispatchRecordWellformedCheck   §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"GRAPH_QUERY", "COMPUTE"}) admits the new
     domain-specific GRAPH_QUERY kind introduced by this pilot)

Usage:
    python examples/kg_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/kg_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# KgReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from KgReDerivationCheck import KgReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="KG-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
