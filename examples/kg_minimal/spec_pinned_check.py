"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for kg_minimal.

Per-dir migration of the kg_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + KgReDerivationCheck.py + kg_re_derivation.py)
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/kg.spec.json).
  - The verifier re-derives the representative output (the answer_nodes collection —
    the set of node ids reachable by the committed path-query) IN-PROCESS via the
    registered primitive (BFS over kg/triples.jsonl), and compares with the generic
    `set` comparator (order-independent). No subprocess, no bundle-supplied code.
...
Usage:
    python examples/kg_minimal/spec_pinned_check.py
...
# Legacy builder (lays down kg/payload/manifest) + the shared canonical compute
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("kg__legacy_build_bundle", _HERE / "_build_bundle.py")
...
from kg_recompute import compute_answer_nodes  # noqa: E402

_OUTPUT_ID = "kg_answer_nodes"
_TYPE_KEY = "kg_answer_nodes"
_SPEC_SRC = _HERE / "spec_pinned" / "kg.spec.json"
...
def _honest_answer_nodes(out_dir: Path) -> list[str]:
    """Auditor-side canonical recompute of the honest answer_nodes from the built
    bundle's committed triple set + payload query (BFS closure)."""
    triples: list[dict] = []
    for line in (out_dir / "kg" / "triples.jsonl").read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if not line:
            continue
        triples.append(json.loads(line))
    qr = json.loads((out_dir / "payload" / "query_result.json").read_bytes())
    return compute_answer_nodes(qr["query"], triples)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned kg_minimal bundle in out_dir. Reuses the legacy builder
    for kg/payload, then overlays the β shape: an auditor spec under spec/, a
    ...
    claimed = _honest_answer_nodes(out_dir)
...
def make_verifier(anchor=None):
    ...
    from kg_recompute import KgRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the kg_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  kg_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  kg_minimal  (spec-pinned dispatch)", file=sys.stderr)
