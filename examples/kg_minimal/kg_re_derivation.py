...
Exit 0 on full match; exit 1 on first mismatch with [RE_DERIVATION_MISMATCH] on stderr.
...
def _load_triples(bundle_dir: Path) -> list[dict] | None:
    ...
    for line in triples_path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            print(f"[RE_DERIVATION_MISMATCH] kg/triples.jsonl: JSON parse error: {exc}", file=sys.stderr)
            return None
    return triples
...
def _load_query_result(bundle_dir: Path) -> dict | None:
    ...
    try:
        ...
        print(f"[RE_DERIVATION_MISMATCH] payload/query_result.json: JSON parse error: {exc}", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    for i, t in enumerate(triples):
        try:
            ...
            print(
                f"[RE_DERIVATION_MISMATCH] kg/triples.jsonl line {i}: malformed triple: {exc}",
                file=sys.stderr,
            )
            return 1
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] payload/query_result.json: malformed structure: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    for i, edge in enumerate(path_edges):
        try:
            ...
            print(
                f"[RE_DERIVATION_MISMATCH] path_edges[{i}]: malformed edge: {exc}",
                file=sys.stderr,
            )
            return 1
        if key not in triple_set:
            print(
                f"[RE_DERIVATION_MISMATCH] path_edges[{i}]: edge {key!r} not found in bundled triples",
                file=sys.stderr,
            )
            return 1
    ...
    if answer_set != bfs_result:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] answer_nodes does not match BFS closure\n"
    ...
    if answer_set != terminal_nodes:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] answer_nodes inconsistent with path_edges terminal nodes\n"
