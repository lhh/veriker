"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for fintech_audit.

Per-dir migration of the fintech_audit_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + PolicyRuleReDerivationCheck.py +
policy_re_derivation.py) and its verify.py / tests are untouched, and no committed
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/fintech_audit.spec.json).
  - The verifier re-derives the representative output (the per-transaction policy
    verdict list) IN-PROCESS via the registered primitive, and compares with the
...
The legacy manifest carries fragment_anchors (the fragment-anchor surface from the
original build). The overlay leaves those intact and only ADDS outputs[] /
spec_files / files[outputs/…] and OVERRIDES typed_checks: the spec-pinned verifier
runs FileIntegrityManySmall + step-5 dispatch only, so typed_checks must name
exactly the registered plugin set -> ["file_integrity_many_small"].

Usage:
    python examples/fintech_audit_minimal/spec_pinned_check.py
...
# Legacy builder (lays down transactions/policies/payload/manifest) + the shared
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "fintech_audit__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from fintech_audit_recompute import (  # noqa: E402
    _load_ordered,
...
_SPEC_SRC = _HERE / "spec_pinned" / "fintech_audit.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned fintech_audit bundle in out_dir. Reuses the legacy
    builder for transactions/policies/payload, then overlays the β shape: an
    ...
    transactions = _load_ordered(out_dir / "transactions")
    policies = _load_ordered(out_dir / "policies")
    claimed = compute_policy_verdicts(transactions, policies)
    ...
    # Leave the legacy fields (fragment_anchors, …) intact; only ADD the
    # spec-pinned surface and OVERRIDE typed_checks.
...
def make_verifier(anchor=None):
    ...
    from fintech_audit_recompute import FintechAuditRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the fintech_audit exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  fintech_audit  (spec-pinned dispatch)")
            return 0
        print("FAIL  fintech_audit  (spec-pinned dispatch)", file=sys.stderr)
