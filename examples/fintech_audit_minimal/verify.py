"""verify.py — fintech_audit_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  PolicyRuleReDerivationCheck   §C6 — policy-rule verdict re-derivation (local pack)

Usage:
    python examples/fintech_audit_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/fintech_audit_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# PolicyRuleReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from PolicyRuleReDerivationCheck import PolicyRuleReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fintech-audit-minimal bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
