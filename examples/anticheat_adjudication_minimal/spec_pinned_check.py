"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for anticheat_adjudication.

Per-dir migration of the anticheat_adjudication_minimal pilot onto the disclosed
spec-pinned, auditor-anchored, in-process recompute-then-compare method (S0). This is
ADDITIVE: the legacy bundle (_build_bundle.py + AnticheatReDerivationCheck.py +
anticheat_re_derivation.py) and its verify.py / test are untouched, and no committed
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/anticheat_adjudication.spec.json).
  - The verifier re-derives the representative output (the ordered per-case
    {model_recommendation, matched_rule_id} verdict list) IN-PROCESS via the
    registered primitive, and compares with the generic `exact` comparator. No
    subprocess, no bundle-supplied code. The HMAC adjudicator-attestation half of
    the pilot is left to the legacy pack — it is trust mechanics, not a re-derived
    value.
...
decision_provenance_log (the dispatch-record / fragment-anchor / decision-provenance
surface from the original build). The
overlay leaves those intact and only ADDS outputs[]/spec_files/files[outputs/…] and
OVERRIDES typed_checks: the spec-pinned verifier runs FileIntegrityManySmall +
step-5 dispatch only, so typed_checks must name exactly the registered plugin set ->
["file_integrity_many_small"].

Usage:
    python examples/anticheat_adjudication_minimal/spec_pinned_check.py
...
# Legacy builder (lays down evidence/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "anticheat_adjudication_minimal__legacy_build_bundle",
...
from anticheat_adjudication_recompute import (  # noqa: E402
    _load_cases,
    _load_policy,
...
_SPEC_SRC = _HERE / "spec_pinned" / "anticheat_adjudication.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned anticheat_adjudication bundle in out_dir. Reuses the
    legacy builder for evidence/payload, then overlays the β shape: an auditor spec
    ...
    cases = _load_cases(out_dir)
    policy = _load_policy(out_dir)
    claimed = compute_verdict_list(cases, policy)
...
def make_verifier(anchor=None):
    ...
    from anticheat_adjudication_recompute import (  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the anticheat_adjudication exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  anticheat_adjudication  (spec-pinned dispatch)")
            return 0
        print("FAIL  anticheat_adjudication  (spec-pinned dispatch)", file=sys.stderr)
