"""verify.py — anticheat_adjudication_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  AnticheatReDerivationCheck      §C6 — ban-adjudication decision re-derivation +
                                        adjudicator attestation HMAC re-verification
  DispatchRecordWellformedCheck   §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"DETECTION_EVAL", "ADJUDICATOR_ATTEST", "COMPUTE"})
     admits the domain-specific op kinds introduced by this pilot)

Usage:
    python examples/anticheat_adjudication_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/anticheat_adjudication_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# AnticheatReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from AnticheatReDerivationCheck import AnticheatReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Anti-cheat ban-adjudication-minimal audit bundle verifier "
