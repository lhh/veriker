...
Checks performed (see anticheat_re_derivation.py for the full invariant list):
  1. Detection-policy re-derivation: re-evaluates the threshold rule set from
     ...
     rule IDs match payload/ban_decisions.json, INCLUDING its final_verdict
     field (must equal the re-derived model_recommendation — no adjudicator
     override exists in this pilot's honest construction).
  2. Cross-checks payload/adjudication_provenance.jsonl's derived
     model_recommendation copy against the payload/ban_decisions.json value.
  3. Adjudicator attestation HMAC re-verification: for every row in
     ...
     adjudicator_type) under the bundle's committed attestation key and
     asserts it matches the stored attestation_hmac.

Reason codes (top-level; the subprocess pack emits dedicated
[ANTICHEAT_*] tags as detail-text prefixes for each sub-invariant, since a
wrapping PluginResult surfaces reason_code + detail but this subprocess wrap
only distinguishes the two families below from stderr content):
  RE_DERIVED                                — all invariants passed
  RE_DERIVATION_MISMATCH                    — decision-policy, final_verdict,
                                              or model_recommendation-drift
                                              mismatch (see detail for the
                                              specific [ANTICHEAT_*] tag)
  ANTICHEAT_ADJUDICATOR_ATTESTATION_INVALID — HMAC verification failure

§C6 (domain-agnostic re-derivation).
