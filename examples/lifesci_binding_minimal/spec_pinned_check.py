"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for lifesci_binding.

Per-dir migration of the lifesci_binding pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + BindingAffinityReDerivationCheck.py +
binding_affinity_re_derivation.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/lifesci_binding.spec.json).
  - The verifier re-derives the representative output (binding affinity_pred)
    IN-PROCESS via the registered primitive, and compares with the generic
    rational_band comparator. No subprocess, no bundle-supplied code.
...
Usage:
    python examples/lifesci_binding_minimal/spec_pinned_check.py
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("lifesci_binding__legacy_build_bundle", _HERE / "_build_bundle.py")
    _m = _ilu.module_from_spec(_s)
    _s.loader.exec_module(_m)
    return _m.build

_build_legacy_bundle = _load_legacy_build()
from lifesci_binding_recompute import compute_affinity_pred  # noqa: E402

_OUTPUT_ID = "lifesci_binding_affinity"
_TYPE_KEY = "lifesci_binding_affinity"
_SPEC_SRC = _HERE / "spec_pinned" / "lifesci_binding.spec.json"
...
def _honest_claimed(out_dir: Path) -> float:
    """The auditor's own canonical recompute over the bundle's committed bytes."""
    compound = json.loads(
        (out_dir / "inputs" / "compound_descriptor.json").read_bytes()
    )
    target = json.loads((out_dir / "inputs" / "target_descriptor.json").read_bytes())
    weights = json.loads((out_dir / "payload" / "scoring_weights.json").read_bytes())
    return compute_affinity_pred(
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned lifesci_binding bundle in out_dir. Reuses the legacy
    ...
    claimed = _honest_claimed(out_dir)
...
def make_verifier(anchor=None):
    ...
    from lifesci_binding_recompute import LifesciBindingRecompute  # noqa: PLC0415

    register_primitive(LifesciBindingRecompute())
    return BundleVerifier(plugins=[FileIntegrityManySmall()], spec_anchor=anchor)


def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the lifesci_binding exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  lifesci_binding  (spec-pinned dispatch)")
            return 0
        print("FAIL  lifesci_binding  (spec-pinned dispatch)", file=sys.stderr)
