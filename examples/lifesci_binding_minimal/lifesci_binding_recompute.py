...
RECOMPUTE side (rational_band, this migration): the same dot-products and
bias, computed in EXACT rational arithmetic (Fraction) and returned
UNROUNDED as {"kind": "rational", "num", "den"}. w_compound/w_target are
committed as ints by design (see _build_bundle.py's _SCORING_WEIGHTS
comment) and the feature counts are int bucket counts, so both dot products
are exact integers; bias=2.5 is exactly representable in binary64
(Fraction(bias) is therefore exact). The exact value is always
integer + 2.5 — on the 0.5 grid, at most 1dp — so the producer's terminal
round(raw, 6) is a NO-OP on the exact value; epsilon=1e-6 covers only
producer-side float representation noise (effectively zero here).
...
output type "lifesci_binding_affinity" to this primitive_id and to a rational_band
...
def compute_affinity_pred(
    ...
    if len(w_target) != n_buckets:
        raise ValueError(f"w_target has {len(w_target)} elements; expected {n_buckets}")
...
def compute_affinity_pred_exact(
    smiles_string: str,
    sequence: str,
    w_compound: list,
    w_target: list,
    ...
    """EXACT-rational sibling of compute_affinity_pred(), for the rational_band
    recompute path only — NEVER used to produce the claim (that stays
    compute_affinity_pred(), byte-identical, above).

    w_compound/w_target are committed as ints by design (see
    _build_bundle.py's _SCORING_WEIGHTS comment) and the feature counts are
    int bucket counts, so both dot products are exact integers; bias is
    exactly representable in binary64 (Fraction(bias) is exact per Python's
    float-to-Fraction conversion — it reads the float's own exact binary
    value, not a decimal approximation). The result is returned UNROUNDED —
    no terminal round() here, unlike compute_affinity_pred().
    """
    if len(w_compound) != n_buckets:
        raise ValueError(
            f"w_compound has {len(w_compound)} elements; expected {n_buckets}"
    ...
    target_features = _extract_features(sequence, n_buckets)
    dot_compound = sum(
        Fraction(ai) * Fraction(bi) for ai, bi in zip(compound_features, w_compound)
    )
    dot_target = sum(
        Fraction(ai) * Fraction(bi) for ai, bi in zip(target_features, w_target)
    )
    return dot_compound + dot_target + Fraction(bias)
...
class LifesciBindingRecompute:
    ...
    def recompute(self, inputs, pack_section: dict):
        ...
        no float on this path — compute_affinity_pred() (float) is the
        ...
        return RecomputedValue(
            ...
            detail=(
                ...
                f"target={target.get('target_id')!r}: R={R.numerator}/{R.denominator} "
                f"(~{float(R):.6f})"
            ),
        )
