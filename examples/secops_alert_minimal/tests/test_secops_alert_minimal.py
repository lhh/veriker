...
Coverage:
  ...
  3. Tamper test: mutate payload/alert_classification.json final_label →
     verify detects mismatch (RE_DERIVATION_MISMATCH).
...
def test_tamper_final_label_fails_verify(tmp_path: Path) -> None:
    ...
    # propagates the plugin's own reason_code, so the domain code reaches
    # stderr. While that arm stood it would have accepted a failure from ANY
    # check -- which is what the message already said it did not want.
    assert "RE_DERIVATION_MISMATCH" in stderr, (
        f"Expected RE_DERIVATION_MISMATCH in stderr; got: {stderr!r}"
    )
...
def test_tamper_dispatch_record_op_kind_fails_verify(tmp_path: Path) -> None:
    ...
    # stderr. While that arm stood it would have accepted a failure from ANY
    # check -- which is what the message already said it did not want.
    assert "OP_KIND_OUT_OF_ENUM" in stderr, (
