"""verify.py — secops_alert_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  DispatchRecordWellformedCheck         §C15  — dispatch_records well-formedness;
                                                op_kinds_admitted extended with
                                                ALERT_CLASSIFY (domain-specific kind
                                                not in the default enum)
  AlertClassificationReDerivationCheck  §C6   — alert re-derivation pack (local plugin)

Usage:
    python examples/secops_alert_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/secops_alert_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# AlertClassificationReDerivationCheck lives alongside this script
...
from AlertClassificationReDerivationCheck import AlertClassificationReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="SecOps-alert-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
    ...
    # ALERT_CLASSIFY is NOT in the default op_kinds_admitted enum —
    # pass a custom frozenset that includes it alongside RETRIEVAL and COMPUTE.
    plugins = [
        ...
        StampLatticeCheck(),
        AlertClassificationReDerivationCheck(),
    ]
