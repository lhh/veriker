"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for secops_alert.

Per-dir migration of the secops_alert_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + AlertClassificationReDerivationCheck.py +
alert_classification_re_derivation.py) and its verify.py / test are untouched, and
no committed manifest gains an `outputs` array — the spec-pinned bundle is built to
a fresh directory here, so the substrate's "0 committed manifests declare outputs"
inertness invariant is preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/secops_alert.spec.json).
  - The verifier re-derives the representative output (the final classification
    label string) IN-PROCESS via the registered primitive, and compares with the
    generic `exact` comparator. No subprocess, no bundle-supplied code.
  - Honest bundle -> PASS; tampered claimed label or tampered input -> FAIL
...
Usage:
    python examples/secops_alert_minimal/spec_pinned_check.py
...
# Legacy builder (lays down inputs/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "secops_alert__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from secops_alert_recompute import (  # noqa: E402
    _first_log_line,
    compute_final_label,
)

_OUTPUT_ID = "secops_alert_final_label"
_TYPE_KEY = "secops_alert_final_label"
_SPEC_SRC = _HERE / "spec_pinned" / "secops_alert.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned secops_alert bundle in out_dir. Reuses the legacy
    ...
    alert_log_text = (out_dir / "inputs" / "alert_log.txt").read_text(encoding="utf-8")
    rule_set = json.loads((out_dir / "inputs" / "rule_set.json").read_bytes())
    claimed = compute_final_label(_first_log_line(alert_log_text), rule_set)
...
def make_verifier(anchor=None):
    ...
    from secops_alert_recompute import SecopsAlertRecompute  # noqa: PLC0415

    register_primitive(SecopsAlertRecompute())
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the secops_alert exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  secops_alert  (spec-pinned dispatch)")
            return 0
        print("FAIL  secops_alert  (spec-pinned dispatch)", file=sys.stderr)
