...
def _verify(bundle_dir: Path) -> int:
    ...
    try:
        ...
        return _fail("RE_DERIVATION_MISMATCH", f"failed to read inputs/alert_log.txt: {exc}")

    if not lines:
        return _fail("RE_DERIVATION_MISMATCH", "inputs/alert_log.txt is empty")
    ...
    try:
        ...
        return _fail("RE_DERIVATION_MISMATCH", f"failed to read inputs/rule_set.json: {exc}")

    if not isinstance(rule_set, list):
        return _fail("RE_DERIVATION_MISMATCH", "inputs/rule_set.json must be a JSON array")
    ...
    try:
        ...
        return _fail("RE_DERIVATION_MISMATCH", f"failed to read manifest.json: {exc}")
    ...
    try:
        ...
        return _fail("RE_DERIVATION_MISMATCH", f"re-derivation error: {exc}")
