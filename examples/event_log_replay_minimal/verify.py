"""verify.py — the AUDITOR entry point for event_log_replay_minimal.

    python examples/event_log_replay_minimal/verify.py [--bundle-dir DIR]
...
auditor's OWN committed `spec_pinned/event_log_replay.spec.json` bytes via the
blessed `SpecAnchor.from_files`, with `forbid_within=bundle_dir`.

No primitive is registered here. `event_log_replay_recompute` ships in the
verifier distribution and resolves purely by core auto-registration, so this
pilot is reachable by the bare CLI as well:
...
WHAT IS AND IS NOT PROVEN. A PASS certifies that replaying the bundle's sole
append-only log reconstructs a record whose digest is exactly the one the
producer claimed — so a dropped, reordered or forged AMEND is caught, and so is
a second log smuggled in beside the first (that is refused outright rather than
resolved). It does NOT certify that the log is COMPLETE: a maintenance action
that was never written down leaves no trace to replay, and no re-derivation over
the log can find it. Nor does it certify the truth of any field. Faithfulness of
the reconstruction, not completeness of the history.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Verify an event_log_replay_minimal bundle"
    )
    ap.add_argument(
        "--bundle-dir",
        type=Path,
