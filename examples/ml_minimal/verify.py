"""verify.py — ml_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  MlReDerivationCheck      §C6 — ML inference re-derivation (integer-only)

Usage:
    python examples/ml_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/ml_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# MlReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from MlReDerivationCheck import MlReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="ml_minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
