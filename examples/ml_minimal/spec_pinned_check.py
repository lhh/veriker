"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for ml_minimal.

Per-dir migration of the ml_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + MlReDerivationCheck.py + ml_re_derivation.py)
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/ml.spec.json).
  - The verifier re-derives the representative output (the list of predicted class
    indices — the integer argmax per input sample) IN-PROCESS via the registered
    primitive (integer-only linear classifier: logits = W@x + b, argmax), and
...
Usage:
    python examples/ml_minimal/spec_pinned_check.py
...
# Legacy builder (lays down inputs/weights/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("ml_minimal__legacy_build_bundle", _HERE / "_build_bundle.py")
...
from ml_recompute import compute_prediction_classes  # noqa: E402

_OUTPUT_ID = "ml_prediction_classes"
_TYPE_KEY = "ml_prediction_classes"
_SPEC_SRC = _HERE / "spec_pinned" / "ml.spec.json"
...
def _honest_classes(out_dir: Path) -> list[int]:
    """Auditor-side canonical recompute of the honest predicted-class list from the
    built bundle's committed weights + inputs."""
    model = json.loads((out_dir / "weights" / "model.json").read_bytes())
    feature_vectors = json.loads((out_dir / "inputs" / "features.json").read_bytes())
    return compute_prediction_classes(model, feature_vectors)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned ml bundle in out_dir. Reuses the legacy builder for
    ...
    claimed = _honest_classes(out_dir)
...
def make_verifier(anchor=None):
    ...
    from ml_recompute import MlRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the ml_minimal pilot"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  ml_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  ml_minimal  (spec-pinned dispatch)", file=sys.stderr)
