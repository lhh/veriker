"""verify.py — iso42001_external_reporting_minimal domain pilot bundle verifier.

the audit-bundle contract §C5 (auditor independence). Registers TWO primitives
(decision-count -> exact; oversight-rate -> rational_band) and verifies a
two-output spec-pinned bundle. The auditor's SpecAnchor is derived from the
COMMITTED spec file, NOT the bundle's copy.

Usage:
    python examples/iso42001_external_reporting_minimal/verify.py --bundle-dir <path>
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="iso42001_external_reporting_minimal audit bundle verifier (§C5)"
    )
