...
| `disclosed_human_oversight_rate_pct` | 100 · (automated ∧ human_reviewed) / automated | `rational_band` (1e-9) | 66.6667 |
...
| `iso42001_external_reporting_recompute.py` | Two `ReDerivationPrimitive`s (count → exact, rate → rational_band), each re-deriving on its OWN implementation, plus the separate producer helpers the builder uses. |
...
2. **Rate mutation (rational_band)** — inflate the oversight rate by +0.5 →
   `RE_DERIVATION_MISMATCH`.
...
(×2), `SpecAnchor` from committed spec bytes, the `exact` **and** `rational_band`
