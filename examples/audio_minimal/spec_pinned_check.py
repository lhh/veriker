"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for audio_minimal.

Per-dir migration of the audio_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + AudioReDerivationCheck.py + audio_re_derivation.py)
and its verify.py / test are untouched, and no committed manifest gains an `outputs`
array — the spec-pinned bundle is built to a fresh directory here, so the substrate's
"0 committed manifests declare outputs" inertness invariant is preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/audio.spec.json).
  - The verifier re-derives the representative output (the set of VAD segment
    boundaries — the unordered set of [start_frame, end_frame] pairs) IN-PROCESS
    via the registered primitive (frame-energy-threshold VAD over the committed
    int16 PCM bytes in audio/samples.bin per spec/segmentation.json), and compares
    with the generic `set` comparator (order-independent). No subprocess, no
    bundle-supplied code. segment_id/text are NOT compared.
  - Honest bundle -> PASS; tampered claimed value or tampered input -> FAIL
    (RE_DERIVATION_MISMATCH, wrapping the set-comparator mismatch); no auditor anchor
    -> fail-closed (AnchorViolation).

Usage:
    python examples/audio_minimal/spec_pinned_check.py
...
# Legacy builder (lays down audio/spec/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "audio_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from audio_recompute import compute_vad_boundaries  # noqa: E402

_OUTPUT_ID = "audio_vad_boundaries"
_TYPE_KEY = "audio_vad_boundaries"
_SPEC_SRC = _HERE / "spec_pinned" / "audio.spec.json"
...
def _honest_boundaries(out_dir: Path) -> list[list[int]]:
    """Producer-side honest claimed value = the set of [start_frame, end_frame]
    pairs derived from payload/transcript.json segments (segment_id/text dropped)."""
    transcript = json.loads(
        (out_dir / "payload" / "transcript.json").read_text(encoding="utf-8")
    )
    return [[s["start_frame"], s["end_frame"]] for s in transcript["segments"]]
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned audio_minimal bundle in out_dir. Reuses the legacy
    builder for audio/spec/payload, then overlays the β shape: an auditor spec
    ...
    # --- Producer claimed value = honest boundary pairs from the transcript. ---
    claimed = _honest_boundaries(out_dir)
    ...
    # The spec-pinned verifier resolves the auditor spec by basename
    # (FileIntegrity Pass-3 skips spec/). Record the auditor binding spec's SHA.
...
def make_verifier(anchor=None):
    ...
    from audio_recompute import AudioRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the audio_minimal pilot"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  audio_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  audio_minimal  (spec-pinned dispatch)", file=sys.stderr)
