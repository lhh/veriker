"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for tabular_minimal.

Per-dir migration of the tabular_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + TabularReDerivationCheck.py +
tabular_re_derivation.py) and its verify.py / test are untouched, and no committed
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/tabular.spec.json).
  - The verifier re-derives the representative output (the SHA-256 hex digest of
    the recomputed result CSV bytes) IN-PROCESS via the registered primitive,
    re-executing the committed GROUP BY + SUM/COUNT query over data/sales.csv,
    and compares it with the generic `exact` comparator. No subprocess, no
...
Usage:
    python examples/tabular_minimal/spec_pinned_check.py
...
# Legacy builder (lays down data/spec/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "tabular_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from tabular_recompute import compute_result_sha  # noqa: E402

_OUTPUT_ID = "result_sha"
_TYPE_KEY = "result_sha"
_SPEC_SRC = _HERE / "spec_pinned" / "tabular.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned tabular_minimal bundle in out_dir. Reuses the legacy
    builder for data/spec/payload, then overlays the β shape: an auditor spec
    ...
    query_bytes = (out_dir / "spec" / "query.json").read_bytes()
    sales_bytes = (out_dir / "data" / "sales.csv").read_bytes()
    claimed = compute_result_sha(query_bytes, sales_bytes)
    ...
    # Preserve the legacy spec_files (query.json) and add the auditor spec.
    manifest.setdefault("spec_files", {})[_SPEC_BASENAME] = spec_sha
    manifest["outputs"] = [
        {
    ...
    # primitive re-derives the artifact DIGEST on the outputs/ claim, and the
    # legacy byte-comparing plugin (the check that opens and compares the
    # declared opaque artifact) is deliberately not wired here. Carrying the
    # declaration would be a false coverage story, so the variant drops it
    # and the verdict carries the honest "claimset: not declared" disclosure.
    # The declared pilot path is exercised by the pilot's own test module.
...
def make_verifier(anchor=None):
    ...
    from tabular_recompute import TabularRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the tabular_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  tabular_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  tabular_minimal  (spec-pinned dispatch)", file=sys.stderr)
