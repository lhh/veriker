...
class TabularReDerivationCheck:
    ...
    def check(self, bundle_dir: Path, manifest) -> PluginResult:
        ...
        if result.returncode == 0:
            # Claimset coverage accounting: the pack byte-compared the
            # re-derived aggregate against payload/result.csv. Report the
            # opaque key whose DECLARED path is that file (never a constant
            # key); result.csv is already in files_audited. ONLY on a full pass.
            return PluginResult(
                ok=True,
