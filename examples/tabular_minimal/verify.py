"""verify.py — tabular_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  TabularReDerivationCheck     §C6 — tabular GROUP BY re-derivation (local pack)

Usage:
    python examples/tabular_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/tabular_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# TabularReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from TabularReDerivationCheck import TabularReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Tabular-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
