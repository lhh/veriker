...
class Iso42001DisparateImpactRecompute:
    ...
    (compared rational_band).

    The producer's claim stays `compute_disparate_impact_ratio` (a float,
    committed UNROUNDED) — that shared helper is untouched. The recompute
    path here is a SEPARATE, exact-rational implementation: per-group rates
    are exact Fractions(favorable, total) over the declared int counts, and
    the ratio is min/max of those Fractions, decided exactly — no float
    anywhere on this path, so the claim can never be derived from it.
    """
    ...
    def recompute(self, inputs, pack_section: dict):
        ...
        rates = [Fraction(favorable[g], totals[g]) for g in totals]
        max_rate = max(rates)
        if max_rate == 0:
            raise ValueError("max group selection rate is 0 — ratio undefined")
        ...
        n_groups = len(totals)
        detail = (
            f"re-derived disparate-impact ratio over {len(records)} subjects, "
            f"{n_groups} groups; disparate_impact_ratio~{float(ratio):.6f}"
        )
