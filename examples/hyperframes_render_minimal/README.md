...
verifier reports `RE_DERIVATION_MISMATCH`.
...
`RE_DERIVATION_MISMATCH` because re-rendering the tampered HTML
...
The rendered MP4 is the claim, declared whole-file (opaque):
`manifest.claimset.opaque_claim_files = {"output": "payload/output.mp4"}`.
`HyperFramesReDerivationCheck` reports `output` covered ONLY when the pack
exited 0 and its `[COMPARED]` line names the file. **What the pack binds,
after the 2026-08-20 fix:** `sha256(bundled payload/output.mp4) ==
manifest.payload.output_mp4_sha256 == sha256(re-rendered MP4)`. Before the
fix the pack compared only the re-render against the committed digest and
never hashed the delivered file — the per-field tamper battery (port 2b,
prediction P3) flipped a delivered byte at first/middle/last, re-pinned the
manifest, and the verdict stayed green. The battery in
`tests/test_hyperframes_render_minimal.py` now re-pins BOTH the file sha and
the committed digest (everything a producer controls) and requires the
RE-RENDER's `[HYPERFRAMES_REDER_FAIL]` refusal at all five probes. Needs
node / npx / ffmpeg; where they are absent the module is skipped and the
bundle verifies could-not-conclude (exit 2) naming `'output'`, never a
silent PASS.
