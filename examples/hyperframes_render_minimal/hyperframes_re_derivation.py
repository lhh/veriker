...
Re-derivation:
  ...
  4b. Compute sha256(payload/output.mp4) — the BUNDLED artifact, the claim a
      relying party receives.
  5. Compute sha256(rederive.mp4); require committed == re-derived AND
     bundled == re-derived (the render binds the delivered bytes; a digest
     the producer commits never does).
  6. Exit 0 when bundled == committed == re-derived; exit 1 with
     [HYPERFRAMES_REDER_FAIL] RE_DERIVATION_MISMATCH otherwise.
...
def _emit_compared(rels: "list[str]") -> None:
    """Print the bundle-relative artifact paths this pack BYTE-COMPARED (the
    bundled MP4, whose sha must equal the re-render's), as a machine-readable
    stdout line, after the comparison succeeded."""
    print("[COMPARED] " + json.dumps(sorted(rels)))
...
def _verify(bundle_dir: Path) -> str | None:
    ...
    # ---- 3b. the BUNDLED artifact's digest (compared AFTER the render) ----
    # Deliberately NOT short-circuited against manifest.payload.output_mp4_sha256
    # here: that digest is producer-committed, and a refusal at a digest the
    # producer controls is indistinguishable from a re-derivation refusal to
    # the tamper battery (round-2 audit). The render is the binding.
    bundled_sha = _sha256_file(mp4_path)
    ...
    # The binding that makes the delivered artifact the claim: bundled bytes
    # equal the re-render's bytes. (Stated explicitly even though bundled ==
    # committed == re-derived already implies it — a producer who re-pins the
    # committed digest to a tampered delivery is caught HERE, by the render,
    # not by a digest they control.)
    if bundled_sha != rederive_sha:
        return (
            f"RE_DERIVATION_MISMATCH: bundled payload/output.mp4 sha "
            f"{bundled_sha[:16]}... differs from the re-rendered sha {rederive_sha[:16]}..."
        )

    _emit_compared(["payload/output.mp4"])
    return None
