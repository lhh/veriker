...
- **Placement binding (FIX-B, claim-set coverage sweep).** The C16 leg alone
  never opened `payload/placement.json` — the human-facing placement was fully
  decoupled from the discharged numbers. `PayrollPlacementBindingCheck`
  (pilot-level, wired in `verify.py`) closes that: strict parse + exact-keys
  closed schema (`PAD_PLACEMENT_MALFORMED`), `acting_rate_cents` must equal
  the re-discharged context value (`PAD_PLACEMENT_RATE_UNBOUND`), the
  context's band edges must re-derive from the sha-pinned
  `spec/acting_pay_rules.json` (`PAD_BAND_EDGES_NOT_REDERIVED`), and the
  admissible band / classifications / `in_band` must match recomputation
  (`PAD_PLACEMENT_BAND_MISMATCH` / `PAD_PLACEMENT_FIELD_MISMATCH`).
  **Recorded residuals:** `employee_id` has no binding target in this bundle
  (no roster ships), and `band_source` prose is not semantically bound —
