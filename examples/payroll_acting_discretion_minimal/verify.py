...
def verify(bundle_dir: Path) -> int:
    ...
    plugins = [
        ...
        *default_post_w3_plugin_set(),
        # FIX-B binding leg: payload/placement.json ↔ discharged context ↔
        # pinned agreement (the C16 leg alone never opened the payload).
    ...
    if result.ok:
        ...
        print(
            "  payroll_placement_binding: payload/placement.json equals the "
            "re-discharged context; band edges re-derived from the pinned "
            "agreement"
        )
        return 0
