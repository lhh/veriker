...
def build(out_dir: Path) -> None:
    ...
    content = BundleContent(
        ...
        extra_manifest_fields={
            ...
            # Claim-field coverage declaration: every schema-level field of
            # placement.json must be read-and-compared by a wired check or
            # excused with a committed reason. The two residuals below are the
            # pilot's long-recorded honest limits, now machine-readable on the
            # verdict instead of prose: employee_id has no binding target
            # anywhere in this bundle (no roster, no second copy), and
            # band_source is prose whose semantics no mechanical check binds.
            "claimset": {
                "claim_files": {"placement": "payload/placement.json"},
                "residuals": {
                    "placement:employee_id": "NO_BINDING_TARGET",
                    "placement:band_source": "NOT_MECHANICALLY_CHECKABLE",
                },
            },
        },
    )
    ...
    print(
        f"  discretionary rate  : {acting_rate}  (in-band — re-derivation has no answer here)"
    )
