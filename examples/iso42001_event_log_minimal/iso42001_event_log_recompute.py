...
# ---------------------------------------------------------------------------
# PRODUCER computation (imported by _build_bundle.py). The verifier does NOT
# call this -- see _recompute_chain_head_independent below.
# ---------------------------------------------------------------------------
...
def _recompute_chain_head_independent(events: list) -> str:
    """Verifier-side chain fold, written SEPARATELY from the producer helper.

    `compute_log_head_digest` above stays the PRODUCER's helper and is
    untouched. This function never calls it, so the disclosed head can never be
    derived from the verifier's own code -- the sharing made the check
    f(x) == f(x), so an error in the CHAINING RULE (leaf serialization, fold
    order, genesis) was baked into claim and check alike and could not be
    caught. A tampered VALUE was still caught either way; method independence
    was not demonstrated. The two must never be unified.

    Same rule, expressed by accumulating the running digest with explicit
    index-ordered iteration and byte-level concatenation rather than string
    concatenation.
    """
    if not events:
        raise ValueError("event log is empty -- head digest undefined")
    ...
    for position in range(len(events)):
        ...
        if not isinstance(event, dict):
            raise ValueError(f"each event must be an object, got {type(event).__name__}")
        leaf_digest = hashlib.sha256(
            json.dumps(event, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
        ).hexdigest()
        running = hashlib.sha256(running.encode("ascii") + leaf_digest.encode("ascii")).hexdigest()
    return running
...
class Iso42001LogChainHeadRecompute:
    ...
    def recompute(self, inputs, pack_section: dict):
        ...
        value = _recompute_chain_head_independent(events)
