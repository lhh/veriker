...
Surfaces:
  ...
  1. Happy path -> PASS.
  2. Digest mutation (exact): doctor the claimed head -> RE_DERIVATION_MISMATCH.
  3. Event field tamper: edit an event 'action' without updating manifest.files
     -> BAD_FILE_SHA + RE_DERIVATION_MISMATCH (head changes).
  4. Event reorder: swap two events -> head changes (order-sensitive) ->
     BAD_FILE_SHA + RE_DERIVATION_MISMATCH.
...
def test_verifier_chain_fold_is_independent_of_the_producer_helper():
    """MUTANT CONTROL for the producer/verifier tautology (fixed 2026-08-29).

    The producer computes the disclosed head with `compute_log_head_digest`;
    until this fix the registered primitive re-derived with THE SAME function,
    so an error in the CHAINING RULE (leaf serialization, fold order, genesis)
    was baked into claim and check alike. Measured then: reversing the fold to
    sha256(leaf + h) in the producer helper still verified CLEAN; with the
    verifier on its own fold the same bundle is RE_DERIVATION_MISMATCH.
    """
    events = [{"seq": 1, "op": "CREATE"}, {"seq": 2, "op": "AMEND"}]
    truth = _prim_mod._recompute_chain_head_independent(events)
    assert truth == _prim_mod.compute_log_head_digest(events), (
        "the two implementations must AGREE on honest input"
    )
    original = _prim_mod.compute_log_head_digest
    try:
        _prim_mod.compute_log_head_digest = lambda *a, **k: "0" * 64
        again = _prim_mod._recompute_chain_head_independent(events)
    finally:
        _prim_mod.compute_log_head_digest = original
    assert again == truth, (
        "the verifier fold changed when the PRODUCER helper was replaced -- "
        "the two are not independent"
    )
    # order sensitivity survives the split
    assert _prim_mod._recompute_chain_head_independent(list(reversed(events))) != truth
