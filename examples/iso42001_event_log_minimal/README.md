...
| `iso42001_event_log_recompute.py` | The `ReDerivationPrimitive` (SHA-256 chain fold, pinned genesis) re-deriving on its OWN fold, plus the separate producer helper the builder uses. |
...
1. **Digest mutation** — doctor the claimed head → `RE_DERIVATION_MISMATCH`.
