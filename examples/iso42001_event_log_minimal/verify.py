"""verify.py — iso42001_event_log_minimal domain pilot bundle verifier.

the audit-bundle contract §C5 (auditor independence). Registers the
Iso42001LogChainHeadRecompute primitive and verifies a single-output spec-pinned
bundle whose claimed value is a hash-chain head digest (compared exact). The
auditor's SpecAnchor is derived from the COMMITTED spec file, NOT the bundle's
copy.

Usage:
    python examples/iso42001_event_log_minimal/verify.py --bundle-dir <path>
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="iso42001_event_log_minimal audit bundle verifier (§C5)"
    )
