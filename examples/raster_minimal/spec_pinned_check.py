"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for raster_minimal.

Per-dir migration of the raster_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + RasterReDerivationCheck.py + raster_re_derivation.py)
and its verify.py / test are untouched, and no committed manifest gains an `outputs`
array — the spec-pinned bundle is built to a fresh directory here, so the
substrate's "0 committed manifests declare outputs" inertness invariant is preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/raster.spec.json).
  - The verifier re-derives the representative output (the in_polygon_cell_count
    integer) IN-PROCESS via the registered primitive (ray-casting point-in-polygon
    over the committed int8 raster grid + polygon vertices), and compares with the
    generic `exact` comparator (integer equality). No subprocess, no bundle code.
...
Usage:
    python examples/raster_minimal/spec_pinned_check.py
...
# Legacy builder (lays down raster/spec/payload/manifest) + the shared canonical
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "raster_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from raster_recompute import (  # noqa: E402
    _read_raster,
...
_SPEC_SRC = _HERE / "spec_pinned" / "raster.spec.json"
...
def _honest_count(out_dir: Path) -> int:
    """Auditor-side canonical recompute of the honest in_polygon_cell_count from the
    built bundle's committed raster grid + polygon spec."""
    spec = json.loads((out_dir / "spec" / "zonal_query.json").read_text(encoding="utf-8"))
    shape = spec.get("raster", {}).get("shape", [32, 32])
    rows, cols = int(shape[0]), int(shape[1])
    polygon = spec.get("polygon", [])
    grid = _read_raster(out_dir / "raster" / "grid.bin", rows, cols)
    return compute_in_polygon_cell_count(grid, polygon)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned raster bundle in out_dir. Reuses the legacy builder for
    raster/spec/payload, then overlays the beta shape: an auditor spec under spec/, a
...
def make_verifier(anchor=None):
    ...
    from raster_recompute import RasterRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the raster_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  raster_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  raster_minimal  (spec-pinned dispatch)", file=sys.stderr)
