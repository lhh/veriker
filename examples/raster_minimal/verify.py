"""verify.py — raster_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  RasterReDerivationCheck    §C6 — geospatial raster zonal re-derivation

Usage:
    python examples/raster_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/raster_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# RasterReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from RasterReDerivationCheck import RasterReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Raster-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
