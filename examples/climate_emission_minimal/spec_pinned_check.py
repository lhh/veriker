...
What it demonstrates:
  ...
  - Honest bundle -> PASS; tampered claimed list or tampered input -> FAIL
    (RE_DERIVATION_MISMATCH); no auditor anchor / substituted spec -> fail-closed
    (AnchorViolation).
...
def _reset_dispatch_shape(out_dir: Path) -> None:
    """Clear the dispatch shape the pilot's own builder now emits.

    Since the 2026-08-16 spec-pinned migration, `_build_bundle.build()` writes
    its OWN spec/ + outputs/ + manifest.outputs (two declared outputs). This
    driver lays down a DIFFERENT, single-output shape on top, so it must first
    remove the native one — otherwise the §4a.4 coverage invariant correctly
    reports the pilot's extra output as present-but-undeclared.
    """
    import shutil as _shutil

    for sub in ("spec", "outputs"):
        _shutil.rmtree(out_dir / sub, ignore_errors=True)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            ...
            return 0
        print(
            "FAIL  climate_emission  (spec-pinned dispatch, structured)",
            file=sys.stderr,
        )
