...
def _reset_dispatch_shape(out_dir: Path) -> None:
    ...
    its OWN spec/ + outputs/ + manifest.outputs. This driver lays down its own
    two-output shape on top, so it must first remove the native one — otherwise
    the §4a.4 coverage invariant correctly reports a duplicate/undeclared
    output.
    """
