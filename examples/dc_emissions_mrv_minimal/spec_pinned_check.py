"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for dc_emissions_mrv.

Per-dir migration of the dc_emissions_mrv pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + DCEmissionsMRVReDerivationCheck.py +
re_derive/dc_emissions_mrv_pack.py) and its verify.py / test are untouched, and no
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/dc_emissions_mrv.spec.json).
  - The verifier re-derives the representative output (total embodied carbon)
...
Usage:
    python examples/dc_emissions_mrv_minimal/spec_pinned_check.py
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "dc_emissions_mrv__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from dc_emissions_mrv_recompute import compute_embodied_total  # noqa: E402

_OUTPUT_ID = "dc_embodied_carbon_total"
_TYPE_KEY = "dc_embodied_carbon_total"
_SPEC_SRC = _HERE / "spec_pinned" / "dc_emissions_mrv.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned dc_emissions_mrv bundle in out_dir. Reuses the legacy
    ...
    materials = json.loads(
        (out_dir / "inputs" / "embodied_carbon_inventory.json").read_bytes()
    )
    claimed = compute_embodied_total(materials)
...
def make_verifier(anchor=None):
    ...
    from dc_emissions_mrv_recompute import DcEmissionsMrvEmbodiedRecompute  # noqa: PLC0415

    register_primitive(DcEmissionsMrvEmbodiedRecompute())
    return BundleVerifier(plugins=[FileIntegrityManySmall()], spec_anchor=anchor)


def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the dc_emissions_mrv exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  dc_emissions_mrv  (spec-pinned dispatch)")
            return 0
        print("FAIL  dc_emissions_mrv  (spec-pinned dispatch)", file=sys.stderr)
