...
# In-place build, verified by the pilot's own anchored wrapper:
python examples/dc_emissions_mrv_minimal/_build_bundle.py
python examples/dc_emissions_mrv_minimal/verify.py --bundle-dir examples/dc_emissions_mrv_minimal/    # PASS
...
#     rejected by design).
# (4) Inflate a displayed per-pollutant threshold (major_source_threshold_tpy /
#     synthetic_minor_threshold_tpy) in payload/nsr_submission.json to a more
#     LENIENT ceiling than inputs/permit_thresholds.json actually commits,
#     leaving total_tpy and classification unchanged, AND realign manifest SHA
#     → file_integrity passes; total_tpy/classification comparisons pass
#     (a classification is stable across a range of threshold values); the
#     re-derivation pack's threshold-binding check catches the fabricated
#     ceiling: DCMRV_THRESHOLD_MISMATCH. Same code fires fail-closed if the
#     displayed threshold field is missing entirely.
# See tests/test_dc_emissions_mrv_minimal.py for all four flows.
```
...
the re-derivation pack as a hard fail (`CITATION_MISSING`); the displayed
per-pollutant `synthetic_minor_threshold_tpy` / `major_source_threshold_tpy`
values are bound to `inputs/permit_thresholds.json` by exact equality, fail
closed on either field missing (`DCMRV_THRESHOLD_MISMATCH`) — a classification
string alone does not pin the ceiling it was computed against.
