...
LCA submission pilot. Re-derivation primitive (one sentence):
  ...
  payload/nsr_submission.json. Also binds the per-pollutant threshold VALUES
  the submission displays (synthetic_minor_threshold_tpy /
  major_source_threshold_tpy) to inputs/permit_thresholds.json by exact
  equality, fail-closed on either field missing (DCMRV_THRESHOLD_MISMATCH) —
  a classification alone is stable across a range of threshold values, so it
  does not by itself pin the ceiling it was computed against.
...
Re-derivation primitive:
  ...
  exactly. Also bind the per-pollutant threshold VALUES the submission displays
  (synthetic_minor_threshold_tpy / major_source_threshold_tpy) to
  inputs/permit_thresholds.json by exact equality, fail-closed if either field
  is missing or non-numeric — a classification is stable across a range of
  threshold values, so it does not by itself pin the displayed ceiling.

Exit 0 on full match; exit 1 with [DCMRV_REDER_FAIL] <description> on stderr
(threshold-binding failures are additionally tagged DCMRV_THRESHOLD_MISMATCH).
"""
...
def main() -> int:
    ...
    for poll in _CRITERIA_POLLUTANTS:
        ...
        # ---- Threshold-binding (DCMRV_THRESHOLD_MISMATCH) ----
        # The submission DISPLAYS synthetic_minor_threshold_tpy /
        # major_source_threshold_tpy per pollutant next to the classification
        # string, but a classification is stable across a RANGE of threshold
        # values -- nothing above binds the displayed ceiling to the committed
        # inputs/permit_thresholds.json value, so a submission could show a
        # fabricated (more lenient) ceiling next to an otherwise-correct
        # classification. Bind both displayed fields to the committed input
        # exactly; fail closed if either is missing/non-numeric on either side.
        if poll not in syn_thr or poll not in maj_thr:
            return _fail(
                f"{poll} absent from inputs/permit_thresholds.json "
                f"(DCMRV_THRESHOLD_MISMATCH): cannot bind displayed thresholds"
            )
        displayed_synmin = bundled.get("synthetic_minor_threshold_tpy")
        displayed_majsrc = bundled.get("major_source_threshold_tpy")
        if not isinstance(displayed_synmin, (int, float)) or isinstance(displayed_synmin, bool):
            return _fail(
                f"{poll} synthetic_minor_threshold_tpy missing or non-numeric "
                f"(DCMRV_THRESHOLD_MISMATCH): displayed={displayed_synmin!r}"
            )
        if not isinstance(displayed_majsrc, (int, float)) or isinstance(displayed_majsrc, bool):
            return _fail(
                f"{poll} major_source_threshold_tpy missing or non-numeric "
                f"(DCMRV_THRESHOLD_MISMATCH): displayed={displayed_majsrc!r}"
            )
        if float(displayed_synmin) != synmin:
            return _fail(
                f"{poll} synthetic_minor_threshold_tpy mismatch "
                f"(DCMRV_THRESHOLD_MISMATCH): displayed={displayed_synmin} "
                f"committed={synmin}"
            )
        if float(displayed_majsrc) != majsrc:
            return _fail(
                f"{poll} major_source_threshold_tpy mismatch "
                f"(DCMRV_THRESHOLD_MISMATCH): displayed={displayed_majsrc} "
                f"committed={majsrc}"
            )
