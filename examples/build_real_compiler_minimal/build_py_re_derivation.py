...
Re-derivation procedure:
  ...
  4. For each source name in recipe.sources (in order):
       ...
       d. On first mismatch: exit 1 with [BUILD_PY_REDER_FAIL] RE_DERIVATION_MISMATCH.
...
def _emit_compared(rels: "list[str]") -> None:
    """Print the bundle-relative artifact paths this pack BYTE-COMPARED, as a
    machine-readable stdout line, after the comparison succeeded. The pilot's
    plugin reports claimset coverage for exactly these files — coverage
    follows what the pack says it compared, never a constant in the wrapper."""
...
def _verify(bundle_dir: Path) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        return f"failed to read recipe/build_recipe.json: {exc}", "RE_DERIVATION_MISMATCH"
    ...
    try:
        ...
        if not isinstance(sources_list, list) or not sources_list:
            return "recipe.sources missing or empty", "RE_DERIVATION_MISMATCH"
        ...
        for source_name in sources_list:
            ...
            compared.append(f"payload/artifacts/{pyc_name}")
            ...
            if fresh_pyc != bundled_pyc:
                ...
                for i in range(n):
                    if fresh_pyc[i] != bundled_pyc[i]:
                        ...
                        break
                return (
                    f"RE_DERIVATION_MISMATCH for {source_name!r}: "
