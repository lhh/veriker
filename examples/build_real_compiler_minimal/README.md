...
# 4. Verify — must FAIL with RE_DERIVATION_MISMATCH
python verify.py --bundle-dir /tmp/tbr
# stderr: [build_py_re_derivation] RE_DERIVATION_MISMATCH: ...
```
...
Each compiled `.pyc` is a whole-file (opaque) claim, keyed by module stem:
`manifest.claimset.opaque_claim_files = {"mod_a": "payload/artifacts/mod_a.pyc",
…}`. `BuildPyReDerivationCheck` reports a key covered ONLY when the pack
exited 0 and its `[COMPARED]` line names that file (the pack recompiles each
recipe source and byte-compares the `.pyc`); the verdict then carries
`n_universe=3 n_covered=3 … n_opaque=3`. A pack that cannot run reports
nothing → could-not-conclude naming the three keys. The tamper battery in
`tests/test_build_real_compiler_minimal.py` flips each artifact at five
probes (first / middle / last byte, truncate, append) and requires the pack's
own `[BUILD_PY_REDER_FAIL]` refusal every time. The spec-pinned overlay lane
drops the declaration (its plugin set does not wire this byte comparison).
Note for adopters: the key is the module stem with no `.`, `:`, `[`, `]`
(reserved by the element rendering) — CPython's `name.cpython-312.pyc`
naming would need a sanitized key.

## Reason codes

| Code | Meaning |
|---|---|
| `RE_DERIVED` | ok=True — all .pyc bytes match |
| `RE_DERIVATION_MISMATCH` | re-derived .pyc bytes differ from bundled |
| `BUILD_PY_TOOLCHAIN_MISMATCH` | verifier's cache_tag != recipe's cache_tag |
| `RE_DERIVATION_TIMEOUT` | re-derivation pack subprocess exceeded 60 s |
