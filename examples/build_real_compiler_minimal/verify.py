"""verify.py — build_real_compiler_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  BuildPyReDerivationCheck       §C6 — deterministic py_compile re-derivation (local pack)

Usage:
    python examples/build_real_compiler_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/build_real_compiler_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# BuildPyReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from BuildPyReDerivationCheck import BuildPyReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="build-real-compiler-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
