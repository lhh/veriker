...
def build(out_dir: Path) -> None:
    ...
    try:
        ...
        content = BundleContent(
            ...
            # Claim-field coverage: each compiled .pyc is a whole-file (opaque) claim,
            # keyed by module stem; BuildPyReDerivationCheck reports a key covered only
            # after the pack byte-compares that module's recompiled bytes.
            extra_manifest_fields={
                "claimset": {"opaque_claim_files": {Path(n).stem: f"payload/artifacts/{n}" for n in sorted(pyc_bytes)}}
            },
        )
