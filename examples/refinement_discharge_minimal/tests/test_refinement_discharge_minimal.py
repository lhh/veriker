"""Pilot tests for refinement_discharge_minimal — the honest S2 / C16 demo.

Builds the bundle, verifies it green through the default plugin set (real Z3
re-run agrees), and confirms the four demo scenarios behave — including the
divergence branch that retains a signed, re-verifiable divergence record.
"""

from __future__ import annotations

import copy
import json
import shutil
import sys
import tempfile
from pathlib import Path

import pytest

_PILOT_DIR = Path(__file__).resolve().parents[1]
_PKG_ROOT = Path(__file__).resolve().parents[3]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

from audit_bundle.discharge.verifier_signing import (  # noqa: E402
    DIVERGENCE_RECORD_KIND,
    VerifierSigningKey,
    sign_and_write,
    verify_divergence_record,
)
from audit_bundle.discharge.z3_runner import (  # noqa: E402
    InProcessZ3Invoker,
    pick_default_invoker,
)
from audit_bundle.plugins import default_post_w3_plugin_set  # noqa: E402
from audit_bundle.plugins.file_integrity_many_small import FileIntegrityManySmall  # noqa: E402
from audit_bundle.plugins.refinement_discharge import RefinementDischargeCheck  # noqa: E402
from audit_bundle.plugins.spec_sha_pin import SpecShaPinCheck  # noqa: E402
from audit_bundle.verifier import BundleVerifier  # noqa: E402

# Import the pilot build module.
sys.path.insert(0, str(_PILOT_DIR))
import _build_bundle  # noqa: E402

_DEMO_KEY = "demo-vkernel-verifier-secret-0123456789abcdef"
_BUNDLE_ID = "refinement-discharge-minimal-rc"

_z3_unavailable = pick_default_invoker() is None
pytestmark = pytest.mark.skipif(
    _z3_unavailable, reason="no Z3 invoker available (z3-solver module or binary)"
)


@pytest.fixture()
def built_bundle(tmp_path, monkeypatch):
    monkeypatch.setenv("VKERNEL_VERIFIER_HMAC_KEY", _DEMO_KEY)
    out = tmp_path / "bundle"
    _build_bundle.build(out)
    return out


def _manifest_stub(records, bundle_id=_BUNDLE_ID):
    class _M:
        def __init__(self):
            self.dispatch_records = tuple(records)
            self.bundle_id = bundle_id

    return _M()


def test_build_produces_signed_discharged_record(built_bundle):
    """The shipped record carries a verifier-signed 'discharged' status over a
    smt-z3 proof with a digest-pinned obligation file."""
    manifest = json.loads((built_bundle / "manifest.json").read_text())
    recs = manifest["dispatch_records"]
    assert len(recs) == 1
    proof = recs[0]["proof"]
    assert proof["kind"] == "smt-z3"
    assert proof["discharge_status"] == "discharged"
    assert proof["verifier_signature"]["algorithm"] == "hmac-sha256"
    # obligation file present and digest matches the manifest.files entry
    obl_uri = proof["obligation_uri"]
    assert (built_bundle / obl_uri).exists()
    assert manifest["files"][obl_uri] == proof["obligation_sha"]


def test_verifies_green_through_default_plugin_set(built_bundle, monkeypatch):
    """Headline: the honest bundle PASSes through the SAME default plugin set
    veriker/cli/verify.py uses — C16 admits the signed discharge and the Z3 re-run
    agrees (re-discharged)."""
    monkeypatch.setenv("VKERNEL_VERIFIER_HMAC_KEY", _DEMO_KEY)
    plugins = [
        FileIntegrityManySmall(),
        SpecShaPinCheck(),
        *default_post_w3_plugin_set(),
    ]
    result = BundleVerifier(plugins=plugins).verify(built_bundle)
    assert result.ok is True, [
        f"{f.check_name}:{f.reason_code}" for f in result.failures
    ]


def test_c16_re_discharges_on_honest_bundle(built_bundle):
    """Direct C16 check with a real in-process Z3 invoker: the signed status is
    admitted AND the re-run agrees (detail says re-discharged)."""
    key = VerifierSigningKey.from_secret_bytes(_DEMO_KEY.encode("utf-8"))
    recs = json.loads((built_bundle / "manifest.json").read_text())["dispatch_records"]
    plugin = RefinementDischargeCheck(
        recheck_key=key, recheck_invoker=InProcessZ3Invoker()
    )
    result = plugin.check(built_bundle, _manifest_stub(recs))
    assert result.ok is True, result.detail
    assert "re-discharged" in result.detail


def test_fail_closed_without_key(built_bundle):
    """No verifier key → the non-trivial signed status is rejected as forged."""
    recs = json.loads((built_bundle / "manifest.json").read_text())["dispatch_records"]
    plugin = RefinementDischargeCheck(
        recheck_key=None, recheck_invoker=InProcessZ3Invoker()
    )
    result = plugin.check(built_bundle, _manifest_stub(recs))
    assert result.ok is False
    assert result.reason_code == "DISCHARGE_STATUS_FORGED"


def test_stripped_signature_rejected(built_bundle):
    key = VerifierSigningKey.from_secret_bytes(_DEMO_KEY.encode("utf-8"))
    recs = copy.deepcopy(
        json.loads((built_bundle / "manifest.json").read_text())["dispatch_records"]
    )
    del recs[0]["proof"]["verifier_signature"]
    plugin = RefinementDischargeCheck(
        recheck_key=key, recheck_invoker=InProcessZ3Invoker()
    )
    result = plugin.check(built_bundle, _manifest_stub(recs))
    assert result.ok is False
    assert result.reason_code == "DISCHARGE_STATUS_FORGED"


def test_obligation_digest_tamper_rejected(built_bundle):
    key = VerifierSigningKey.from_secret_bytes(_DEMO_KEY.encode("utf-8"))
    recs = copy.deepcopy(
        json.loads((built_bundle / "manifest.json").read_text())["dispatch_records"]
    )
    recs[0]["proof"]["obligation_sha"] = "b" * 64
    plugin = RefinementDischargeCheck(
        recheck_key=key, recheck_invoker=InProcessZ3Invoker()
    )
    result = plugin.check(built_bundle, _manifest_stub(recs))
    assert result.ok is False
    assert result.reason_code == "PROOF_OBLIGATION_SHA_MISMATCH"


def test_verifier_vs_claim_divergence_retains_signed_record(built_bundle):
    """The claimed advance: producer signs 'discharged' over a context whose
    total is wrong; the verifier's own Z3 re-run finds FAILED, contradicting the
    claim. The verdict fails closed AND a signed divergence record is retained
    on the verdict face and re-verifies (retain-and-still-reject)."""
    key = VerifierSigningKey.from_secret_bytes(_DEMO_KEY.encode("utf-8"))
    recs = copy.deepcopy(
        json.loads((built_bundle / "manifest.json").read_text())["dispatch_records"]
    )
    rec = recs[0]
    rec["proof"]["discharge_status"] = "not-attempted"
    rec["proof"].pop("verifier_signature", None)
    rec["proof"]["recheck_context"] = {**rec["proof"]["recheck_context"], "total": 9999}
    diverged = sign_and_write(
        rec,
        key=key,
        discharge_status="discharged",
        z3_status="discharged",
        bundle_id=_BUNDLE_ID,
        record_idx=0,
    )

    with tempfile.TemporaryDirectory() as td:
        tmp_bundle = Path(td) / "bundle"
        shutil.copytree(built_bundle, tmp_bundle)
        plugin = RefinementDischargeCheck(
            recheck_key=key, recheck_invoker=InProcessZ3Invoker()
        )
        result = plugin.check(tmp_bundle, _manifest_stub([diverged]))
        assert result.ok is False
        assert result.reason_code == "DISCHARGE_STATUS_VERIFIER_DIVERGENCE"

        # Read-only verify() invariant: the verifier-signed divergence record is
        # retained on the verdict face (PluginResult.disclosures), NOT appended to
        # the bundle (writing events.jsonl into the bundle would flip a re-verify
        # GREEN→RED as UNOWNED surplus). retain-and-still-reject persists.
        assert not (tmp_bundle / "events.jsonl").exists()
        disclosures = [
            d for d in result.disclosures if "DISCHARGE_STATUS_VERIFIER_DIVERGENCE" in d
        ]
        assert len(disclosures) == 1, (
            f"divergence record was not retained: {result.disclosures!r}"
        )
        det = json.loads(disclosures[0].split(" — ", 1)[1])
        assert det["record_kind"] == DIVERGENCE_RECORD_KIND
        assert det["producer_claimed"] == "discharged"
        assert det["verifier_computed"] == "failed"
        assert verify_divergence_record(
            det, key=key, bundle_id=_BUNDLE_ID, record_idx=0
        )


# ---------------------------------------------------------------------------
# FIX-B planted violations (claim-set coverage sweep): payload/attribution.json
# was never opened by any plugin — a producer could report attributions that
# differ from the numbers Z3 discharged. The binding leg closes it.
# ---------------------------------------------------------------------------

import hashlib

from AttributionBindingCheck import AttributionBindingCheck  # noqa: E402


def _rewrite_attribution(bundle_dir: Path, mutate) -> None:
    """Strongest producer: rewrite payload/attribution.json AND re-sha
    manifest.files so file-integrity stays green — only the binding leg can
    catch the divergence."""
    p = bundle_dir / "payload" / "attribution.json"
    attribution = json.loads(p.read_bytes())
    mutate(attribution)
    new_bytes = json.dumps(attribution, indent=2).encode("utf-8")
    p.write_bytes(new_bytes)
    mp = bundle_dir / "manifest.json"
    m = json.loads(mp.read_bytes())
    m["files"]["payload/attribution.json"] = hashlib.sha256(new_bytes).hexdigest()
    mp.write_bytes(json.dumps(m, indent=2, sort_keys=True).encode("utf-8"))


def _full_chain(built_bundle, monkeypatch):
    monkeypatch.setenv("VKERNEL_VERIFIER_HMAC_KEY", _DEMO_KEY)
    plugins = [
        FileIntegrityManySmall(),
        SpecShaPinCheck(),
        *default_post_w3_plugin_set(),
        AttributionBindingCheck(),
    ]
    return BundleVerifier(plugins=plugins).verify(built_bundle)


def _codes_blob(result) -> str:
    return " || ".join(
        f"[{f.check_name}] {f.reason_code}: {f.detail}" for f in result.failures
    )


def test_binding_leg_passes_honest_bundle(built_bundle, monkeypatch):
    result = _full_chain(built_bundle, monkeypatch)
    assert result.ok is True, _codes_blob(result)


def test_sum_preserving_relabel_fails(built_bundle, monkeypatch):
    """The headline FIX-B attack: shift 1000 between two edges so the SUM (the
    only property Z3 proves) still holds, re-sha file-integrity green — the
    C16 leg stays satisfied and only the binding leg refutes the relabel."""

    def shift(attribution):
        attribution["edge_attribution"]["e0"] += 1000
        attribution["edge_attribution"]["e1"] -= 1000

    _rewrite_attribution(built_bundle, shift)
    result = _full_chain(built_bundle, monkeypatch)
    assert result.ok is False
    assert "RD_ATTRIBUTION_CONTEXT_MISMATCH" in _codes_blob(result), _codes_blob(
        result
    )


def test_source_hash_relabel_fails(built_bundle, monkeypatch):
    def relabel_source(attribution):
        attribution["source_sha256"] = "00" * 32

    _rewrite_attribution(built_bundle, relabel_source)
    result = _full_chain(built_bundle, monkeypatch)
    assert result.ok is False
    assert "RD_ATTRIBUTION_SOURCE_UNBOUND" in _codes_blob(result), _codes_blob(result)


def test_smuggled_attribution_key_fails(built_bundle, monkeypatch):
    def smuggle(attribution):
        attribution["methodology_note"] = "trust us"

    _rewrite_attribution(built_bundle, smuggle)
    result = _full_chain(built_bundle, monkeypatch)
    assert result.ok is False
    assert "RD_ATTRIBUTION_MALFORMED" in _codes_blob(result), _codes_blob(result)


def test_context_vs_table_divergence_caught_by_binding_leg(built_bundle):
    """Direct-plugin leg: a recheck_context that diverges from the pinned
    allocation table is refuted by the table binding (independent of the C16
    HMAC, which would ALSO fail — the grounding is not signature-only)."""
    mp = built_bundle / "manifest.json"
    m = json.loads(mp.read_bytes())
    ctx = m["dispatch_records"][0]["proof"]["recheck_context"]
    ctx["e0"] += 500
    ctx["e1"] -= 500  # sum preserved; Z3 would still discharge
    mp.write_bytes(json.dumps(m, indent=2, sort_keys=True).encode("utf-8"))
    res = AttributionBindingCheck().check(built_bundle, None)
    assert res.ok is False
    # payload↔context divergence surfaces first (context no longer matches the
    # honest payload); both codes witness the same planted divergence.
    assert res.detail.startswith(
        ("RD_ATTRIBUTION_CONTEXT_MISMATCH", "RD_ATTRIBUTION_TABLE_MISMATCH")
    ), res
