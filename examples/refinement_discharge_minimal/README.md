...
- **Attribution binding (FIX-B, claim-set coverage sweep).** The C16 leg
  proves the sum invariant under `proof.recheck_context` but never opened
  `payload/attribution.json` — a sum-preserving relabel of the per-edge
  amounts verified green. `AttributionBindingCheck` (pilot-level, wired in
  `verify.py`; the bare-CLI discharge path stays substrate-only) closes that:
  strict parse + exact-keys/exact-edge-set closure
  (`RD_ATTRIBUTION_MALFORMED`), payload values must equal the re-discharged
  context (`RD_ATTRIBUTION_CONTEXT_MISMATCH`), the context must equal the
  pinned allocation table (`RD_ATTRIBUTION_TABLE_MISMATCH`), and
  `source_sha256` must hash the actual `spec/allocation_table.json` bytes
