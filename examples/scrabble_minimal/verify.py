"""verify.py — scrabble_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  ScrabbleReDerivationCheck       §C6 — Scrabble adjudication re-derivation (local pack)
  DispatchRecordWellformedCheck   §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"EDITION_RESOLVE", "MEMBERSHIP_LOOKUP", "COMPUTE"})
     admits the two new domain-specific kinds introduced by this pilot)

Usage:
    python examples/scrabble_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/scrabble_minimal/verify.py -> parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# ScrabbleReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from ScrabbleReDerivationCheck import ScrabbleReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scrabble-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
