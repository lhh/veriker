...
Re-derivation:
  ...
  4. Bind the DELIVERED payload/segmentation.nii.gz bytes to the manifest's
     committed sha256 — sha256(payload/segmentation.nii.gz on disk) must equal
     manifest.payload.segmentation_sha256. This is the artifact-substitution
     check: file_integrity_many_small only confirms the delivered file matches
     its OWN entry in manifest.files (self-consistent — a producer can re-align
     that entry to whatever bytes it ships), so without this step nothing ties
     the actually-delivered NIfTI to the sidecar hash the rest of this script
     re-derives against. Exit 1 with [TOTALSEGMENTATOR_REDER_FAIL]
     TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH on drift or absence.
  5. Copy source/ tree → scratch dir; run inference with full determinism
     incantation → rederive.nii.gz.
  6. Compute sha256(rederive) and compare to committed sha256 (additive to
     step 4 — this leg re-derives the segmentation from source rather than
     merely checking bytes-on-disk, and is kept even though step 4 alone
     already closes the substitution gap).
  7. Exit 0 on match; exit 1 with [TOTALSEGMENTATOR_REDER_FAIL]
     RE_DERIVATION_MISMATCH on drift.
...
def _verify(bundle_dir: Path) -> str | None:
    ...
    for pkg in _PINNED_TOOLS:
        ...
        if pinned in (None, "missing"):
            return f"TOTALSEGMENTATOR_TOOLCHAIN_MISMATCH: spec missing pinned version for {pkg!r}"
    ...
    if not pinned_chk_sha:
        return (
            "TOTALSEGMENTATOR_TOOLCHAIN_MISMATCH: spec.model.checkpoint_sha256 absent"
        )
    ...
    # ---- 4b. bind the DELIVERED payload file to the committed sha ----
    # file_integrity_many_small only asserts payload/segmentation.nii.gz matches
    # its OWN entry in manifest.files — self-consistency a producer can satisfy
    # by re-aligning that entry to whatever bytes it ships. Nothing else ties
    # the file actually delivered in this bundle to
    # manifest.payload.segmentation_sha256, the value the re-derived inference
    # below is checked against. A producer could ship the honest sha in that
    # field while substituting a different NIfTI as payload/segmentation.nii.gz
    # and sail through both the file-integrity walk and (if the substitute
    # happens to be the producer's own honest output) the re-derivation compare
    # — the PASS would then certify a segmentation that isn't the delivered
    # artifact. Close that gap by hashing the delivered bytes directly.
    if not seg_path.exists():
        return (
            "TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH: payload/segmentation.nii.gz "
            "absent from bundle_dir — cannot bind delivered payload to "
            "manifest.payload.segmentation_sha256"
        )
    delivered_sha = _sha256_file(seg_path)
    if delivered_sha != committed_sha:
        return (
            f"TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH: delivered "
            f"payload/segmentation.nii.gz sha256={delivered_sha[:16]!r}... does "
            f"not match manifest.payload.segmentation_sha256="
            f"{committed_sha[:16]!r}... — the file shipped in this bundle is not "
            f"the segmentation the manifest commits to"
        )
    ...
    if rederive_sha != committed_sha:
        return (
            f"RE_DERIVATION_MISMATCH: re-derived sha "
