"""verify.py — fp_ml_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  FpMlReDerivationCheck      §C6 — FP ML inference re-derivation (float32 tolerance-bounded)

Usage:
    python examples/fp_ml_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/fp_ml_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# FpMlReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from FpMlReDerivationCheck import FpMlReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="fp_ml_minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
