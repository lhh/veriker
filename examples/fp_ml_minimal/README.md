...
- **Tamper 1 (exceeds ε):** Mutate weight W[0][0] by +1e-3 → logit delta ~0.587,
  which is 5.87×10^8 × ε.  Verification FAILS with RE_DERIVATION_MISMATCH.
...
`payload/predictions.json` is a JSON array, and array position alone is not
a claim of identity. The checker (`fp_ml_re_derivation.py`) re-derives
`bundled_predictions[i]` against `feature_vectors[i]` for every `i`, but
before trusting a record's logits it first asserts
`bundled_predictions[i]["input_idx"] == i` (fail-closed if the field is
absent — `FP_ML_INPUT_IDX_MISSING`; mismatched — `FP_ML_INPUT_IDX_MISMATCH`).

Without that check, a producer could permute the `input_idx` values across
records — attributing one input's inference result to a different input's
identity — without touching any `logits` or `predicted_class` byte, and a
checker that trusts array position alone would still report full
re-derivation success. See the **Tamper-3** test
(`test_fp_ml_minimal_tamper_input_idx_swap_fails`) in
`tests/test_fp_ml_minimal.py`.
...
tests/
  test_fp_ml_minimal.py     Round-trip + tamper-exceeds-ε + tamper-within-ε +
                             tamper-input_idx-swap (identity binding) tests
```
...
# Run the full test suite (4 tests)
...
# → FAIL: [fp_ml_re_derivation] RE_DERIVATION_MISMATCH: ...delta=...
```
