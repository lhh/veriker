...
def build(out_dir: Path) -> None:
    ...
    # --- Build model.json ---
    model_bytes = (json.dumps(model, indent=2, ensure_ascii=False) + "\n").encode(
        "utf-8"
    )
    ...
    content = BundleContent(
        ...
        # predictions.json must be read-and-compared by a wired check or
        # excused with a committed reason — a field nothing reads fails the
        # verdict closed instead of riding a green PASS. All three fields
        # (input_idx identity, logits within ε, predicted_class exact) are
        # bound by fp_ml_re_derivation, so no residuals are declared.
        extra_manifest_fields={
            "claimset": {"claim_files": {"predictions": "payload/predictions.json"}}
        },
    )
    ...
    print(
        f"  inputs           : {_N_INPUTS} feature vectors of dim {_N_FEATURES} (float32)"
    )
