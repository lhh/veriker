"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for fp_ml.

Per-dir migration of the fp_ml pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + FpMlReDerivationCheck.py +
fp_ml_re_derivation.py) and its verify.py / test are untouched, and no committed
...
What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/fp_ml.spec.json).
  - The verifier re-derives the representative output (the float32 scalar logit
    predictions[0].logits[0]) IN-PROCESS via the registered primitive, and
    compares with the generic scalar_epsilon comparator. No subprocess, no
...
Usage:
    python examples/fp_ml_minimal/spec_pinned_check.py
...
# Legacy builder (lays down inputs/weights/payload/manifest) + the shared
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "fp_ml_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from fp_ml_recompute import compute_rep_logit  # noqa: E402

_OUTPUT_ID = "fp_ml_logit"
_TYPE_KEY = "fp_ml_logit"
_SPEC_SRC = _HERE / "spec_pinned" / "fp_ml.spec.json"
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned fp_ml bundle in out_dir. Reuses the legacy builder for
    inputs/weights/payload, then overlays the beta shape: an auditor spec under
    ...
    model = json.loads((out_dir / "weights" / "model.json").read_bytes())
    features = json.loads((out_dir / "inputs" / "features.json").read_bytes())
    claimed = compute_rep_logit(model["W"], model["b"], features)
    ...
    # This overlay demonstrates spec-pinned dispatch on the outputs/ claim and
    # deliberately does NOT wire the predictions re-derivation plugin, so it
    # does not read predictions.json's fields. The legacy builder emits the
    # pilot's claim-field coverage declaration; carrying it on this lane would
    # be a false coverage story (could-not-conclude at best), so it is dropped
    # here with the reason in place. The declared-claimset path is exercised
    # by tests/test_fp_ml_minimal.
...
def make_verifier(anchor=None):
    ...
    from fp_ml_recompute import FpMlRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the fp_ml exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  fp_ml  (spec-pinned dispatch)")
            return 0
        print("FAIL  fp_ml  (spec-pinned dispatch)", file=sys.stderr)
