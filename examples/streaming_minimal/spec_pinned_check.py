"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for streaming_minimal.

Per-dir migration of the streaming_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + StreamingReDerivationCheck.py +
streaming_re_derivation.py) and its verify.py / test are untouched, and no committed
manifest gains an `outputs` array — the spec-pinned bundle is built to a fresh
directory here, so the substrate's "0 committed manifests declare outputs"
inertness invariant is preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/streaming.spec.json).
  - The verifier re-derives the representative output (the per-window aggregate
    list from payload/checkpoint.json) IN-PROCESS via the registered primitive,
    by replaying events/stream.jsonl through the committed windowing spec, and
...
Note on typed_checks: the legacy builder writes a manifest whose typed_checks are
["spec_sha_pin", "file_integrity_many_small", "streaming_re_derivation"]. The
spec-pinned verifier runs FileIntegrityManySmall + step-5 dispatch only, so this
overlay REWRITES typed_checks to ["file_integrity_many_small"] (the verifier
rejects any typed_checks name without a registered plugin). The auditor binding
spec lives under spec/streaming.spec.json (FileIntegrityManySmall Pass-3 skips
spec/), recorded by basename in manifest.spec_files.

Usage:
    python examples/streaming_minimal/spec_pinned_check.py
...
# Legacy builder (lays down events/spec/payload/manifest) + the shared canonical
# compute function (imported standalone — no audit_bundle needed).
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location(
        "streaming_minimal__legacy_build_bundle", _HERE / "_build_bundle.py"
    )
...
from streaming_recompute import compute_window_aggregates, _parse_stream  # noqa: E402
from audit_bundle.admission import admit_jsonl_file  # noqa: E402

_OUTPUT_ID = "streaming_window_aggregates"
_TYPE_KEY = "streaming_window_aggregates"
_SPEC_SRC = _HERE / "spec_pinned" / "streaming.spec.json"
...
def _honest_aggregates(out_dir: Path) -> list[dict]:
    """Auditor-side canonical recompute of the honest per-window aggregate list
    from the built bundle's committed event stream + windowing spec."""
    events = _parse_stream(admit_jsonl_file(out_dir / "events" / "stream.jsonl"))
    spec = json.loads((out_dir / "spec" / "segmentation.json").read_bytes())
    return compute_window_aggregates(events, spec)
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned streaming bundle in out_dir. Reuses the legacy builder
    for events/spec/payload, then overlays the beta shape: an auditor spec under
    spec/, a producer claimed-value file under outputs/, a manifest.outputs entry,
    and a typed_checks set matching the spec-pinned verifier's plugin set.
    ...
    claimed = _honest_aggregates(out_dir)
    ...
    # The legacy builder set spec_sha_pin + streaming_re_derivation here — drop them.
    manifest["typed_checks"] = ["file_integrity_many_small"]
    # The claimset declaration does not hold on this lane: the promoted
    # primitive re-derives the categorical checkpoint on the outputs/ claim,
    # and StreamingReDerivationCheck (the check that reads and compares
    # payload/checkpoint.json's fields) is deliberately not wired here.
    # Carrying the declaration would be a false coverage story, so the
    # variant drops it and the verdict carries the honest "claimset: not
    # declared" disclosure. The declared pilot path is exercised by
    # tests/test_streaming_minimal.py.
    manifest.pop("claimset", None)
...
def make_verifier(anchor=None):
    ...
    from streaming_recompute import StreamingRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the streaming_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  streaming_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  streaming_minimal  (spec-pinned dispatch)", file=sys.stderr)
