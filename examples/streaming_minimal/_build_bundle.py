...
def build(out_dir: Path) -> None:
    ...
    content = BundleContent(
        ...
        extra_manifest_fields={
            ...
            # payload/checkpoint.json must be read-and-compared by a wired
            # check or excused with a committed reason. All four fields
            # (window_start_ms, window_end_ms, aggregate, event_count) are
            # bound: streaming_re_derivation.py replays events/stream.jsonl
            # through the committed windowing spec and compares every field
            # of every window row positionally against the bundled
            # checkpoint before returning success — there is no residual.
            "claimset": {
                "claim_files": {"checkpoint": "payload/checkpoint.json"},
            },
        },
    )
