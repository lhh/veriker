...
claim fields it compared, keyed by bundle-relative file:
    ...
    [COMPARED] {"payload/checkpoint.json": ["[].aggregate", "[].event_count", ...]}

The set is accumulated as each field comparison passes (never a constant) and
emitted only from the success return — a removed or skipped comparison drops
out of the line. The pilot's plugin reports claim-field coverage for exactly
those fields; coverage follows what this pack says it compared. Field paths
use the claimset rendering (object keys joined by ".", every array position
"[]"); `payload/checkpoint.json` is a bare array at the root, so every path
here starts with "[]" (no wrapping key).
...
def _verify(bundle_dir: Path) -> str | None:
    ...
    # Compare each window on all four fields, accumulating the claim-field
    # paths whose values were actually compared on this run — never a
    # constant; a comparison that is removed or skipped drops out of the
    # emitted [COMPARED] line and the claimset gate stops reporting that
    # field as covered. Positional (zip, not id-indexed), so a duplicated or
    # reordered row is caught by the length check above, not by an identity
    # lookup that could shadow a row.
    ...
    for idx, (derived, bundled) in enumerate(zip(derived_windows, bundled_windows)):
        for key in ("window_start_ms", "window_end_ms", "aggregate", "event_count"):
            ...
            # Compared as canonical JSON, not Python equality: `false` is not
            # `0`, `-300.0` is not `-300`, `6e4` is not `60000` — a relying
            # party typing the payload must see the value the replay produced.
            if json.dumps(d_val, sort_keys=True) != json.dumps(b_val, sort_keys=True):
                return (
...
def _emit_compared(compared: "set[str]") -> None:
    ...
    machine-readable stdout line, after every comparison succeeded. Never a
    constant; no producer-controlled string is interpolated. Paths use the
    claimset rendering."""
    print(
        "[COMPARED] "
        + json.dumps({"payload/checkpoint.json": sorted(compared)}, sort_keys=True)
    )
