"""spec_pinned_check.py — self-contained Axis-2 spec-pinned dispatch for dp_minimal.

Per-dir migration of the dp_minimal pilot onto the disclosed spec-pinned,
auditor-anchored, in-process recompute-then-compare method (S0). This is ADDITIVE:
the legacy bundle (_build_bundle.py + DpReDerivationCheck.py + dp_re_derivation.py)
and its verify.py / test are untouched, and no committed manifest gains an
`outputs` array — the spec-pinned bundle is built to a fresh directory here, so
the substrate's "0 committed manifests declare outputs" inertness invariant is
preserved.

What it demonstrates:
  - The auditor pins the binding (type -> primitive_id + comparator) in a
    SHA-anchored spec (spec_pinned/dp.spec.json).
  - The verifier re-derives the representative output (the DP noised_count) IN
    PROCESS via the registered primitive — re-counting true_count from the
    committed dataset and re-drawing the Laplace noise from the committed seed —
    and compares with the generic scalar_epsilon comparator. No subprocess, no
...
Usage:
    python examples/dp_minimal/spec_pinned_check.py
...
# Legacy builder (lays down data/payload/manifest) + the shared canonical compute
...
def _load_legacy_build():
    _s = _ilu.spec_from_file_location("dp__legacy_build_bundle", _HERE / "_build_bundle.py")
    _m = _ilu.module_from_spec(_s)
    _s.loader.exec_module(_m)
    return _m.build

_build_legacy_bundle = _load_legacy_build()
from dp_recompute import compute_noised_count  # noqa: E402

_OUTPUT_ID = "dp_noised_count"
_TYPE_KEY = "dp_noised_count"
_SPEC_SRC = _HERE / "spec_pinned" / "dp.spec.json"
...
def _load_rows(bundle_dir: Path) -> list:
    return [
        json.loads(line)
        for line in (bundle_dir / "data" / "dataset.jsonl")
...
def build_spec_pinned(
    ...
    *,
    ...
    """Build a spec-pinned dp_minimal bundle in out_dir. Reuses the legacy builder
    for data/payload, then overlays the β shape: an auditor spec under spec/, a
    ...
    claimed = compute_noised_count(rows, release)
...
def make_verifier(anchor=None):
    ...
    from dp_recompute import DpRecompute  # noqa: PLC0415
...
def main() -> int:
    argparse.ArgumentParser(
        description="Build + spec-pinned-verify the dp_minimal exemplar"
    ).parse_args()
    with tempfile.TemporaryDirectory() as td:
        ...
        if result.ok:
            print("PASS  dp_minimal  (spec-pinned dispatch)")
            return 0
        print("FAIL  dp_minimal  (spec-pinned dispatch)", file=sys.stderr)
