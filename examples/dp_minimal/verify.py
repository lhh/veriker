"""verify.py — dp_minimal domain pilot bundle verifier.
...
Registers two plugins:
  ...
  DpReDerivationCheck      §C6 — DP-release re-derivation (stochastic-but-seed-pinned)

Usage:
    python examples/dp_minimal/verify.py --bundle-dir <path>
...
# Layout: examples/dp_minimal/verify.py → parents[2] = pkg root.
_PKG_ROOT = Path(__file__).resolve().parents[2]
if str(_PKG_ROOT) not in sys.path:
    sys.path.insert(0, str(_PKG_ROOT))

# DpReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from DpReDerivationCheck import DpReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="dp_minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
