"""verify.py — pharmacophore_fit_minimal domain pilot bundle verifier.
...
Registers three plugins:
  ...
  PharmacophoreFitReDerivationCheck     §C6  — spatial-fit RMSD re-derivation (local pack)
  DispatchRecordWellformedCheck         §C15 — op-kind + effect well-formedness
    (op_kinds_admitted=frozenset({"PHARMACOPHORE_FIT","COMPUTE"}) admits the new
     domain-specific PHARMACOPHORE_FIT kind introduced by this pilot)

Usage:
    python examples/pharmacophore_fit_minimal/verify.py --bundle-dir <path>
...
# PharmacophoreFitReDerivationCheck lives alongside this script (AB4 — duplicate-don't-import).
...
from PharmacophoreFitReDerivationCheck import PharmacophoreFitReDerivationCheck


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pharmacophore-fit-minimal audit bundle verifier (AUDIT_BUNDLE_CONTRACT §C5)"
    )
    parser.add_argument(
        "--bundle-dir",
        required=True,
    ...
    plugins = [
        ...
        PharmacophoreFitReDerivationCheck(),
        DispatchRecordWellformedCheck(
            op_kinds_admitted=frozenset({"PHARMACOPHORE_FIT", "COMPUTE"})
        ),
        StampLatticeCheck(),
    ]
