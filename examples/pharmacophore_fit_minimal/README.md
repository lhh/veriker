...
`RE_DERIVATION_MISMATCH`. You cannot adjust an alignment
...
first) or `RE_DERIVATION_MISMATCH` (if manifest SHA was
...
`RE_DERIVATION_MISMATCH` (advanced-set mismatch). You cannot
...
## Axis-2 spec-pinned surface (greenfield, additive)

Alongside the legacy full-ledger TypedCheck above, this pilot carries a
SEPARATE, greenfield Axis-2 spec-pinned dispatch surface
(RATIONAL_BAND_MIGRATION.md §4b.3). It is purely additive: the legacy
`_build_bundle.py` / `verify.py` / `PharmacophoreFitReDerivationCheck` and the
product-root `tests/test_pharmacophore_fit_minimal.py` above are UNTOUCHED. No
committed manifest in this pilot dir declares `outputs` — the spec-pinned
bundle is built to a fresh, self-contained temp directory by
`spec_pinned_check.py`.

This pilot is the **second consumer family of the `rational_sqrt_band`
comparator kind** (the first is the FEA pilot's witness+certificate
posture) — both distances and RMSD are square roots of exact rationals, so
the recompute side never touches float arithmetic.

**Representative-output choice** (stated in full in
`spec_pinned/pharmacophore_fit.spec.json`'s description): rather than bind
every per-candidate / per-pair distance as a spec-pinned claim — the legacy
full ledger above already covers that, in float — this surface commits to
exactly two claims, both drawn from the **best-fit (rank-1, smallest
mean-squared-distance) candidate**:

| output_id                          | type                                | primitive_id                                   | comparator          | epsilon |
|-------------------------------------|--------------------------------------|-------------------------------------------------|----------------------|---------|
| `pharmacophore_best_fit_rmsd`       | `pharmacophore_best_fit_rmsd`       | `pharmacophore_best_fit_rmsd_recompute`         | `rational_sqrt_band` | 1e-6    |
| `pharmacophore_best_fit_distance`   | `pharmacophore_best_fit_distance`   | `pharmacophore_best_fit_distance_recompute`     | `rational_sqrt_band` | 1e-6    |

`pharmacophore_best_fit_distance` is that same candidate's paired distance for
its alphabetically-first `pharmacophore_feature_id` (tie-break identical to
the legacy pipeline's `sorted(feature_mapping.keys())` ordering).

Both `pharmacophore_recompute.py` primitives independently re-derive the
best-fit candidate (argmin over exact `Fraction` mean-squared distances — sqrt
is monotone, so no sqrt is needed to find the ranking) rather than trusting
which candidate the producer calls "best"; the claimed values themselves come
from the SAME float `run_spatial_fit` pipeline the legacy check replays,
never from the exact-rational recompute path.

Because both types share an identical comparator (kind + epsilon), an auditor
**work-set** is wired (`audit_bundle.work_set.WorkSet`, `output_id -> required
type`, `SELF_AUTHORED`) in `spec_pinned_check.py` so a producer cannot rebind
an output_id's declared type to its sibling — monotone-strictness alone (keyed
on primitive_id sharing) would not catch that swap — and cannot drop, add, or
duplicate a claim either (`WORK_SET_VIOLATION`: the set is an exact multiset
bijection over `manifest.outputs`).

Run the self-contained build+verify:

```bash
python examples/pharmacophore_fit_minimal/spec_pinned_check.py
```

Expected stdout: `PASS  pharmacophore_fit_minimal  (spec-pinned dispatch)`.

Tests: `tests/test_pharmacophore_fit_spec_pinned.py` — honest pass; tampered
RMSD claim; tampered distance claim; tampered input; no-anchor fail-closed;
weak-spec (same-kind) substitution fail-closed; role-policy rebinding
fail-closed; exact epsilon-band edge.
...
├── verify.py                                 # runs all three TypedCheck plugins (legacy full-ledger surface)
...
├── pharmacophore_recompute.py                 # Axis-2 recompute primitives (rational_sqrt_band, greenfield)
├── spec_pinned/
│   └── pharmacophore_fit.spec.json            # auditor-anchored binding spec (2 types)
├── spec_pinned_check.py                       # self-contained Axis-2 build+verify harness
├── pilot.json                                # pilot v1 metadata
└── README.md

Pilot pytest lives at PRODUCT-ROOT:
tests/test_pharmacophore_fit_minimal.py         # legacy full-ledger happy-path + tamper tests
tests/test_pharmacophore_fit_spec_pinned.py     # Axis-2 spec-pinned dispatch tests (this surface)
```
