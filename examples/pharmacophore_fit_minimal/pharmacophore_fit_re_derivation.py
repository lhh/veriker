...
def _run_spatial_fit(
    ...
    for rank_1indexed, (rmsd, cid, _) in enumerate(survivors, start=1):
        ranked_list.append({"compound_id": cid, "rmsd": rmsd, "rank": rank_1indexed})
...
# numeric_model: binary64_exact (RATIONAL_BAND_MIGRATION.md §6.3 vocabulary;
# this legacy pack carries it in-comment because its comparison is
# hand-rolled, not registry-dispatched). The only transcendental in this
# pack's path is math.sqrt, which is IEEE-correctly-rounded and therefore
# deterministic pure-arithmetic binary64 replay — not a libm-wobble surface
# like math.exp/math.log. eps=1e-9 covers float representation noise on
# values already rounded to 6dp (a no-op grain), not cross-platform
# transcendental drift. This legacy full-ledger check is untouched by the
# greenfield `rational_sqrt_band` spec-pinned surface added alongside it
# (spec_pinned_check.py) — see RATIONAL_BAND_MIGRATION.md §4b.3.
...
def main() -> int:
    ...
    for label, val in [
        ...
        if val is None:
            print(
                f"[RE_DERIVATION_MISMATCH] {label}: file absent",
                file=sys.stderr,
            )
            return 1
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] payload/inputs malformed: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(
            f"[RE_DERIVATION_MISMATCH] re-derivation raised "
            f"exception: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if rederived["candidate_count"] != bundled_candidate_count:
        print(
            f"[RE_DERIVATION_MISMATCH] candidate_count mismatch: "
    ...
    if rederived["scored_count"] != bundled_scored_count:
        print(
            f"[RE_DERIVATION_MISMATCH] scored_count mismatch: "
    ...
    if rederived["advanced_count"] != bundled_advanced_count:
        print(
            f"[RE_DERIVATION_MISMATCH] advanced_count mismatch: "
    ...
    if len(rederived["fits"]) != len(bundled_ledger):
        print(
            f"[RE_DERIVATION_MISMATCH] ledger length mismatch: "
    ...
    for i, (rd, bd) in enumerate(zip(rederived["fits"], bundled_ledger)):
        if rd["compound_id"] != bd.get("compound_id"):
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].compound_id: "
        ...
        if rd["paired_count"] != bd.get("paired_count"):
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].paired_count: "
        ...
        if not _approx_equal(rd["rmsd"], bd.get("rmsd")):
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].rmsd: "
        ...
        if len(rd["pair_records"]) != len(bundled_pairs):
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].pair_records "
        ...
        if bd.get("rank") != expected_rank:
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].rank: "
        ...
        if bd.get("advanced") != expected_advanced:
            print(
                f"[RE_DERIVATION_MISMATCH] ledger[{i}].advanced: "
    ...
    if len(rederived["ranked"]) != len(bundled_ranked):
        print(
            f"[RE_DERIVATION_MISMATCH] ranked length mismatch: "
    ...
    for i, (rr, br) in enumerate(zip(rederived["ranked"], bundled_ranked)):
        if rr["compound_id"] != br.get("compound_id") or rr["rank"] != br.get("rank"):
            print(
                f"[RE_DERIVATION_MISMATCH] ranked[{i}]: "
        ...
        if not _approx_equal(rr["rmsd"], br.get("rmsd")):
            print(
                f"[RE_DERIVATION_MISMATCH] ranked[{i}].rmsd: "
    ...
    if bundled_advanced_cids != rederived_advanced_cids_sorted:
        only_rederived = sorted(
            set(rederived_advanced_cids_sorted) - set(bundled_advanced_cids)
        )
        only_bundled = sorted(
            set(bundled_advanced_cids) - set(rederived_advanced_cids_sorted)
        )
        print(
            f"[RE_DERIVATION_MISMATCH] advanced set mismatch:\n"
