...
Test flow:
  ...
  4. PRIMARY TAMPER: mutate one voxel-region in source/phantom.nii.gz;
     ...
     assert RE_DERIVATION_MISMATCH because the re-derived
     segmentation sha differs from the committed sha.
  5. PAYLOAD-SWAP TAMPER: substitute different bytes as
     payload/segmentation.nii.gz; re-align its SHA in manifest.files so
     FileIntegrityManySmall passes (self-consistent) while leaving
     manifest.payload.segmentation_sha256 honest; assert
     TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH because the delivered file's
     bytes no longer match the manifest-committed segmentation sha.
  6. SPEC TAMPER: append whitespace to spec/tooling.json without realigning
...
def test_tamper_input_ct_fails_rederivation(tmp_path: Path) -> None:
    ...
    assert result.ok is False, "expected ok=False after mutating source/phantom.nii.gz"
...
def test_tamper_payload_swap_fails_binding(tmp_path: Path) -> None:
    """Substituting different bytes as payload/segmentation.nii.gz — with its
    manifest.files entry re-aligned to the substitute (self-consistent) but
    manifest.payload.segmentation_sha256 left honest — must trigger
    TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH.

    This is the strongest producer shape: file_integrity_many_small passes
    because the substitute matches its OWN manifest.files entry, and
    manifest.payload.segmentation_sha256 is left untouched (the honest
    inference output's hash), so a re-derivation that reproduces the honest
    hash would otherwise PASS even though a different file was delivered.
    Nothing but the payload-binding check ties the DELIVERED bytes to that
    committed sha256.
    """
    bundle_dir = tmp_path / "ts_bundle_payload_swap"
    build(bundle_dir)

    seg_path = bundle_dir / "payload" / "segmentation.nii.gz"
    original_bytes = seg_path.read_bytes()
    substitute_bytes = b"NOT-THE-DELIVERED-SEGMENTATION" + original_bytes[:64]
    assert substitute_bytes != original_bytes
    seg_path.write_bytes(substitute_bytes)

    # Re-align manifest.files to the substitute (self-consistent) but leave
    # manifest.payload.segmentation_sha256 honest.
    manifest_path = bundle_dir / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["files"]["payload/segmentation.nii.gz"] = _sha256_file(seg_path)
    manifest_path.write_text(
        json.dumps(manifest, indent=2, sort_keys=True), encoding="utf-8"
    )
    ...
    assert result.ok is False, (
        "expected ok=False after substituting payload/segmentation.nii.gz"
    )
    combined = " ".join(f.reason_code + " " + f.detail for f in result.failures).upper()
    assert "TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH" in combined, (
        f"expected TOTALSEGMENTATOR_PAYLOAD_SHA_MISMATCH in failures; "
        f"got: {result.failures}"
    )
