...
aggregate) shape is PROMOTED into the shippable core registry (PRIMITIVES.md).
...
Surfaces:
  ...
  2. Tampered claimed value (add large offset, well outside epsilon) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed input (flip predicate match in dataset) -> RE_DERIVATION_MISMATCH.
  ...
  5. Just-over-epsilon perturbation (delta = epsilon * 1.5) -> RE_DERIVATION_MISMATCH.
...
def test_promoted_just_over_epsilon_perturbation_fails(tmp_path):
    """A claimed value perturbed by more than epsilon MUST fail with RE_DERIVATION_MISMATCH.
