...
Mirrors test_climate_emission_minimal.py — tests covering shape + tamper
discipline for the data-center NSR + embodied-carbon pilot.

Tests:
  1. test_clean_bundle_passes                        — happy-path build + verify.
  2. test_clean_bundle_shape                          — 2 sources + 5 materials,
                                                         every criteria pollutant
                                                         has positive total tpy,
                                                         overall classification
                                                         == synthetic_minor,
                                                         OpaqueFragment anchors
                                                         well-formed (7 total).
  3. test_tamper_emission_sources_sha_fails           — mutate
                                                         emission_sources.json
                                                         without realigning
                                                         manifest SHA →
                                                         file_integrity catches.
  4. test_tamper_payload_total_fails                  — mutate NOx total_tpy +
                                                         realign manifest SHA →
                                                         re-derivation pack
                                                         catches.
  5. test_tamper_displayed_threshold_more_lenient_fails — FIX-E: inflate the
                                                         displayed NOx
                                                         major_source_threshold_tpy
                                                         (a more lenient
                                                         fabricated ceiling)
                                                         while leaving total_tpy
                                                         / classification
                                                         unchanged + realign
                                                         manifest SHA →
                                                         threshold-binding catches
                                                         (DCMRV_THRESHOLD_MISMATCH).
  6. test_tamper_displayed_threshold_missing_field_fails — strip the displayed
                                                         PM
                                                         synthetic_minor_threshold_tpy
                                                         field + realign manifest
                                                         SHA → threshold-binding
                                                         fails closed on the
                                                         missing field
                                                         (DCMRV_THRESHOLD_MISMATCH).
  7. test_tamper_citation_strip_fails                 — blank a citation_url +
                                                         realign manifest SHA →
                                                         re-derivation pack
                                                         catches CITATION_MISSING.
"""
    ...
    return BundleVerifier(
        plugins=[
            ...
            ReDerivationInvocationCheck(
                pack_filename="dc_emissions_mrv_pack.py", permit_execution=True
            ),
            DCEmissionsMRVReDerivationCheck(),
        ]
    )
...
def test_tamper_displayed_threshold_more_lenient_fails(tmp_path: Path) -> None:
    """FIX-E: mutate criteria_pollutants.NOx.major_source_threshold_tpy to a more
    LENIENT (higher) ceiling than inputs/permit_thresholds.json actually commits,
    leaving total_tpy and classification untouched, then realign manifest SHA.

    This is the strongest producer attack the finding names: a classification is
    stable across a range of threshold values, so a submission can display a
    fabricated (more lenient) regulatory ceiling next to an otherwise-correct
    classification and neither total_tpy nor classification comparisons catch
    it. file_integrity passes (SHA realigned); the re-derivation pack must now
    bind the displayed threshold to the committed input and fail closed."""
    bundle_dir = _build_clean(tmp_path)

    submission_path = bundle_dir / "payload" / "nsr_submission.json"
    submission = json.loads(submission_path.read_text(encoding="utf-8"))
    nox = submission["criteria_pollutants"]["NOx"]
    original_majsrc = float(nox["major_source_threshold_tpy"])
    original_total = float(nox["total_tpy"])
    original_classification = nox["classification"]
    # Inflate the displayed major-source ceiling tenfold — a fabricated, more
    # lenient limit. total_tpy stays well under BOTH the real and the fake
    # ceiling, so classification is unaffected: the mismatch is on the
    # displayed threshold value alone.
    nox["major_source_threshold_tpy"] = round(original_majsrc * 10.0, 6)
    assert nox["classification"] == original_classification
    assert float(nox["total_tpy"]) == original_total
    ...
    result = verifier.verify(bundle_dir)
    assert result.ok is False, (
        "expected ok=False after inflating the displayed NOx "
        "major_source_threshold_tpy without changing total_tpy/classification"
    )
    combined = " ".join(f.reason_code + " " + f.detail for f in result.failures).upper()
    assert "DCMRV_THRESHOLD_MISMATCH" in combined, (
        f"expected DCMRV_THRESHOLD_MISMATCH; got: {result.failures}"
    )


def test_tamper_displayed_threshold_missing_field_fails(tmp_path: Path) -> None:
    """Fail-closed leg: strip the displayed synthetic_minor_threshold_tpy field
    from one pollutant entirely (rather than inflating it) AND realign manifest
    SHA. The threshold-binding check must fail closed on the missing field, not
    silently pass because there is nothing to compare against."""
    bundle_dir = _build_clean(tmp_path)

    submission_path = bundle_dir / "payload" / "nsr_submission.json"
    submission = json.loads(submission_path.read_text(encoding="utf-8"))
    del submission["criteria_pollutants"]["PM"]["synthetic_minor_threshold_tpy"]
    ...
    result = verifier.verify(bundle_dir)
    assert result.ok is False, (
        "expected ok=False after stripping PM synthetic_minor_threshold_tpy"
    )
    combined = " ".join(f.reason_code + " " + f.detail for f in result.failures).upper()
    assert "DCMRV_THRESHOLD_MISMATCH" in combined, (
        f"expected DCMRV_THRESHOLD_MISMATCH; got: {result.failures}"
