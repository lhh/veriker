...
Test flow:
  ...
  4. Tamper test: mutate data/dataset.jsonl (flip one row's age_bucket so the
     ...
     RE_DERIVATION_MISMATCH (or BAD_FILE_SHA from file_integrity) in failures.
"""
...
def test_dp_minimal_tamper_dataset_fails(tmp_path: Path) -> None:
    """Mutating dataset.jsonl must cause RE_DERIVATION_MISMATCH (or BAD_FILE_SHA)."""
    ...
    assert "RE_DERIVATION_MISMATCH" in failure_codes or "BAD_FILE_SHA" in failure_codes, (
        f"Expected RE_DERIVATION_MISMATCH or BAD_FILE_SHA in failures; got: "
