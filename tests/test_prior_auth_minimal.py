...
# ---------------------------------------------------------------------------
# Tamper A: mutate clinical_finding content → RE_DERIVATION_MISMATCH
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tamper C: flip provider_role without recomputing HMAC →
#           PRIOR_AUTH_PROVIDER_ATTESTATION_INVALID (FIX-C regression test)
#
# CONFIRMED FINDING (claim-set coverage sweep): decision_provenance.jsonl
# carries provider_role (e.g. medical_director vs. attending_physician) but
# it used to be excluded from the attestation HMAC and had no other copy
# anywhere in the bundle — forgeable for free. This test plants exactly that
# violation against the strongest available producer (a fully honest bundle,
# manifest SHA re-aligned so FileIntegrityManySmall cannot mask the failure)
# and asserts the binding catches it.
# ---------------------------------------------------------------------------


def test_prior_auth_minimal_tamper_provider_role_fails_attestation(
    tmp_path: Path,
) -> None:
    """Flipping provider_role in a provenance row without recomputing its HMAC must
    cause PRIOR_AUTH_PROVIDER_ATTESTATION_INVALID even when the manifest SHA is
    re-aligned. Regression test for FIX-C: provider_role must be bound into the
    attestation HMAC, not forgeable independently of the rest of the record."""
    bundle_dir = tmp_path / "prior_auth_tampered_c"
    bundle_dir.mkdir()
    _build_bundle_mod.build(bundle_dir)

    # Find a row with provider_role='attending_physician' and relabel it as
    # 'medical_director' — the direction CO Reg 10-1-1 §5.A.5 cares about,
    # since it changes who is on record as the accountable decision-maker.
    prov_path = bundle_dir / "payload" / "decision_provenance.jsonl"
    lines = prov_path.read_text(encoding="utf-8").splitlines()
    ...
    for i, line in enumerate(lines):
        if not line.strip():
            continue
        row = json.loads(line)
        if row.get("provider_role") == "attending_physician":
            row["provider_role"] = "medical_director"  # flip without recomputing HMAC
            lines[i] = json.dumps(row, sort_keys=True)
            flipped = True
            break

    assert flipped, "No 'attending_physician' row found in provenance log to tamper"
    prov_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    # Re-align manifest SHA so FileIntegrityManySmall does NOT catch this first —
    # isolates the assertion to the attestation-HMAC binding, not file integrity.
    _realign_manifest_sha(bundle_dir, "payload/decision_provenance.jsonl")
    ...
    assert result.ok is False, (
        "Expected result.ok=False after flipping provider_role without recomputing HMAC"
    )
    ...
    assert "PROVIDER_ROLE" in combined.upper(), (
        "Expected the provider_role field to be named in the failure detail; "
