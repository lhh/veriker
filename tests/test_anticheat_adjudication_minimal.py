...
Test flow:
  ...
  6. Re-run verifier — assert result.ok is False with RE_DERIVATION_MISMATCH.
...
# ---------------------------------------------------------------------------
# Tamper A: mutate detection_signal content → RE_DERIVATION_MISMATCH
# ---------------------------------------------------------------------------
...
def test_anticheat_minimal_tamper_signal_fails_rederivation(tmp_path: Path) -> None:
    ...
    RE_DERIVATION_MISMATCH even when the manifest SHA is re-aligned.
...
# ---------------------------------------------------------------------------
# FIX-C — confirmed defect 1: payload/ban_decisions.json.final_verdict was
# NEVER READ by the checker at all. Flip it directly (everything else
# honest — evidence, policy, model_recommendation, matched_rule_id, and the
# entire adjudication_provenance.jsonl / HMAC chain are untouched) and
# re-align the manifest SHA. Pre-fix this passed silently; post-fix it must
# fail because final_verdict is now bound to the re-derived
# model_recommendation.
# ---------------------------------------------------------------------------


def test_anticheat_minimal_tamper_ban_decisions_final_verdict_fails(
    tmp_path: Path,
) -> None:
    """Flipping payload/ban_decisions.json final_verdict (e.g. ban -> no-ban)
    with everything else honest must cause ANTICHEAT_FINAL_VERDICT_MISMATCH
    (surfaced under the RE_DERIVATION_MISMATCH reason_code family),
    even when the manifest SHA is re-aligned and adjudication_provenance.jsonl
    / its HMACs are left completely untouched."""
    bundle_dir = tmp_path / "anticheat_tampered_final_verdict"
    bundle_dir.mkdir()
    _build_bundle_mod.build(bundle_dir)
    ...
    for decision in decisions:
        if decision["final_verdict"] == "ban":
            decision["final_verdict"] = "clear"  # flip ban -> no-ban
            flipped = True
            break
    assert flipped, "No 'ban' decision found to tamper"

    decisions_path.write_text(
        json.dumps(decisions, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    # Re-align manifest SHA so FileIntegrityManySmall does not mask the failure.
    _realign_manifest_sha(bundle_dir, "payload/ban_decisions.json")

    verifier = _make_verifier()
    result = verifier.verify(bundle_dir)

    assert result.ok is False, (
        "Expected result.ok=False after flipping ban_decisions.json final_verdict "
        "with everything else honest"
    )

    reason_codes = [f.reason_code for f in result.failures]
    detail_texts = [f.detail for f in result.failures]
    combined = " ".join(reason_codes + detail_texts).upper()
    assert "ANTICHEAT_FINAL_VERDICT_MISMATCH" in combined, (
        f"Expected ANTICHEAT_FINAL_VERDICT_MISMATCH in failures; "
        f"got reason_codes={reason_codes!r}, details={detail_texts!r}"
    )


# ---------------------------------------------------------------------------
# FIX-C — confirmed defect 2: payload/adjudication_provenance.jsonl.adjudicator_type
# was not in the signed HMAC message and never cross-checked. Flip it
# (human <-> automated) WITHOUT recomputing the HMAC. Pre-fix this passed
# silently; post-fix it must fail because adjudicator_type is now part of
# the signed message.
# ---------------------------------------------------------------------------


def test_anticheat_minimal_tamper_adjudicator_type_fails_attestation(
    tmp_path: Path,
) -> None:
    """Flipping adjudicator_type (human -> automated, or vice versa) in a
    provenance row without recomputing its HMAC must cause
    ANTICHEAT_ADJUDICATOR_ATTESTATION_INVALID even when the manifest SHA is
    re-aligned. Before this fix adjudicator_type was excluded from the HMAC
    message and had no other copy in the bundle, so this exact mutation
    passed silently."""
    bundle_dir = tmp_path / "anticheat_tampered_adjudicator_type"
    bundle_dir.mkdir()
    _build_bundle_mod.build(bundle_dir)
    ...
    for i, line in enumerate(lines):
        if not line.strip():
            continue
        row = json.loads(line)
        if row.get("adjudicator_type") == "human":
            row["adjudicator_type"] = "automated"  # relabel; HMAC no longer valid
            lines[i] = json.dumps(row, sort_keys=True)
            flipped = True
            break

    assert flipped, "No 'human' adjudicator_type row found in provenance log to tamper"
    prov_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    assert result.ok is False, (
        "Expected result.ok=False after flipping adjudicator_type without "
        "recomputing its HMAC"
    )
...
# ---------------------------------------------------------------------------
# FIX-C — noted drift: adjudication_provenance.jsonl.model_recommendation is
# DERIVED and must equal the ban_decisions.json field that IS checked. Flip
# the provenance copy only (ban_decisions.json + its final_verdict stay
# honest, HMACs are recomputed over the mutated row so this is NOT an HMAC
# failure) and confirm the dedicated drift check catches it.
# ---------------------------------------------------------------------------


def test_anticheat_minimal_tamper_provenance_model_recommendation_drift(
    tmp_path: Path,
) -> None:
    """Drifting adjudication_provenance.jsonl model_recommendation away from
    the ban_decisions.json value it is derived from (HMAC recomputed over the
    mutated row, so this is a pure consistency gap, not a signature failure)
    must cause ANTICHEAT_MODEL_RECOMMENDATION_DRIFT."""
    bundle_dir = tmp_path / "anticheat_tampered_model_rec_drift"
    bundle_dir.mkdir()
    _build_bundle_mod.build(bundle_dir)
    ...
    key_path = bundle_dir / "payload" / "attestation_key.hex"
    ...
    for i, line in enumerate(lines):
        if not line.strip():
            continue
        row = json.loads(line)
        if row.get("model_recommendation") == "ban":
            row["model_recommendation"] = "clear"  # drift from ban_decisions.json
            drifted = True
            drift_row = row
            drift_idx = i
            break

    assert drifted, "No 'ban' model_recommendation row found in provenance log"

    # Recompute the HMAC over the mutated row so this is isolated to the drift
    # check, not incidentally caught as an HMAC mismatch.
    msg = (
        f"{drift_row['adjudicator_id']}|{drift_row['case_id']}|"
        f"{drift_row['final_verdict']}|{drift_row['attestation_timestamp']}|"
        f"{drift_row['adjudicator_type']}"
    ).encode("utf-8")
    drift_row["attestation_hmac"] = _hmac.new(key, msg, hashlib.sha256).hexdigest()
    lines[drift_idx] = json.dumps(drift_row, sort_keys=True)
    prov_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    assert result.ok is False, (
        "Expected result.ok=False after drifting provenance model_recommendation "
        "away from ban_decisions.json"
    )

    reason_codes = [f.reason_code for f in result.failures]
    detail_texts = [f.detail for f in result.failures]
    combined = " ".join(reason_codes + detail_texts).upper()
    assert "ANTICHEAT_MODEL_RECOMMENDATION_DRIFT" in combined, (
        f"Expected ANTICHEAT_MODEL_RECOMMENDATION_DRIFT in failures; "
        f"got reason_codes={reason_codes!r}, details={detail_texts!r}"
    )
