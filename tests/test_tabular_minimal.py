...
Test flow:
  ...
  4. Tamper test: mutate one row's revenue value in data/sales.csv, re-align
     the manifest SHA so FileIntegrity passes, assert RE_DERIVATION_MISMATCH
...
def test_tamper_sales_revenue_fails(tmp_path: Path) -> None:
    ...
    must trigger RE_DERIVATION_MISMATCH or TABULAR_REDER_FAIL."""
    ...
    assert "TABULAR_REDERIV" in combined or "TABULAR_REDER_FAIL" in combined, (
        f"expected RE_DERIVATION_MISMATCH or TABULAR_REDER_FAIL in failures; "
...
def test_tabular_minimal_claimset_receipt_on_verdict(tmp_path: Path) -> None:
...
def test_tabular_minimal_claimset_ratchet_all_artifacts_flip(tmp_path: Path) -> None:
    ...
    assert {o.element for o in report.flipped} == {'result'}
    for outcome in report.flipped:
        ...
        assert any("TABULAR_REDER_FAIL" in d and "result.csv mismatch" in d for _, d in outcome.reasons), outcome.reasons
