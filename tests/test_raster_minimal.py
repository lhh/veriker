...
def test_tamper_raster_cell_fails(tmp_path: Path) -> None:
    """Mutate a raster cell value, re-align manifest SHA, then assert RE_DERIVATION_MISMATCH.
    ...
    # Accept RE_DERIVATION_MISMATCH (reason_code) or RASTER_REDER_FAIL
    ...
    assert "RASTER_REDERIV" in combined or "RASTER_REDER_FAIL" in combined, (
        f"expected RE_DERIVATION_MISMATCH or RASTER_REDER_FAIL in failures; "
...
def test_tamper_spec_query_fails_spec_sha(tmp_path: Path) -> None:
    ...
    # SPEC_SHA_MISMATCH -- which now reaches reason_code directly, the verifier
    # having stopped overwriting it with plugin_failed on 2026-08-30 -- or the
    # literal "SHA MISMATCH" wording carried in the detail.
    assert (
