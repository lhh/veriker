...
# ---------------------------------------------------------------------------
# Upgrade-predicate re-derivation (by-catch, 2026-07-21): the record-0
# predicate was asserted at BUILD time only — no runtime reader existed, so a
# tampered extraction value rode through every leg while the record still
# displayed the predicate string. UpgradePredicateCheck closes that.
# ---------------------------------------------------------------------------


def _manifest_of(bundle: Path):
    from audit_bundle.bundle_manifest import BundleManifest
...
def test_upgrade_predicate_rederives_true_on_honest_bundle(tmp_path):
    sys.path.insert(0, str(_PKG_ROOT))
    from examples.provenance_upgrade_minimal.predicate_check import (
        UpgradePredicateCheck,
    )

    bundle = _build(tmp_path)
    result = UpgradePredicateCheck().check(bundle, _manifest_of(bundle))
    assert result.ok, result.detail
    assert "1 predicate(s)" in result.detail


def test_tampered_extraction_value_refutes_predicate(tmp_path):
    """The upgrade signature covers the record body, not the payload bytes —
    only the predicate re-derivation refuses this tamper."""
    sys.path.insert(0, str(_PKG_ROOT))
    from examples.provenance_upgrade_minimal.predicate_check import (
        UpgradePredicateCheck,
    )
    ...
    bundle = _build(tmp_path)
    xp = bundle / "payload" / "extractions.json"
    ...
    assert result.reason_code == "UPGRADE_PREDICATE_REFUTED", result.detail


def test_unknown_predicate_on_upgraded_record_fails_closed(tmp_path):
    sys.path.insert(0, str(_PKG_ROOT))
    from examples.provenance_upgrade_minimal.predicate_check import (
        UpgradePredicateCheck,
    )
    ...
    m["dispatch_records"][0]["predicates"] = ["totally_novel_predicate()"]
    mp.write_bytes(json.dumps(m, indent=2).encode("utf-8"))

    result = UpgradePredicateCheck().check(bundle, _manifest_of(bundle))
    assert not result.ok
    assert result.reason_code == "UPGRADE_PREDICATE_UNKNOWN", result.detail
