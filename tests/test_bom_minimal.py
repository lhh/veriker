...
Test flow:
  ...
  6. Assert result.ok is False with RE_DERIVATION_MISMATCH in failures.
"""
...
def test_tamper_lockfile_hash_fails(tmp_path: Path) -> None:
    """Mutating one package's hash in lockfile.json must trigger RE_DERIVATION_MISMATCH."""
    ...
    # The bom_re_derivation plugin failure wraps its reason_code ("RE_DERIVATION_MISMATCH")
    ...
    assert "BOM_REDERIV" in combined or "BOM_REDER_FAIL" in combined, (
        f"expected RE_DERIVATION_MISMATCH or BOM_REDER_FAIL in failures; got: {result.failures}"
    )
