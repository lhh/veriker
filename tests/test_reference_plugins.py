...
class TestReDerivationInvocationCheck:
    ...
    # -----------------------------------------------------------------------
    # No pack → COULD NOT CONCLUDE (not "opted out", and not a pass)
    #
    # These asserted `ok is True` until 2026-08-31. An absent pack means the
    # verifier checked nothing, and the sibling branch one level down (pack
    # PRESENT but not executed) had already reported incomplete=True since the
    # ADV-01 verdict-laundering fix. Absent evidence is strictly WEAKER than
    # present-but-unexecuted, so reporting OK for it while reporting incomplete
    # for the stronger case was backwards.
    # -----------------------------------------------------------------------
