...
decision list` shape is PROMOTED into the shippable core registry (PRIMITIVES.md,
...
Surfaces:
  ...
  3. Tampered committed input rule (drop a rule's required diagnosis so a denied
     request now first-matches that rule) -> RE_DERIVATION_MISMATCH.
...
def test_promoted_tampered_input_rule_fails(tmp_path):
    ...
    # the honest producer claim -> RE_DERIVATION_MISMATCH.
