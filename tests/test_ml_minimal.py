...
Test flow:
  ...
  4. Tamper test: mutate W[0][0] in weights/model.json from 3 to 2 (a
     ...
     the verifier returns ok=False with RE_DERIVATION_MISMATCH in failures
...
def test_ml_minimal_tamper_weight_fails(tmp_path: Path) -> None:
    """Mutating W[0][0] (3→2) must cause RE_DERIVATION_MISMATCH.
    ...
    assert "RE_DERIVATION_MISMATCH" in failure_codes or "ML_REDER_FAIL" in failure_codes, (
        "Expected RE_DERIVATION_MISMATCH or ML_REDER_FAIL in failures; got: "
