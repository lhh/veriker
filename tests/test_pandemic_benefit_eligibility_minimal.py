...
Gate 3 of the v-kernel-pilot four-gate criteria. Tests:
  ...
  period_week forgery              — period_week shifted and RE-SIGNED with the disclosed key;
                                     verifier binds it to the applicant's attested
                                     eligibility_period_week [PERIOD_WEEK_MISMATCH]
  disbursement_system_id forgery   — system id relabelled and RE-SIGNED with the disclosed
                                     key; verifier checks it against a verifier-held roster
                                     [UNAUTHORIZED_DISBURSEMENT_SYSTEM]
"""
...
_pack = _load(
    "pandemic_eligibility_pack", _PILOT_DIR / "pandemic_eligibility_rederivation.py"
)
...
def _load_decisions(bundle_dir: Path) -> list:
    return json.loads(
        (bundle_dir / "payload" / "disbursement_decisions.json").read_text()
    )
...
def _write_decisions(bundle_dir: Path, decisions: list) -> None:
    ...
    man["files"]["payload/disbursement_decisions.json"] = hashlib.sha256(
        path.read_bytes()
    ).hexdigest()
...
def test_happy_path(tmp_path):
    ...
    assert (
        verdicts["APP-003"] == _pack.APPROVED
    )  # $39k/yr, self-employed, minimal income
...
def test_ineligible_applicant_approved_is_caught(tmp_path):
    ...
    with RE_DERIVATION_MISMATCH in the failures.
    """
    bundle = tmp_path / "bundle"
    _build.build(bundle)
    key = _key(bundle)
    ...
    assert proc.returncode == 1, (
        "expected FAIL — ineligible applicant should have been caught"
    )
    stderr = proc.stderr.decode()
    assert "RE_DERIVATION_MISMATCH" in stderr, (
        f"expected RE_DERIVATION_MISMATCH in stderr, got: {stderr}"
    )
...
def test_file_sha_tamper_caught(tmp_path):
    ...
    assert (
        b"manifest_sha" in proc.stderr.lower()
        or b"file_integrity" in proc.stderr.lower()
    )
...
def test_signature_tamper_rejected(tmp_path):
    ...
    _write_decisions(
        bundle, decisions
    )  # updates manifest SHA so file-integrity is clean
...
# ---------------------------------------------------------------------------
# Tamper test: period_week forged (re-signed with the disclosed key)
# ---------------------------------------------------------------------------


def test_period_week_forged_is_caught(tmp_path):
    """FIX-E claim-set coverage: period_week shifted and RE-SIGNED with the bundled
    (disclosed-by-design) HMAC key, so the signature alone cannot catch it.

    _build_bundle.py sets period_week = applicant["eligibility_period_week"] — the
    applicant's own attested value is the only committed source for it. The verifier
    must bind period_week to that attested field by exact equality and reject the
    forged, correctly-signed record with PERIOD_WEEK_MISMATCH."""
    bundle = tmp_path / "bundle"
    _build.build(bundle)
    key = _key(bundle)
    ...
    for dec in decisions:
        if dec["applicant_id"] == "APP-001":
            dec["period_week"] = (
                dec["period_week"] + 1
            )  # shift away from the attested value
            dec.pop("signature", None)
            dec["signature"] = _pack.sign_decision(
                dec, key
            )  # re-sign with the disclosed key
    _write_decisions(bundle, decisions)

    proc = _full_verify(bundle)
    assert proc.returncode == 1, (
        "expected FAIL — forged period_week should have been caught"
    )
    stderr = proc.stderr.decode()
    assert "PERIOD_WEEK_MISMATCH" in stderr, (
        f"expected PERIOD_WEEK_MISMATCH in stderr, got: {stderr}"
    )
    assert "APP-001" in stderr


# ---------------------------------------------------------------------------
# Tamper test: disbursement_system_id relabelled (re-signed with the disclosed key)
# ---------------------------------------------------------------------------


def test_disbursement_system_id_forged_is_caught(tmp_path):
    """FIX-E claim-set coverage: disbursement_system_id relabelled to an unprovisioned
    system and RE-SIGNED with the bundled (disclosed-by-design) HMAC key, so the
    signature alone cannot catch it.

    The verifier must reject any disbursement_system_id outside its verifier-held,
    never-read-from-the-bundle roster of provisioned systems, even with a valid
    signature, with UNAUTHORIZED_DISBURSEMENT_SYSTEM."""
    bundle = tmp_path / "bundle"
    _build.build(bundle)
    key = _key(bundle)
    ...
    for dec in decisions:
        if dec["applicant_id"] == "APP-002":
            dec["disbursement_system_id"] = "system://rogue-shadow-disbursement-v9"
            dec.pop("signature", None)
            dec["signature"] = _pack.sign_decision(
                dec, key
            )  # re-sign with the disclosed key
    ...
    proc = _full_verify(bundle)
    assert proc.returncode == 1, (
        "expected FAIL — forged disbursement_system_id should have been caught"
    )
    stderr = proc.stderr.decode()
    assert "UNAUTHORIZED_DISBURSEMENT_SYSTEM" in stderr, (
        f"expected UNAUTHORIZED_DISBURSEMENT_SYSTEM in stderr, got: {stderr}"
    )
    assert "APP-002" in stderr
