...
Surfaces:
  ...
  2. Tampered claimed value (+1.0, far above epsilon) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed input (perturb inputs/features.json[0][0] by +1.0 so the
     re-derived logit shifts well beyond epsilon) -> RE_DERIVATION_MISMATCH.
...
def _build(out_dir: Path, *, claimed_override=None):
    ...
    # compares predictions.json's fields) is deliberately not wired here. The
    # pilot bundle's claim-field coverage declaration therefore does not hold
    # on this lane and is dropped from the variant, so the verdict carries the
    # honest "claimset: not declared" disclosure instead of a false coverage
    # story. The declared pilot path is exercised in test_fp_ml_minimal.
    m.pop("claimset", None)
