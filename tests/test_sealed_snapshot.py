...
def _short_write(fd, data):
    """os.write that transfers at most ONE byte per call — the raw-syscall
    short write (filling filesystem, RLIMIT_FSIZE ceiling, interrupted
    transfer) that returns a partial count instead of raising. Left alone for
    stdout/stderr so pytest's own capture is unaffected."""
    if fd <= 2:
...
def test_short_write_does_not_truncate_the_snapshot_copy(tmp_path, monkeypatch):
    """os.write may transfer fewer bytes than handed to it WITHOUT raising.
    Discarding the count truncates the copy, and the strict-SHA walk then
    reports a verifier-side disk failure as a bundle hash mismatch — a REJECT
    blamed on the artifact. The copy must loop until the chunk is on disk."""
    bundle = tmp_path / "b"
    src_file, payload = _write_snapshot_bundle(bundle)
    snap_root = tmp_path / "snap_short"
    snap_root.mkdir()

    monkeypatch.setattr(os, "write", _short_write)
    materialize_sealed_snapshot(bundle.resolve(), snap_root)
    monkeypatch.undo()

    # Byte-exact, not truncated to the first byte of each chunk.
    assert (snap_root / "snapshots" / "doc.txt").read_bytes() == payload
    assert (snap_root / "manifest.json").read_bytes() == (
        bundle / "manifest.json"
    ).read_bytes()

    # And the end-to-end verdict stays green under short writes, rather than
    # surfacing as a bogus bundle-side REJECT.
    monkeypatch.setattr(os, "write", _short_write)
    verdict = BundleVerifier().verify(bundle)
    monkeypatch.undo()
    assert verdict.ok, [f.reason_code for f in verdict.failures]
    assert src_file.read_bytes() == payload


def test_zero_progress_write_is_a_clean_materialization_error(tmp_path, monkeypatch):
    """A write that reports no progress must raise the destination-side error
    rather than spin forever in the retry loop."""
    bundle = tmp_path / "b"
    _write_snapshot_bundle(bundle)
    snap_root = tmp_path / "snap_stuck"
    snap_root.mkdir()
    ...
    with pytest.raises(
        snapshot_mod.SnapshotMaterializationError, match="no progress"
    ):
        materialize_sealed_snapshot(bundle.resolve(), snap_root)
