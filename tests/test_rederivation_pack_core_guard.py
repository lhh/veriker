...
def _add_redundant_pack(bundle_dir: Path) -> Path:
    """Drop a redundant re-derivation pack into a spec-pinned bundle.

    The boundary under test is: a pack present in a bundle whose re-derivation
    IS carried by spec-pinned dispatch is a REDUNDANT unsafe artifact, not
    present-but-unverified, so the guard must not fire.

    This used to rely on climate_emission_minimal shipping a pack that the
    spec-pinned driver overlayed. That pilot migrated off bundle-supplied
    execution on 2026-08-16 and ships no pack, so the fixture is now
    constructed explicitly — which is also a truer test: the boundary is about
    ANY pack coexisting with dispatch, not about one pilot's layout.
    """
    import hashlib as _hashlib
    import json as _json

    re_derive = bundle_dir / "re_derive"
    re_derive.mkdir(parents=True, exist_ok=True)
    pack = re_derive / "redundant_pack.py"
    pack.write_bytes(
        b"#!/usr/bin/env python3\n"
        b'"""Redundant re-derivation pack fixture. Never executed by these\n'
        b'tests: the point is that dispatch already covers the property."""\n'
        b"import sys\n\n"
    ...
    manifest["files"]["re_derive/redundant_pack.py"] = _hashlib.sha256(
        pack.read_bytes()
    ).hexdigest()
...
def test_pack_with_spec_pinned_dispatch_is_not_flagged(tmp_path: Path) -> None:
    ...
    _add_redundant_pack(bundle_dir)
    assert _bundle_pack_files(bundle_dir), (
        "precondition: the bundle carries a re_derive pack alongside dispatch"
    )
...
def test_anchored_dispatch_bundle_with_pack_converges_to_ok(tmp_path: Path) -> None:
    ...
    bundle_dir = spc._spc.build_spec_pinned(tmp_path / "bundle")
    _add_redundant_pack(bundle_dir)
    assert _bundle_pack_files(bundle_dir), "precondition: bundle carries a pack"
    anchor = spc._spc.anchor_from_committed_spec()
