...
# pycose is a `dev`-extra dependency, not a runtime one. A bare top-level import
# does not skip this module when it is absent -- it INTERRUPTS collection for the
# whole suite, so a run provisioned with the wheel + pytest alone (which is how
# the publish workflow provisions) reports exit 2 having run nothing. Its sibling
# test_m7_crypto_extension_did.py already guards the same import this way.
...
from pycose.keys import OKPKey  # noqa: E402
