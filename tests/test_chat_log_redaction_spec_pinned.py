"""tests/test_chat_log_redaction_spec_pinned.py — Axis-2 spec-pinned dispatch tests
for the per-dir migration of examples/chat_log_redaction_minimal.

Representative re-derived output: redacted_output_sha, the SHA-256 hex digest of
the redacted transcript produced by re-running the bundled regex + entity-dict
PII redaction over inputs/transcript.txt under inputs/redaction_policy.json;
comparator is `exact` (byte-exact hex-string equality).
...
Covers the required surfaces (S0 disclosed-method exit gate):
  ...
  3. Tampered input (mutate inputs/transcript.txt bytes so the redacted output
     sha differs) -> FAIL (RE_DERIVATION_MISMATCH): re-derivation over the mutated
     evidence no longer equals the (honest) claimed sha. Manifest SHA re-aligned
     so FileIntegrity does not fire first.
...
_load("chat_log_redaction_recompute", _PILOT_DIR / "chat_log_redaction_recompute.py")
_spc = _load("chat_log_redaction_spec_pinned_check", _PILOT_DIR / "spec_pinned_check.py")
...
def _honest_sha(bundle_dir: Path) -> str:
    transcript = (bundle_dir / "inputs" / "transcript.txt").read_text(encoding="utf-8")
    policy = json.loads(
        (bundle_dir / "inputs" / "redaction_policy.json").read_text(encoding="utf-8")
    )
    return _spc.compute_redacted_output_sha(transcript, policy)
...
def test_tampered_value_fails(tmp_path):
    # Producer claims a redacted_output_sha with one hex character flipped.
    bundle_dir = _spc.build_spec_pinned(tmp_path / "bundle")
    honest = _honest_sha(bundle_dir)
...
def test_tampered_input_fails(tmp_path):
    # Build honest, then mutate the transcript bytes so the redacted output sha
    # differs. The claimed value (honest sha) no longer matches the re-derivation
    # over the tampered evidence.
    bundle_dir = _spc.build_spec_pinned(tmp_path / "bundle")
    transcript_path = bundle_dir / "inputs" / "transcript.txt"
    new_bytes = transcript_path.read_bytes() + b"[2026-05-10 09:09:00] Eve: my SSN is 111-22-3333\n"
    transcript_path.write_bytes(new_bytes)
    # Confirm the mutation actually changes the re-derived sha (overlap/policy may
    # otherwise absorb a no-op edit).
    policy = json.loads(
        (bundle_dir / "inputs" / "redaction_policy.json").read_text(encoding="utf-8")
    )
    assert _spc.compute_redacted_output_sha(
        new_bytes.decode("utf-8"), policy
    ) != _spc.compute_redacted_output_sha(
        (new_bytes[: -len(b"[2026-05-10 09:09:00] Eve: my SSN is 111-22-3333\n")]).decode(
            "utf-8"
        ),
        policy,
    )
    ...
    m["files"]["inputs/transcript.txt"] = hashlib.sha256(new_bytes).hexdigest()
...
def test_substituted_spec_fails_closed(tmp_path):
    ...
    substituted_spec = json.dumps(
        {
            "spec_id": "chat_log_redaction.v1",
            "description": "PRODUCER-SUBSTITUTED weaker spec (not auditor-anchored).",
            "types": {
                "redacted_output_sha": {
                    "primitive_id": "chat_log_redaction_recompute",
