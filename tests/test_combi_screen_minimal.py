...
Test flow:
  ...
  4. Tamper tests:
     a. Delete a rejected compound from the ledger (the money test) — re-derivation
        ...
        firing RE_DERIVATION_MISMATCH (ledger length mismatch).
...
def test_combi_screen_delete_rejected_compound_fails(tmp_path: Path) -> None:
    ...
    fires RE_DERIVATION_MISMATCH. You cannot silently disappear a
    ...
    assert "RE_DERIVATION_MISMATCH" in combined, (
        "Expected RE_DERIVATION_MISMATCH in failures; got: "
