...
(PRIMITIVES.md, Tier-3 decision-list cluster). Fire-and-collect emits a value for
...
Surfaces:
  ...
  2. Tampered claimed code (flip one icd10_code) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed input rule (raise rule-J18's sym-001 min_severity above the
     ...
     from the honest claim) -> RE_DERIVATION_MISMATCH.
...
def _build(out_dir: Path, *, claimed_override=None):
    ...
    # The anchored spec defines TWO types, and a bundle that exercises one of
    # them leaves the other's rule judging nothing -- which the anchored-type
    # coverage channel (ADR E6) correctly refuses to conclude over. This fixture
    # used to overlay only the codes half; that was an incomplete BUNDLE, not a
    # legitimate partial claim, and the pilot's own `spec_pinned_check.
    # build_spec_pinned` has always overlaid both. Each confidence claim is read
    # straight off the producer's payload/diagnosis.json `confidence` field --
    # the producer's own float pipeline, never the verifier's primitive -- so
    # this stays a producer-faithfulness test and does not become f(x)==f(x).
    for record in json.loads((out_dir / _PRODUCER_CLAIM_REL).read_bytes()):
        apply_overlay(
            out_dir,
            spec_src_path=_SPEC_SRC,
            output_id=f"{_CONFIDENCE_OUTPUT_PREFIX}{record['icd10_code']}",
            type_key=_CONFIDENCE_TYPE_KEY,
            claimed_value=record["confidence"],
        )
...
def _register_confidence_primitive() -> None:
    """Register ONLY the pilot-local confidence primitive.

    The anchored spec defines two types. `healthcare_diagnosis_recompute` is
    PROMOTED into the core distribution — that is this file's thesis, and it is
    still proven generically below, because nothing here registers it.
    `healthcare_diagnosis_confidence_recompute` is NOT promoted: it lives in the
    pilot and the pilot's own `spec_pinned_check.make_verifier()` registers it.

    A bundle claiming only the codes half leaves the confidence rule judging
    nothing, which the anchored-type coverage channel (ADR E6) correctly refuses
    to conclude over. The fix belongs HERE, in the fixture, not in the shipped
    spec: marking the confidence type optional would weaken the spec for the
    pilot's own verifier, which CAN exercise it, in order to accommodate a test
    harness that had simply never overlaid it.
    """
    from audit_bundle.rederivation.registry import register_primitive

    sys.path.insert(0, str(_PILOT_DIR))
    from healthcare_diagnosis_confidence_recompute import (
...
def _verify(bundle_dir: Path, anchor):
    # Near-bare verifier: FileIntegrity + spec-pinned dispatch under the auditor
    # anchor. The CODES primitive is never registered here — it must resolve via
    # the CORE registry alone, which is the promotion this file exists to prove.
    _register_confidence_primitive()
...
def test_promoted_tampered_input_rule_fails(tmp_path):
    ...
    result = _verify(bundle_dir, anchor)
    assert not result.ok
    # EXACT set. It used to be {RE_DERIVATION_MISMATCH} alone, when this fixture
    # declared only the codes output. Now that it declares BOTH anchored types
    # (see _build), the same tamper produces a second, equally true signal:
    # rule-J18 no longer fires, so J18.9 drops from the re-derived candidates
    # while the bundle still carries a `confidence:J18.9` claim, and the
    # confidence recompute has no candidate to recompute for it. An orphaned
    # claim that cannot be recomputed is a RECOMPUTE_ERROR, not a mismatch —
    # the primitive raised rather than disagreeing, which is the distinction
    # classify_pack_failure exists to keep (a crashed checker is not a
    # disagreement). Both codes are consequences of the one tampered input.
