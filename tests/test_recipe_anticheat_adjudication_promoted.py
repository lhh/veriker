...
core registry (PRIMITIVES.md, Tier-3 decision-list cluster, first-match sub-family).
...
Surfaces:
  ...
  2. Tampered claimed verdict (flip one model_recommendation) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed policy rule (flip rule-C verdict ban->review): the detection
     policy is auditor-pinned (§4a.7 pinned_inputs), so this is caught by the pin
     -> PINNED_INPUT_MISMATCH (before recompute — strictly stronger than the old
     recompute-divergence catch).
...
def test_promoted_tampered_input_rule_fails(tmp_path):
    bundle_dir, anchor = _build(tmp_path / "bundle")
    # Flip rule-C-wallhack-prefire's verdict ban -> review in the committed policy.
    # detection_policy.json is now auditor-PINNED (§4a.7 pinned_inputs), so this
    # tamper is caught by the pin BEFORE the recompute runs — strictly earlier and
    # stronger than the old recompute-divergence catch. (Recompute-divergence
    # itself is still covered by test_promoted_tampered_value_fails, which tampers
    # the un-pinned claim, and by the condition-agreement test below.)
