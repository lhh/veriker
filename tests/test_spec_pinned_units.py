...
def test_comparator_kinds_are_the_closed_world_roster():
    """The registry is a CLOSED world: adding a kind must update this roster
    deliberately, in the same commit as the kind + its param validation.
    rational_sqrt_band added 2026-07-21 (witness+certificate posture — exact
    squared-band compare of a claim against sqrt of an exactly-recomputed
    rational, with the committed-certified-interval rule).
    rational_band added 2026-07-21 (RATIONAL_BAND_SCOPE.md — the no-sqrt
    sibling: exact |R − c| <= epsilon on rationals; first consumer is the
    u_norm_inf witness-certificate claim)."""
...
def test_unanchored_spec_is_not_authoritative(tmp_path):
    ...
    # The anchor here is NON-empty (it lists a different spec), so this stays
    # the artifact-side AnchorViolation — the empty-anchor case is verifier
    # incapacity and is tested separately below.
    sha = _write_spec(
        tmp_path / "spec",
        "a.spec.json",
        {
    ...
    other_anchor = SpecAnchor(allowed={"other.v1": "00" * 32})
    from audit_bundle.rederivation.spec_binding import AnchorViolation

    with pytest.raises(AnchorViolation):
        build_anchored_spec_set(tmp_path, manifest, other_anchor)


def test_empty_anchor_is_not_supplied_not_a_violation(tmp_path):
    # R3 (CLI-port audit, closed by the auditor-entry ADR): an anchor allowing
    # NOTHING is the no-anchor absence wearing a constructor call. It used to
    # land on AnchorViolation — a verifier with no authority blaming the
    # artifact. Now it is AnchorNotSupplied (could-not-conclude), same as
    # anchor=None, and never a vacuous pass.
    sha = _write_spec(
        tmp_path / "spec",
        "a.spec.json",
        {
    ...
    manifest = _M({"a.spec.json": sha})
    from audit_bundle.rederivation.spec_binding import AnchorNotSupplied

    with pytest.raises(AnchorNotSupplied):
        build_anchored_spec_set(tmp_path, manifest, SpecAnchor(allowed={}))
    with pytest.raises(AnchorNotSupplied):
        build_anchored_spec_set(tmp_path, manifest, None)
