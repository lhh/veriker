...
Test flow:
  ...
  4. Tamper test: zero out 1600 bytes (800 int16 samples) spanning the second
     ...
     SHA in the manifest, and assert RE_DERIVATION_MISMATCH appears in
...
def test_tamper_voiced_region_fails(tmp_path: Path) -> None:
    """Zeroing the second voiced region must trigger RE_DERIVATION_MISMATCH.
    ...
    # Accept RE_DERIVATION_MISMATCH reason_code or [AUDIO_REDER_FAIL] in detail
    combined = " ".join(
        f.reason_code + " " + f.detail for f in result.failures
    ).upper()
    assert "AUDIO_REDERIV" in combined or "AUDIO_REDER_FAIL" in combined, (
        f"expected RE_DERIVATION_MISMATCH or AUDIO_REDER_FAIL in failures; "
