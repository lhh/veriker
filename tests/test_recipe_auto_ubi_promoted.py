...
(PRIMITIVES.md). Family D is an AGGREGATION shape, NOT a decision-list: trips are
...
Surfaces:
  ...
  2. Tampered claimed tier (flip PHD-001's tier) -> RE_DERIVATION_MISMATCH.
...
def _build(out_dir: Path, *, claimed_override=None):
    ...
    # claim only — the pilot's re-derivation plugin (the check that reads and
    # compares payload/rating_decisions.json's and payload/rate_table.json's
    # fields) is deliberately not wired here. The pilot bundle's claim-field
    # coverage declaration therefore does not hold on this lane and is
    # dropped from the variant, so the verdict carries the honest "claimset:
    # not declared" disclosure instead of a false coverage story. The
    # declared pilot path is exercised in test_auto_ubi_minimal.
    m.pop("claimset", None)
