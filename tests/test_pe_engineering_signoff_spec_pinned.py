...
Covers the required surfaces (S0 disclosed-method exit gate):
  ...
  2. Tampered claimed value (a REORDERED verdict list) -> FAIL (RE_DERIVATION_MISMATCH).
  3. Tampered input (mutate a load so a verdict flips fail->pass; re-align the manifest
     SHA so FileIntegrity does not fire first) -> FAIL (RE_DERIVATION_MISMATCH).
  4. No auditor anchor while the bundle declares outputs -> could-not-conclude (AnchorNotSupplied -> VERIFIER_INCOMPLETE, clean-ERROR; never a REJECT).
