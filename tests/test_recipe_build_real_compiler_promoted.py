...
digest` shape is PROMOTED into the shippable core registry (PRIMITIVES.md).
...
Surfaces:
  ...
  2. Tampered claimed value (flip one hex char) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed input (append a comment to sources/mod_a.py so the
     ...
     DIFFERENT .pyc sha) -> RE_DERIVATION_MISMATCH. manifest.files for the source
...
def _build(out_dir: Path, *, claimed_override=None):
    ...
    # This variant bundle demonstrates the PROMOTED PRIMITIVE on the outputs/
    # claim only — the pilot's legacy re-derivation plugin (the check that
    # opens and byte-compares the declared opaque artifact) is deliberately
    # not wired here. The pilot bundle's claim-field coverage declaration
    # therefore does not hold on this lane and is dropped from the variant, so
    # the verdict carries the honest "claimset: not declared" disclosure
    # instead of a false coverage story. The declared pilot path is exercised
    # in the pilot's own test module.
    m.pop("claimset", None)
