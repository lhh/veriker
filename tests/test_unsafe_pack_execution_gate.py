...
def test_no_pack_still_short_circuits_regardless_of_permit(tmp_path: Path) -> None:
    # No pack present → NO_PACK either way; nothing is executed. Since
    # 2026-08-31 that is reported as could-not-conclude rather than OK: the
    # short-circuit is about what was NOT run, so it cannot be a pass.
...
# NOTE: this fixture was examples/climate_emission_minimal until 2026-08-16,
# when that pilot migrated to Axis-2 spec-pinned dispatch and stopped
# shipping a re_derive/ pack. The gate under test is SUBSTRATE behaviour and
# still needs a pack-bearing bundle, so it now points at a pilot that has
# one. When the last pack pilot migrates, these tests should assert that no
# bundle-supplied execution path remains at all.
_CLIMATE_BUNDLE = _PKG_ROOT / "examples" / "healthcare_diagnosis_minimal"
...
@pytest.mark.skipif(not _CLIMATE_BUNDLE.is_dir(), reason="pack-bearing example bundle absent")
