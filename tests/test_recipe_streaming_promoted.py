...
Surfaces:
  ...
  3. Tampered committed input (mutate one event's value so its window aggregate
     differs) -> RE_DERIVATION_MISMATCH (manifest SHA re-aligned so FileIntegrity
     does not fire first).
...
def _build(out_dir: Path, *, claimed_override=None):
    ...
    # compares checkpoint.json's fields) is deliberately not wired here. The
    ...
    # story. The declared pilot path is exercised in test_streaming_minimal.
    m.pop("claimset", None)
    mp.write_bytes(json.dumps(m, indent=2).encode("utf-8"))
