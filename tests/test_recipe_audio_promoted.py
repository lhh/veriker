...
boundary pairs) is PROMOTED into the shippable core registry (PRIMITIVES.md).
...
Surfaces:
  ...
  2. Tampered claimed value (shift one boundary pair) -> RE_DERIVATION_MISMATCH.
  3. Tampered committed input (zero out audio/samples.bin so every frame is below
     threshold → the VAD re-derives an EMPTY boundary set) -> RE_DERIVATION_MISMATCH.
