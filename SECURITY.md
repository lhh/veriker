...
| DSSE revocation check (verify step 4): `revocation.is_revoked` | caller-injected `verifier_now` (required; the CLI's `--dsse-now`, defaulting to the wall clock and printed on the face; refused if more than 300 s BEHIND the wall clock — a backdated clock un-revokes keys and un-stales lists — and the revocation root's expiry is graded against the SAME instant, so one run has one clock) | **yes**: verdict records `(revocation_list_hash, verifier_now)` | `RevocationVerdict` |
...
| C19 cross-host edge timeliness (RADI-bounded inequality, ack bounds) | no verifier clock: bundle-carried timestamp evidence only (shape-checked at v0.3; strict mode crypto-verifies against pinned Roughtime roots, and since 2026-09-02 grades ONLY the signed SREP MIDP/RADI — the unsigned `midp_ms`/`radi_ms` envelope copy is optional and, if present, must agree: `ROUGHTIME_SREP_ENVELOPE_MISMATCH`) | **yes**: pure function of bundle bytes + verifier config | in-bundle `timestamp_evidence` |
| C18 verifier supply-chain fetch (`c18_tuf_client.load_bundled_root` pre-checks + `fetch_release_manifest` + the three role-document fetchers — `sigstore-trust-root`, `plugin-allowlist`, `revocation-root` are targets of the SAME TUF chain since 2026-09-02, hash-pinned and payload-typed exactly, never a local file read; the ≤24h timestamp / ≤7d snapshot / ≤90d root windows are enforced on the fetched metadata, not only declared; the `sigstore-trust-root` and `revocation-root` documents carry their OWN 2-of-3 Ed25519 quorum, verified by the strict fetchers AFTER the chain — the chain authenticates bytes under the targets key, threshold 1, and the quorum is what makes a pin the ceremony's; `plugin-allowlist` is unsigned and says so on its envelope) | host wall clock: both our freshness pre-checks **and** python-tuf's authoritative protocol checks downstream | **no, by design**: a live network feed refresh is not a verdict surface and is not replayable | TUF `trust_dir` persists last-seen metadata versions (rollback/freeze state) |
...
- **Structural / hash mismatches DO reject.** The structural verifier (`audit_bundle/extensions/c18_verifier_identity.py`) emits `VERIFIER_IDENTITY_FIELD_MISSING`, `VERIFIER_IDENTITY_OCI_DIGEST_MALFORMED`, `VERIFIER_IDENTITY_REKOR_INCLUSION_PROOF_MALFORMED`, and `VERIFIER_IDENTITY_RELEASE_MANIFEST_MISMATCH`, plus `VERIFIER_IDENTITY_BLOCK_MALFORMED` for a present-but-unparseable block (row 20: never read as a legacy bundle — in the extension, the tripwire plugin, AND the stdlib CLI's own copy since 2026-09-02, whose auto-enable gate previously skipped the check entirely on a malformed block). `VERIFIER_IDENTITY_RELEASE_MANIFEST_MISMATCH` is a **hash recomputation** of the bundled `release_manifest.json`, fail-closed if unreadable. These are structural facts about the bundle, not self-reports, so they compose into the verdict as failures.
- **The in-bundle `rekor_inclusion_proof` field is shape-checked only at v0.3**: the C18 structural verifier checks its shape (no Merkle-inclusion fold, no STH consistency, no freshness), so a shape-valid proof *carried in the bundle* is **not** by itself evidence of log inclusion. Real, cryptographic Rekor inclusion verification now exists host-side via `host_digest_verify --rekor-bundle` (`veriker/cli/host_digest_verify.py`): it re-derives the entry's RFC 6962 inclusion proof to bind the logged leaf to the checkpoint root **and** verifies the log checkpoint's signature against the Rekor log key **resolved from the `sigstore-trust-root` TUF role** (since 2026-09-02: the role document pins the key's digest and type, the key bytes are a target of the same repository at `sigstore-trust-root/keys/<entry>`, hash-pinned by the chain AND required to hash to the role's declared digest; one operator-selected entry per run, `--rekor-log-key-target`, default `rekor.pub` = the production Rekor v1 P-256 log, a v2 Ed25519 log under its own entry; an unfetchable role, an unpinned entry, a digest or type disagreement is `REKOR_LOG_KEY_UNRESOLVED` / `_DIGEST_MISMATCH` / `_TYPE_MISMATCH`, exit 7 — never a fallback to the former compile-time constant), fail-closed (exit 7) — **and, since 2026-09-02, requires the logged entry to commit to the release**: the entry body is decoded and the digest(s) it commits to are derived by kind (`hashedrekord` data digest; `dsse` / `intoto` through the bundle's `dsseEnvelope.payload`, whose sha256 must equal the logged `payloadHash`, then the in-toto subject digests), and one of them must equal the TUF-pinned release image digest. Without that third leg the first two proved only "these bytes are in the public log", which any genuine entry satisfies — measured: a stranger's production entry verified our release. Binding failures carry their own `REKOR_*` codes, the face names what was bound and via which path, and the signing identity in the entry is explicitly NOT checked there. Ed25519 log keys (tile-backed Rekor v2, c2sp signed-note hint) are accepted beside P-256. Consumer-side `cosign verify-blob` against the public Rekor log remains the other path (Step 1 of the 4-step flow documented at `receipts.vkernel.dev`). **Which release file satisfies that binding (since 2026-09-02):** the release job emits `v-kernel-audit-bundle-<release>.image-binding.sigstore-bundle.json`, a `cosign attest-blob --new-bundle-format` over the OCI image MANIFEST bytes; the image digest is sha256 of those bytes, so the logged in-toto subject IS the image digest. Hand that file to `--rekor-bundle`. The per-artifact `sign-blob` bundles (SBOM, provenance, SCITT statement) commit to their own files' digests and exit 7 here by design; before the image-binding file existed no release artifact could satisfy the leg. What is **not yet live** is a signed release to point either at: the C18 TUF roots are still synthetic and no hardware-signed release has been cut, so the documented page carries its own "not yet live" banner; the end-to-end flow becomes runnable at the C18 key ceremony + first signed release. Do not present the in-bundle shape check as "transparency-log inclusion verified": that property comes from the host-side `--rekor-bundle` verification (or cosign), never from the bundle field alone.
- **C19 single-org HMAC keys come from the verifier's pinned policy, never from the edge.** The registered `cross_host_peerreview_authenticator` check derives `K_send` / `K_ack` from `CrossOrgKeyPolicy.pinned_hmac_ikm[key_id]` with the per-role HKDF label (the same derivation as `verify_cross_host_edge_authenticator`). Until 2026-09-02 its single-org arm read the MAC key out of the edge it was verifying (`_test_only_K_send_hex` / `_test_only_K_ack_hex`), an identity check for anyone holding the bundle bytes; an edge that still carries key material is refused outright (`CROSS_HOST_KEY_MATERIAL_IN_BUNDLE`), and no policy / unpinned `key_id` fails closed.
- **The DSSE sealing lane runs from the open-drop CLI on auditor-held material (since 2026-09-02).** `veriker/cli/verify.py --dsse-allowlist <kid→pubkey JSON> --dsse-revocation-list <signed list> --dsse-revocation-root <role document>` (or `--dsse-revocation-root-tuf <trust dir>` to fetch the role through the TUF chain) builds the `DsseVerifyContext` (`audit_bundle/dsse/context.py`) and hands the lane to `verify()`; every path — the three files and the `-tuf` trust DIRECTORY, which the TUF client writes under — is refused inside the bundle under verdict, and every file is read through the admission-bounded loader (a depth-bombed trust file is a typed refusal). The revocation ROOT is a real 2-of-3 role: strict structural validation (every role keyid is the sha256 of its own key — a label is refused, so one key cannot occupy two seats), then its own `signatures[]` verified as Ed25519 over `rfc8785.dumps(signed)` by ≥ `threshold` DISTINCT KEYS, then its `pinned_revocation_list_signer_fingerprint` (an Ed25519 key under `signed.keys`) is the ONLY `root_kid` the resolver admits — the three root keyids sign the role, not lists. Before this the resolver every caller passed was a test lambda, `fetch_revocation_root` had no consumer, and the document's 2-of-3 was presence-checked only (a targets-key-only trust root under a 2-of-3 label). The lane is strict: supplying trust material asks for a sealed bundle, and an unsealed one is refused. Unusable material (inside the bundle, under-signed, expired, unpinned signer, a list not signed by the pinned signer) is `DSSE_CONTEXT_ARG_INVALID`, exit 2 — operator error, never a REJECT of the bundle. Without the flags the stdlib CLI still fails a sealed bundle closed (`DSSE_SIGNATURE_UNCHECKED_NO_CRYPTO`). The verifier clock is ONE instant per run: `--dsse-now` may be pinned at or after now (at most 300 s back), and the root's expiry, the list's window and every `not_after` are graded against it; a backdated clock is `DSSE_CONTEXT_ARG_INVALID`. What the 2-of-3 proves, exactly: that two of the keys THE DOCUMENT LISTS signed it — the key list rides the same bytes as the signatures, so the quorum is the ceremony's approver set made checkable given whoever holds or serves the document (the auditor's file, or the release targets key), not an authority beyond that holder. The `sigstore-trust-root` document (the Rekor log key's pin) has the same signed shape and the same verification at its strict fetcher since 2026-09-02; before that it was flat, unsigned JSON under the targets key alone while the prose called its pin 2-of-3. **`plugin-allowlist` has no consumer in the shipped tree** (nothing under `audit_bundle/` or `cli/` loads plugins from OCI; the fetch envelope says so on the value) — its intended consumer, `c18_plugin_oci_loader.PluginOCILoader`, is a host-side admission step that is a posture decision still to be made.
...
## Release status and the audit commitment

Veriker is **v0.x experimental**, and every verdict face says so
(`release_status: experimental`, stamped from `audit_bundle/_version.py`).
**No production, verified, or 1.0 claim is made before a completed, dated
third-party audit**, scoped to the verifier core (`audit_bundle/verifier.py`,
the manifest parse boundary, spec-pinned dispatch) and the DSSE / COSE / Rekor
consumption paths, **with its findings fixed.** This is a commitment, not an
aspiration: the target date is published here once the engagement is booked,
and the `experimental` label is removed only by that audit plus the signing
ceremony described in the release plan.

**Named credit.** A reporter who demonstrates a silent false PASS (the
verifier printing `OK` on an artifact they tampered, through any input a
producer controls) is credited by name in the advisory and in the changelog
entry that closes it, unless they ask to stay anonymous. That is the class of
finding this project most wants, and it is treated as the highest severity
regardless of how exotic the input is.
