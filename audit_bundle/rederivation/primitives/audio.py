...
core registry (PRIMITIVES.md, shape `audio`). The generic verifier recomputes
...
class AudioRecompute:
    ...
    primitive_shape: str = "audio"
    primitive_version: str = "1"
    primitive_tier: str = "B"
    primitive_contract: str = (
        "boundaries = the set of [start_frame, end_frame] pairs produced by a "
        "frame-energy-threshold VAD over the committed int16 little-endian PCM "
        "bytes in audio/samples.bin under spec/segmentation.json (frame_size, "
        "energy_threshold, min_segment_frames, n_samples): per-frame energy = "
        "sum(s*s), consecutive runs at or above the threshold of length >= "
        "min_segment_frames emit one [start, end) pair."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "A VAD, not THE VAD: a frame-energy threshold is a reference "
        "implementation, not a citable standard.",
        "Only the boundary pair is the auditable claim; segment_id and text are "
        "neither re-derived nor compared.",
        "Bound by the `set` comparator, which is multiplicity-blind. Sound here "
        "only because VAD runs are disjoint and strictly increasing, so a "
        "legitimate boundary set holds no duplicate.",
    )
