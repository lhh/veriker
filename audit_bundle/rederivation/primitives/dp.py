...
(PRIMITIVES.md). The generic verifier recomputes the representative output on
...
class DpRecompute:
    ...
    primitive_shape: str = "differential privacy (Laplace seeded-noise aggregate)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "noised_count = float(true_count) + laplace_noise(scale = "
        "sensitivity/epsilon, seed), where true_count is re-counted from "
        "data/dataset.jsonl under the committed predicate and the Laplace "
        "variate is re-drawn deterministically from random.Random(seed) by "
        "inverse-CDF: noise = -scale * sign(u-0.5) * log(1 - 2*|u-0.5|), u = "
        "Random(seed).random()."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Re-derives that the RELEASED value follows from the committed seed, "
        "epsilon and sensitivity. It does NOT audit the privacy claim: whether "
        "that epsilon is an appropriate budget is outside the primitive.",
        "Bound by scalar_epsilon 1e-6 because math.log is delegated to the "
        "platform libm and is not guaranteed bit-identical across platforms.",
    )
