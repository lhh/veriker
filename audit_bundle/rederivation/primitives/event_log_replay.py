...
Axis-2 value-return form, PROMOTED into the shippable core registry (PRIMITIVES.md,
...
Generic over domain (no per-pilot code):
  ...
  - The CREATE/AMEND/non-mutating-op semantics and the canonical serialization
    ...
    a migration-time decision, and it is why sec_17a4 is NOT bound to this
    primitive).

Domains it reproduces (verified on each pilot's shipped
...
ADOPTED 2026-08-29 by SIX of the seven: ai_act_art12_logging, hipaa_audit_
controls, mica_casp_records, mifid2_order_records, pci_dss_audit_log and
the HR-logging pilot now BIND this primitive_id in their own auditor
specs, so the consolidation exists in shipped artifacts rather than only in the
promoted test. Each pilot keeps its demo-local module as the PRODUCER's copy
(imported by its `_build_bundle.py`) and no longer registers it as a verifier
primitive — before the migration one module served both sides, which made
`claimed == re-derived` an identity. The six are consequently inside the static
disjointness guard, which they were not before.

sec_17a4_records is DELIBERATELY NOT MIGRATED. Re-pointing it here would broaden
its accepted ops to include LOG (see the op-set note above). Its fixture has no
LOG event, so the digests agree either way — but loosening what a records-
retention verifier ACCEPTS is a posture change, not a refactor, and it is left
as an explicit decision rather than a side effect. It keeps
`sec_17a4_replay_recompute` and therefore keeps its producer/verifier sharing.

(Pilots may carry premium trusted-time / retention / cross-org legs on top; those
are separate checks — this open re-derivation is the floor.)
...
class EventLogReplayRecompute:
    ...
    primitive_shape: str = "event-log replay -> reconstructed-record digest"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "Replay the bundle's single append-only JSONL event log "
        "(inputs/<log>.jsonl) from its genesis CREATE event, applying each "
        "subsequent AMEND patch in file order to reconstruct the authoritative "
        "current record; the re-derived value is the SHA-256 digest of that "
        "canonical reconstructed record."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "State fold, not a hash-chain fold: a log whose claim is chain "
        "integrity rather than reconstructed state is a different shape.",
        "Accepted ops are CREATE, AMEND and LOG. A retention regime that must "
        "REFUSE a LOG op needs a stricter op-set than this primitive accepts.",
    )
