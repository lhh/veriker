...
core registry (PRIMITIVES.md, shape `knowledge-graph derivation`). The generic
...
class KgRecompute:
    ...
    primitive_shape: str = "knowledge-graph derivation"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "answer_nodes = the BFS reachability closure of (query.start, "
        "query.predicate, query.max_depth) over the bundled triple set "
        "kg/triples.jsonl, excluding query.start itself, each node visited at "
        "most once."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Single start node, single predicate, bounded depth. Multi-predicate "
        "paths, joins and negation are not this shape.",
