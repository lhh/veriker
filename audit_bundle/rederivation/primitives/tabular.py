...
shippable core registry (PRIMITIVES.md, shape `tabular`). The generic verifier
...
class TabularRecompute:
    ...
    primitive_shape: str = "tabular aggregation (GROUP BY + SUM/COUNT)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "result_sha = sha256(serialize(aggregate(data/sales.csv, "
        "spec/query.json))).hexdigest() — a GROUP BY with SUM/COUNT aggregation "
        "over the committed CSV under the committed query."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "The claim is the digest of the serialized result, so a mismatch "
        "localises to the whole result, never to a row.",
        "Aggregators are SUM and COUNT over a GROUP BY; no joins, windows or "
        "having-clauses.",
    )
