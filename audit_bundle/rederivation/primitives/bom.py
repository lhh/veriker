...
shippable core registry (PRIMITIVES.md, shape `bill-of-materials rollup`).
...
class BomRecompute:
    ...
    RE_DERIVATION_MISMATCH.  This catches producer↔verifier output disagreement;
    it does not claim to detect bugs that are identical in both implementations.
    """

    primitive_id: str = "bom_recompute"
    primitive_shape: str = "bill-of-materials rollup"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "resolved_tree = BFS walk of the committed lockfile DAG from root "
        "producing the full per-package tree (id, hash, depth, deps per node in "
        "resolution order) plus the deterministic resolution_order list, "
        "returned as the canonical dict {root, nodes, resolution_order}."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Re-derives the resolution over the COMMITTED lockfile; it does not "
        "fetch, and says nothing about whether the pinned hashes match a real "
        "artifact.",
    )
