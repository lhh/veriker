...
1-vs-N — the boundary revealed at N=2. prior_auth and
...
class AnticheatAdjudicationRecompute:
    ...
    primitive_contract: str = (
        "verdict_list = for each case in evidence/detection_signals.jsonl (file "
        "order), the FIRST rule (evidence/detection_policy.json, sorted by "
        "rule_id) ALL of whose AND-conditions (signal >= or <= threshold over "
        "the case's signals) hold, emitting {model_recommendation: "
        "rule.verdict, matched_rule_id: rule.rule_id}; default "
        '{model_recommendation: "clear", matched_rule_id: null} when NO rule '
        "fires."
    )
