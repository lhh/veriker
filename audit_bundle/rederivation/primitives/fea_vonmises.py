...
class FeaVonMisesRecompute:
    ...
    primitive_shape: str = "scalar with tolerance (FEA von-Mises)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "sigma_vm_max = the maximum von-Mises stress from a pure-Python 2D "
        "plane-stress linear-elastic CST solve over "
        "inputs/{mesh,material,bcs}.json, with the SOLVER parameters (tol, "
        "max_iter) read from spec/solver_config.json."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "2D plane-stress linear-elastic CST elements only.",
        "It RE-RUNS the producer's solve, which is unaffordable at a chokepoint "
        "and impossible against a proprietary solver. The "
        "fea_witness_certificate family is the solver-independent form of the "
        "same claim.",
        "The acceptance epsilon is read from the auditor-anchored binding spec, "
        "never from the producer's solver_config.",
    )
