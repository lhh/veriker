...
shippable core registry (PRIMITIVES.md, shape `scrabble dictionary adjudication
...
class ScrabbleRecompute:
    ...
    primitive_shape: str = "scrabble dictionary adjudication (lexical membership)"
    primitive_version: str = "1"
    primitive_tier: str = "B"
    primitive_contract: str = (
        "ruling = resolve(jurisdiction, timestamp) to the effective edition via "
        "the committed timeline, then test word membership in that edition's "
        "wordlist, emitting {edition_cited, word, is_legal}."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Dictionary MEMBERSHIP only: no tile scoring, no multipliers, no bingo "
        "bonuses.",
        "The wordlist is whatever the bundle commits; the primitive does not "
        "vouch for the edition's contents.",
    )
