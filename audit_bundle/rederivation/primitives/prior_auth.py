...
collect (healthcare_diagnosis). Per the recorded 1-vs-N decision, the
decision-list cluster splits by control structure; this module is
...
class PriorAuthRecompute:
    ...
    primitive_shape: str = "first-match rule -> verdict + default (decision list)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "decisions = for each prior-auth request in clinical/findings.jsonl "
        "(file order), the FIRST plan rule (clinical/plan_rules.json, sorted by "
        "rule_id) whose procedure_category matches AND every required diagnosis "
        "is present AND every required prior_treatment is present AND the "
        "optional max_lab_value (>= / <=) check passes, emitting {request_id, "
        "model_recommendation: rule.verdict, matched_rule_id: rule.rule_id}; "
        'default {model_recommendation: "deny", matched_rule_id: null} when NO '
        "rule matches."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "First-match-with-default control structure only.",
        "The condition vocabulary is this pilot's: procedure-category equality, "
        "required-list membership, and one optional numeric lab bound. It is "
