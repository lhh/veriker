...
(transaction × policy) pair. The recorded 1-vs-N decision is that the ~16-pilot decision-list cluster does NOT
...
(fail-closed RE_DERIVATION_MISMATCH, never a false bless). This binding therefore
...
class FintechAuditRecompute:
    ...
    primitive_shape: str = "all-pairs predicate -> verdict-or-NOT_APPLICABLE (decision list)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "policy_verdicts = [{txn_id, rule_id, matched_conditions, verdict} for "
        "each (transaction, policy-rule) pair, in transactions-then-policies "
        "file-sorted order, where each rule's conditions are AND-ed over the "
        "transaction (gt/lt/eq/ne/in/not_in), matched_conditions lists the "
        "condition fields that held, and verdict is the rule's verdict_if_match "
        'when ALL conditions hold else "NOT_APPLICABLE".'
    )
    primitive_scope_limits: tuple[str, ...] = (
        "All-pairs control structure only: one record per (transaction x rule) "
        "pair. The first-match and fire-and-collect structures are separate "
        "primitives.",
        "Condition operators are exactly gt/lt/eq/ne/in/not_in; a policy using "
        "any other operator is not re-derivable here.",
    )
