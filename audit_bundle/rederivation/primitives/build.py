...
shippable core registry (PRIMITIVES.md, shape `build artifact digest`). The
...
class BuildRecompute:
    ...
    primitive_shape: str = "build artifact digest"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "artifact_sha = sha256(gzip(mtime=0, level=6, concat(sources, "
        'sep="\\n"))).hex().'
    )
    primitive_scope_limits: tuple[str, ...] = (
        "A deterministic recipe over committed sources, not a real toolchain. "
        "build_real_compiler_recompute is the primitive that invokes an actual "
        "compiler.",
    )
