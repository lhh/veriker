...
class AutoUbiRecompute:
    ...
    primitive_shape: str = "feature-aggregation -> threshold-classify"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "tier_list = ordered list, by SORTED policyholder_id, of "
        "{policyholder_id, tier}, where each tier is the rate-table "
        "classification of the telematics features re-aggregated per "
        "policyholder from the committed telematics/trips.jsonl (annual-mileage "
        "estimate / hard-brake rate / harsh-accel rate / late-night fraction) "
        "evaluated against payload/rate_table.json: high-risk surcharge first "
        "(ANY of the three rate-per-mile / late-night thresholds exceeded, "
        "STRICT >), then low-mileage discount (annual estimate <= low_max), "
        "else standard."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Feature-aggregation then threshold-classify: an AGGREGATION shape, not "
        "a decision list. There is no per-rule replay.",
        "The categorical TIER only. The float features and adjustment_pct are "
        "OUT OF SCOPE and are neither re-derived nor compared.",
    )
