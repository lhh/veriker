...
shippable core registry (PRIMITIVES.md, shape `ML metric`). The generic
...
class MlRecompute:
    ...
    primitive_shape: str = "ML metric"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "predicted_classes = the integer argmax (lowest-index tie-break) of the "
        "integer logits logits[k] = sum(W[k][j]*x[j]) + b[k], re-executed per "
        "input sample over the committed weights (weights/model.json, linear- "
        "classifier-v1) and committed inputs (inputs/features.json)."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Integer arithmetic linear classifier only — exact, no tolerance. "
        "fp_ml_recompute is the floating-point shape.",
    )
