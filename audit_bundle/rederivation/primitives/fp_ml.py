...
core registry (PRIMITIVES.md, shape `floating-point ML`). The generic verifier
...
class FpMlRecompute:
    ...
    primitive_shape: str = "floating-point ML"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "rep_logit = f32(sum(W[0][j] * x0[j] for j in range(n_features)) + "
        "b[0]), the single representative scalar predictions[0].logits[0], "
        "accumulated in Python double and snapped to single precision at the "
        "serialization boundary via f32(v) = struct.unpack('f', "
        "struct.pack('f', v))[0]."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "ONE representative scalar (class 0, input 0), not the full logit matrix.",
        "Bound by scalar_epsilon, not exact: two float32-snapped doubles can "
        "differ at ULP scale across libm and accumulation order.",
    )
