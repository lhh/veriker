...
class SpectraSpanRecompute:
    ...
    primitive_shape: str = "extractive span"
    primitive_version: str = "1"
    primitive_tier: str = "B"
    primitive_contract: str = (
        "source fragment = sentence-segment corpus/<source_cid>.txt on [.!?]+ "
        "not adjacent to a digit, strip and drop empty segments, and return the "
        "segment at inputs/span_claim.json.fragment_id."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Returns the SOURCE fragment. The producer's claimed span is compared "
        "against it by the bound text_normalized comparator, not here.",
        "Sentence segmentation is the terminator regex alone: no abbreviation, "
        "quotation or list handling.",
        "source_cid is bundle-controlled and the read is contained inside "
        "corpus/; a path that escapes fails closed.",
    )
