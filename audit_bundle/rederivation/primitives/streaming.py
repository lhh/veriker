...
shippable core registry (PRIMITIVES.md, shape `streaming aggregation`). The
...
class StreamingRecompute:
    ...
    primitive_shape: str = "streaming aggregation"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "per-window aggregate list = replay of the committed event stream "
        "(events/stream.jsonl) through event-time tumbling-window aggregation "
        "per the committed windowing spec (spec/segmentation.json: "
        "window_size_ms + aggregator + late_event_policy), bucketing each event "
        "into window index timestamp_ms // window_size_ms, applying the "
        "aggregator per bucket, and emitting windows in ascending "
        "window_start_ms."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Event-time TUMBLING windows only: no sliding or session windows, and "
        "no processing-time semantics.",
        "Late events are handled by the committed late_event_policy; the "
        "primitive does not decide that policy.",
    )
