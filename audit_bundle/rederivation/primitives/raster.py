...
shippable core registry (PRIMITIVES.md, shape `geospatial zonal count
...
class RasterRecompute:
    ...
    primitive_shape: str = "geospatial zonal count (point-in-polygon)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "in_polygon_cell_count = the number of grid cells whose center (c+0.5, "
        "r+0.5) is inside the polygon, decided by a standard horizontal ray- "
        "casting crossing count, driven by the committed polygon vertices and "
        "grid dimensions in spec/zonal_query.json."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Cell-center point-in-polygon: a cell is in or out, never partial. It "
        "is not an area-weighted zonal statistic.",
        "A single simple polygon; no holes, no multi-part geometry, no "
        "projection handling.",
    )
