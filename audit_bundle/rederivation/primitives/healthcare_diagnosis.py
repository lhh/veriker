...
class HealthcareDiagnosisRecompute:
    ...
    primitive_shape: str = "fire-and-collect (decision list)"
    primitive_version: str = "1"
    primitive_tier: str = "A"
    primitive_contract: str = (
        "icd10_codes = for each rule in inputs/rules.json (sorted by rule_id), "
        "the rule FIRES iff every condition's symptom is present in "
        "inputs/symptoms.json AND its severity >= the condition's min_severity; "
        "each fired rule appends its icd10_code to an ordered list (rules that "
        "do not fire emit nothing)."
    )
    primitive_scope_limits: tuple[str, ...] = (
        "Fire-and-collect control structure: the ordered icd10_code list is the "
        "whole claim.",
        "The per-rule confidence float is OUT OF SCOPE and is neither re- "
        "derived nor compared.",
    )
