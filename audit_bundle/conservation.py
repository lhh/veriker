...
on a sidecar-absent bundle got no surplus sweep at all, so undeclared on-disk
...
* ``UNOWNED`` on-disk path → REJECT (``EXTRA_FILE_NOT_IN_MANIFEST``, the
  ...
  already enforce). Declared paths can never classify UNOWNED, since
  declaration determines class, so this reject condition only ever fires on
  disk.
* non-regular, non-directory on-disk object (FIFO / socket / device node) →
  ...
  verifier — a declared FIFO would hang the strict-SHA walk's
  ``read_bytes`` — so the gate rejects before any content step opens it.
  The walk here ``lstat``s every entry and never opens file contents.
* symlinks are classified by their bundle-relative name, exactly like the
  Pass-3 sweep: an undeclared symlink is UNOWNED (rejected), and a declared
  one is owned by its declared class, whose checker resolves it through the
  ``_safe_bundle_path`` containment guard (escaping links fail closed
  there). Symlinked directories are never descended.

Per-lane ENVELOPE semantics ("whose checker ran")
-------------------------------------------------
The ENVELOPE class's checker differs by lane, and the gate records that
...
* sidecar absent (unsealed): ``manifest.json``'s checker is the parse
  validator plus admission bounds. Parse-validated is not
  byte-integrity-owned, and the gate discloses that residual on the verdict
  face (``envelope_residual``) instead of silently passing it.
...
store is the caller's job — present the bundle in an isolated context —
never a verifier weakening. ``fs_ignore`` exists so an auditor can make a
...
class ConservationResult:
    ...
    differs from plain string order; that order is mirrored here so the
    shim's first-flagged path is byte-identical to the historical sweep.
    """
...
def validate_fs_ignore_patterns(patterns: Sequence[str]) -> tuple[str, ...]:
    ...
    Raises ``ValueError``, since this is auditor-side configuration: a bad
...
def _walk_lstat(root: Path) -> tuple[list[str], list[str], list[tuple[str, str]]]:
    ...
    Never opens file contents, since a FIFO must not block the gate, and
    never descends a symlinked directory. An unreadable directory or
    unstattable entry fails closed into the nonregular bucket. All paths
    are relative POSIX strings under *root*.
    """
    regular: list[str] = []
    symlinks: list[str] = []
...
def run_conservation(
    ...
    *,
    ...
    matches a declared/ENVELOPE path: an auditor configuration conflict,
