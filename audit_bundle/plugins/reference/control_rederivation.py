...
matches the claim AND the attestation's observed_at binds to the evidence's own
captured_at). Any error, unknown control/test_fn, bad signature, hash mismatch,
verdict divergence, or observed_at/captured_at drift fails the bundle.

observed_at/captured_at binding (auditability of WHICH period a verdict applies to):
every attestation carries observed_at, the wall-clock instant the collector claims to
...
Reads:
  ...
  evidence/<file>.json                — captured evidence object(s), e.g. iam:GetAccountSummary at time T;
                                        each object's own 'captured_at' is cross-checked against the
