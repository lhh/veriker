...
object, claims a verdict the verifier does not re-derive, or carries an observed_at that
does not bind to the evidence object's own captured_at (OBSERVED_AT_EVIDENCE_MISMATCH /
OBSERVED_AT_EVIDENCE_UNBOUND / OBSERVED_AT_UNPARSEABLE — see control_rederivation.py's
module docstring for the honesty-rail scope of that binding).
    ...
    def check(self, bundle_dir: Path, manifest) -> PluginResult:
        ...
        if result.returncode == 0:
            return PluginResult(
                ...
                verified_rederivations=frozenset({"control_rederivation"}),
            )
