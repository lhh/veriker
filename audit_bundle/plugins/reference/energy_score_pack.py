...
If derived/energy_score.json exists, diffs the FULL re-derived claim set against
it — not just value_float:
  - value_float          TOLERANCE (1e-3, the pack's own published epsilon)
  - value                exact (== round(value_float))
  - withheld_reason      exact
  - off_finger_fraction  exact (the wear-quality withhold gate)
  - components.*         exact, every field compute() recomputes (mean_hr_bpm,
                          hrv_rmssd_ms, resp_rate_bpm, deep_frac/rem_frac,
                          total_steps, activity_kcal, the cardio_score /
                          sleep_quality / activity_score sub-blends, raw_score,
                          and their *_norm counterparts)
FIX-E (claim-set coverage sweep): value_float alone has three weighted-layer
degrees of freedom, so a producer could previously report wildly divergent
individual biometric readings while landing on a value_float within tolerance
— those fields were computed and printed but never diffed. Every field the
pack recomputes is now bound.
  Exit 0 → full match (or both sides honestly withheld)
  Exit 1 → mismatch on any bound field
...
def compute(bundle_dir: pathlib.Path) -> dict:
    ...
    watch_bia = [
        float(r["bia_quality"]) for r in watch_rows if r.get("bia_quality") is not None
    ]
    ...
    sleep_quality = (
        0.4 * deep_norm + 0.3 * rem_norm + 0.2 * dur_norm + 0.1 * latency_norm
    )
    cardio_score = 0.5 * (1.0 - hr_norm) + 0.3 * hrv_norm + 0.2 * resp_norm
    activity_score = 0.6 * kcal_norm + 0.4 * step_norm

    raw_score = 0.4 * sleep_quality + 0.35 * cardio_score + 0.25 * activity_score
...
# when diffing components.* — both computed and reference dicts may be
# missing a key outright (schema drift) as opposed to carrying an explicit null.
_MISSING = object()


def compare(result: dict, ref_path: pathlib.Path) -> int:
    """Diff the FULL re-derived claim set against derived/energy_score.json.

    FIX-E (claim-set coverage sweep): previously this function checked exactly
    one field — value_float, within TOLERANCE — against the reference. None of
    the 19 intermediate health-metric components, nor off_finger_fraction (the
    wear-quality gate deciding whether a score should be withheld at all), were
    compared. The aggregation has many degrees of freedom (three weighted
    layers), so a producer could report wildly different individual biometric
    readings while landing on a value_float that matched within tolerance.

    Equality discipline (per field):
      - value_float: TOLERANCE (1e-3) — the pack's own documented, published
        re-derivation epsilon. Kept only here; not imported to any other field.
      - value, withheld_reason, off_finger_fraction, components.*: EXACT
        equality at compute()'s own serialization precision. These are all a
        deterministic recompute over the bundle's committed raw_traces — there
        is no honest source of drift a tolerance would need to absorb, and
        importing the value_float tolerance to these fields would just widen
        the same undetectable-compensating-drift hole FIX-E closes.

    Fails closed: a missing components dict, a missing components.* field on
    either side, or a missing/unparseable reference file is a mismatch/error,
    never a silent pass.

    Returns 0 on a full match (or both sides honestly withheld with matching
    withheld_reason/off_finger_fraction), 1 on any bound-field mismatch, 2 on
    ...
    # ── withheld_reason (exact) — always present on both compute() branches ──
    result_withheld_reason = result.get("withheld_reason")
    ref_withheld_reason = ref.get("withheld_reason")
    if result_withheld_reason != ref_withheld_reason:
        mismatches.append(
            "RE_DERIVATION_WITHHELD_REASON_MISMATCH field=withheld_reason "
            f"computed={result_withheld_reason!r} ref={ref_withheld_reason!r}"
        )

    # ── off_finger_fraction (exact) — the wear-quality withhold gate ─────────
    result_off_finger = result.get("off_finger_fraction")
    ref_off_finger = ref.get("off_finger_fraction")
    if result_off_finger != ref_off_finger:
        mismatches.append(
            "RE_DERIVATION_OFF_FINGER_MISMATCH field=off_finger_fraction "
            f"computed={result_off_finger!r} ref={ref_off_finger!r}"
        )

    computed_float = result.get("value_float")
    ref_float = ref.get("value_float")

    if computed_float is None or ref_float is None:
        # One or both sides withheld — no scalar to diff within TOLERANCE.
        # withheld_reason / off_finger_fraction above already cover the
        # withhold-gate honesty; additionally fail closed if exactly one side
        # withheld (the other reported a scalar it should not have).
        if (computed_float is None) != (ref_float is None):
            mismatches.append(
                "RE_DERIVATION_WITHHOLD_STATE_MISMATCH field=value_float "
                f"computed={computed_float!r} ref={ref_float!r}"
            )
        if mismatches:
            for m in mismatches:
                print(f"[energy_score_pack] {m}", file=sys.stderr)
            return 1
        print(
            "[energy_score_pack] MATCH  both sides withheld — "
            f"withheld_reason={result_withheld_reason!r} "
            f"off_finger_fraction={result_off_finger!r}",
            file=sys.stderr,
        )
        return 0

    # ── value_float (TOLERANCE — the pack's own published epsilon) ──────────
    diff = abs(computed_float - ref_float)
    if diff > TOLERANCE:
        mismatches.append(
            "RE_DERIVATION_MISMATCH field=value_float "
            f"computed={computed_float:.6f} ref={ref_float:.6f} "
    ...
    if result_value != ref_value:
        mismatches.append(
            "RE_DERIVATION_VALUE_MISMATCH field=value "
            f"computed={result_value!r} ref={ref_value!r}"
        )

    # ── components.* (exact, every field the pack recomputes) ───────────────
    result_components = result.get("components")
    ref_components = ref.get("components")
    if not result_components:
        mismatches.append(
            "RE_DERIVATION_COMPONENTS_MISSING computed result has no "
            "components dict despite reporting a scalar score"
        )
        result_components = {}
    if not ref_components:
        mismatches.append(
            "RE_DERIVATION_COMPONENTS_MISSING reference has no components "
            "dict despite reporting a scalar score"
        )
        ref_components = {}

    for field in sorted(set(result_components) | set(ref_components)):
        cv = result_components.get(field, _MISSING)
        rv = ref_components.get(field, _MISSING)
        if cv is _MISSING:
            mismatches.append(
                f"RE_DERIVATION_COMPONENT_MISSING field=components.{field} "
                "— pack did not recompute it"
            )
        elif rv is _MISSING:
            mismatches.append(
                f"RE_DERIVATION_COMPONENT_MISSING field=components.{field} "
            ...
            mismatches.append(
                f"RE_DERIVATION_COMPONENT_MISMATCH field=components.{field} "
                f"computed={cv!r} ref={rv!r}"
            )
    ...
    print(
        f"[energy_score_pack] MATCH  computed={computed_float:.6f} ref={ref_float:.6f}"
        f" |diff|={diff:.2e} <= {TOLERANCE:.0e}; value/withheld_reason/"
        f"off_finger_fraction exact-match; {len(result_components)} "
        "components.* field(s) exact-match",
        file=sys.stderr,
    )
    return 0
