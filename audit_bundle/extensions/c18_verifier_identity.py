...
released. The release ships:
  - Hermetic Nix-flake CI (with a documented apko/melange/Wolfi fallback)
  - A CycloneDX SBOM from the Nix derivation
  ...
  - A self-check tripwire plugin — a logging-only signal, NEVER a trust assertion
  - A 4-step consumer verification UX hosted at receipts.vkernel.dev
  - A subtree-split mirror script and release workflow (release-tag-only cadence)
...
which takes JCS + crypto + python-tuf dependencies.
...
#: assertion happens HOST-SIDE, via cosign manifest / crane digest reading
#: the TUF-fetched expected digest (veriker/cli/host_digest_verify.py).
...
class VerifierIdentity(TypedDict, total=False):
    ...
    Field semantics:
      ...
      rekor_inclusion_proof      — INLINE inclusion proof; the consumer verifies
                                   it offline against the bundled proof body.
                                   Shape: dict with
...
class VerifierIdentityFieldMissing(VerifierIdentityError):
    """A required field is absent from bundle.evidence.verifier_identity."""


class VerifierIdentityOCIDigestMalformed(VerifierIdentityError):
    """`verifier_oci_digest` does not match the 'sha256:<64hex>' shape."""


class VerifierIdentityRekorInclusionProofMalformed(VerifierIdentityError):
    """`rekor_inclusion_proof` is malformed: missing required keys, wrong types,
    or a non-hex root_hash."""


class VerifierIdentityReleaseManifestMismatch(VerifierIdentityError):
    """`release_manifest_hash` does not match a recomputation against the bundled
    release_manifest.json (when the file is present)."""


class VerifierSelfCheckUnknownStatus(VerifierIdentityError):
    """`verifier_self_check_status` holds a value outside {'passed', 'failed', 'skipped'}.
...
# Row 20: MALFORMED gets its OWN code. Folding it into REASON_FIELD_MISSING
# would report an unparseable block as "every required field is missing": a
# false account of what was found, and the same conflation one level up.
REASON_BLOCK_MALFORMED = "VERIFIER_IDENTITY_BLOCK_MALFORMED"
...
# fields, all optional in the TypedDict, since legacy bundles omit the entire
# block. But when the block IS present, each field below is required.
...
def verify_verifier_identity_structural(
    ...
    NO network. NO TUF fetch. Returns a list of reason codes; an empty list
    ...
    decides the fail-vs-warn shape.
    ...
    if state == VI_ABSENT:
        # Legacy / pre-C18 bundle: the producer said nothing. Clean PASS.
        return reasons
    if state == VI_MALFORMED:
        # The producer said something unparseable. This is REJECT-side, and
        # reported under its OWN code so it is never confused with an
        # incomplete block (THREAT_MODEL row 20).
        reasons.append(
            f"{REASON_BLOCK_MALFORMED}:evidence.verifier_identity is present "
    ...
    if reasons:
        # If basics are missing, skip the deeper structural checks — running
        # them anyway would double-fail on missing-field reason codes.
        return reasons
    ...
    # check. A shape-valid proof here is NOT cryptographic evidence of log
    # inclusion; real Rekor inclusion verification happens out-of-band, via
    # the receipts.vkernel.dev consumer flow (cosign verify-blob + monitor),
    # and is a later roadmap item. Do not present this check as
...
def self_check_tripwire(
    ...
    constant at the top of the module is the linting beacon for the
    external-copy overclaim sweep.
    ...
    in-container self-check is a SIGNAL that surfaces identity divergence into
    the bundle log; consumers use the 4-step receipts.vkernel.dev flow to
    derive trust.
    """
...
# `_extract_verifier_identity_block` used to return `dict | None` and collapse
# both into None, while every caller reads None as "legacy pre-C18 bundle,
# clean PASS". A producer could therefore skip every C18 structural check by
# making the field any non-dict, and the gradient rewarded it: a
# well-formed-but-INCOMPLETE block drew 7 blocking reasons, while a MALFORMED
# one passed clean.
#
# This helper is HAND-DUPLICATED into plugins/verifier_identity_tripwire.py
# (to avoid an import cycle through the plugin loader). Nothing but
...
def _locate_verifier_identity(manifest: object) -> tuple[str, dict | None]:
    ...
    MALFORMED means the producer said something the verifier cannot parse.
    That is a determinate finding about the artifact's own bytes, not an
    incapacity, so it is REJECT-side (precedent: CLAIMED_VALUE_MALFORMED,
    MalformedSpec — NOT the operator-omission rows 10/17).
