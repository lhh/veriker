...
WITHOUT network, that
  ...
  (a) the leaf chains to a PINNED Fulcio root (X.509 signature path), and every cert in the
      path is valid AT the bundle's trusted time — Rekor integratedTime, or, when that is
      absent/zero as in the staging keyless fixture, the CT-attested embedded-SCT timestamp —
      with bounded clock skew;
  (b) ONLY AFTER (a) passes: the SAN (the GitHub-Actions workflow identity) and the Fulcio
      OIDC-issuer extension pin-match the expected release identity. Pinning the SAN before
      chain verification would be a string compare on attacker-controllable bytes, so a broken
  ...
  (d) the artifact signature (`messageSignature`) over the signed-blob digest verifies under
      the LEAF public key from the Fulcio cert. This establishes the identity-bound-key-to-
      digest half of "verify, don't trust the signer" (the half cosign does NATIVELY here).
      It does NOT by itself re-hash the artifact bytes, so the digest-to-artifact half stays
      the CALLER's responsibility — the composite binds the verified digest to the COSE
      statement, and `verified_message_digest` on the verdict is provided for exactly that.
      This is NOT full cosign-equivalence: cosign verify-blob re-hashes the blob, and this leg
      takes no blob bytes.
...
‼ SCOPE / POSTURE:
  ...
  * There is NO runtime {staging,production} trust switch — that would smell like a downgrade
    attack. Staging anchors live ONLY in the test/CI path; callers pass an explicit
  ...
  * This is a bounded OFFLINE subset (X.509 chain, one SCT signature, one artifact signature),
    NOT a second Sigstore stack: no TUF, no network, no CT monitor, no Rekor write.

Tier-2 network-substrate side (sibling to rekor_anchor.py / c18_tuf_client.py). stdlib plus
`cryptography` (an existing substrate dep). MUST NOT be pulled onto the offline stdlib core
(`veriker/cli/verify.py` / `audit_bundle/verifier.py`) — the two-verifier boundary.
...
# Fulcio X.509v3 extension OIDs (the sigstore/fulcio "1.3.6.1.4.1.57264.1.*" arc).
...
#: the face of the verdict. NEVER collapse it to a bare ``ok`` or "trustless" — the label is
#: load-bearing.
...
# --- Reason codes (stable strings; mirrors rekor_anchor's style). ---
...
class FulcioVerifyError(RuntimeError):
    """Raised on a malformed trust-anchors fixture. A programmer/config error, not a verdict."""
...
class FulcioTrustAnchors:
    """Pinned Fulcio CA roots, candidate intermediates, and CT-log keys for ONE trust environment.
    ...
    def from_anchors_json(cls, data: dict) -> "FulcioTrustAnchors":
        ...
        try:
            ...
            # such shape error as a malformed fixture and raise FulcioVerifyError, never a raw
            # escape.
...
class FulcioNativeVerdict:
    ...
    ``ok`` is True ONLY when all four checks pass: chain+validity, identity, SCT,
    artifact-signature. ``label`` is :data:`FULCIO_OK_CT_ASSUMED` on a pass (carrying the
    irreducible CT residual) and a failure reason otherwise. ``provenance`` is
    :data:`PROVENANCE_NATIVE_STAGING` — this leg is advisory, never the authoritative
    production verdict, until the production signing ceremony.
    """
    ...
    #: The artifact digest that the leaf signature (leg d) was VERIFIED to cover, on a pass
    #: (None otherwise). Exposed so a caller can bind it to the actual artifact / COSE
    #: statement WITHOUT re-reading the raw bundle — closing the key→digest→artifact chain off
    #: the verified value.
...
# -----------------------------------------------------------------------------
# (a) chain build and validity at the trusted time.
# -----------------------------------------------------------------------------
...
def _build_and_validate_chain(
    ...
    Minimal sound path validation, NOT full RFC 5280: each link's signature verifies, every
    cert is valid at `when` (± `skew`), non-leaf certs assert basicConstraints CA, and the path
    ...
    have signed the leaf (the leaf's issuing intermediate). Callers that need the issuer for
    SCT `issuer_key_hash` MUST use this returned object, not a separate subject-name rematch, so
    the SCT is bound to the SAME key the chain validated. This defends against a CA-key-rollover
    anchor set with two intermediates sharing a subject DN but different keys.
    """
    reasons: list[str] = []
    ...
    # path validation does the same). Otherwise a PINNED intermediate presented as the leaf
    # would report chain_verified=True and only fail later at SAN/SCT. Fail at the chain,
    # before identity.
    if _is_ca(leaf):
    ...
    # Walk up to the root; the bounded depth guards against cycles / over-long chains.
    for depth in range(8):
        ...
        if depth == 0:
            leaf_signer = (
                signer  # the VERIFIED signer of the leaf (used for SCT issuer_key_hash)
            )
    ...
    )  # depth limit exceeded without reaching a pinned root
    return False, reasons, leaf_signer


# -----------------------------------------------------------------------------
# (b) SAN and OIDC issuer (ONLY after chain validation).
# -----------------------------------------------------------------------------
...
def _der_utf8string(raw: bytes) -> str | None:
    ...
    Returns None on a non-UTF8String tag, an indefinite/implausible length, or a truncated
    value. That way a malformed v2 issuer extension fails closed — no garbled partial that
    could spuriously match — instead of the previous short-form-only parse, which mangled any
    issuer longer than 127 bytes.
    """
    ...
    if length_byte < 0x80:  # short form: the byte IS the length
        ...
        ):  # indefinite or implausibly large length -> reject
            return None
        ln, off = int.from_bytes(raw[2 : 2 + n], "big"), 2 + n
    if len(raw) < off + ln:  # truncated value
        return None
    try:
        # Strict: a non-UTF-8 issuer value is malformed, so reject it (None) rather than
        # lenient-decode attacker bytes into U+FFFD and compare with ==.
...
def _extract_oidc_issuer(leaf: x509.Certificate) -> str | None:
    """The Fulcio OIDC-issuer claim. Prefers the v2 DER-wrapped extension, falls back to v1 raw."""
    for oid, der_wrapped in (
        ...
        try:
            ...
            # A future `cryptography` version could parse this OID into a typed extension
            # that lacks `.value`.
            continue
...
def _verify_embedded_sct(
    ...
    and checks one ECDSA/SHA-256 signature against the CT key whose logId matches the SCT and
    is valid at the SCT timestamp. Returns (ok, sct_time, reasons). NO CT-inclusion fetch.
    """
    reasons: list[str] = []
    ...
    try:
        ...
        )  # the TBSCertificate with the SCT extension stripped
    except Exception:  # noqa: BLE001 - cryptography raises a generic error when unavailable
...
def _verify_artifact_signature(
    ...
    try:
        ...
        # ValueError: a messageDigest whose decoded length != the Prehashed(SHA-256) digest
        # size (32). cryptography raises ValueError, not InvalidSignature, for that — catch
        # both so a malformed digest fails closed rather than escaping as an uncaught exception.
        return False, [REASON_ARTIFACT_SIG_INVALID], None
...
def _rekor_integrated_time(bundle: dict) -> int | None:
    """Best-effort Rekor integratedTime (seconds) from a Sigstore protobuf bundle.

    Zero or absent means None.
    """
    try:
        ...
        # AttributeError: tlogEntries holding non-dicts (entries[0].get on a str/int). Treat
        # any malformed tlog as "no Rekor time" and fall back to the CT-attested SCT time.
        # Fail closed.
        return None
...
def verify_fulcio_identity_native(
    ...
    *,
    ...
    unverified cert: (a) X.509 chain to a pinned root plus validity at the trusted time;
    (b) SAN + OIDC-issuer pin-match, evaluated ONLY after (a) passes; (c) embedded precert SCT
    against the pinned CT key; (d) artifact signature under the leaf key. A pass is labeled
    :data:`FULCIO_OK_CT_ASSUMED`.

    Trusted time: `integrated_time` (Rekor, seconds) when > 0, else the CT-attested
    embedded-SCT timestamp (the staging keyless fixture records integratedTime=0). The SCT
    timestamp's authenticity is itself established by leg (c), which is REQUIRED for ``ok`` —
    so the fallback introduces no unverified trust input.
    """
    reasons: list[str] = []
    ...
    # (a) chain build and validity at the trusted time. MUST pass before SAN is even read.
    # `leaf_signer` is the cert VERIFIED to have signed the leaf. It feeds the SCT leg so
    ...
    # (c) embedded precert SCT against the pinned CT key (issuer = the chain-validated leaf signer).
