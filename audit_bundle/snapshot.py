...
instants, and the verifier does not assume ``bundle_dir`` is quiescent.
Before this module, per-read pin binding covered the checks whose claims name
a digest (spec_sha_pin, refinement_discharge, fragment_attestation). But the
...
Soundness argument: after materialization, every read targets a tree only
the verifier can write. The strict-SHA walk over the snapshot then binds the
snapshot's bytes to the manifest pins in that same snapshot, so a mutation
racing the copy either produced a snapshot that satisfies the pins — in
which case it is the pinned artifact — or one that mismatches (REJECT). The
final verdict is therefore a conjunction over one immutable byte-set, by
...
* **regular file** — copied via ``open_regular_fd_nofollow``, the BLOCK-01
  ...
  the open is refused at open time, never followed or hung on. The
  destination is created ``O_EXCL`` in the private tree; permission bits
  (``0o777`` mask) are preserved. Byte content is the only verdict-bearing
  property — no check in the package reads mtime/inode/xattr semantics (the
  DSSE set-closure walk compares its own pre/post fstats for swap detection,
  never absolute metadata) — so normalizing those in the copy is in-contract.
* **directory** — recreated.
* **symlink** — replicated by ``readlink``/``symlink``, with one
  semantics-preserving transform: an absolute target inside ``bundle_dir`` is
  re-anchored onto the snapshot root, so it keeps naming the same in-tree
  (pinned, walked) bytes, and the strict-walk's as-built contained-symlink
  tolerance survives relocation. Every other target — relative links, true
  escapes — is replicated verbatim and re-adjudicated by the containment
  guards on the snapshot (escapes still reject).
...
* Source-side failure raises ``SnapshotNonQuiescent``: the bundle could not
  be read as one stable artifact, which is fail-closed REJECT evidence,
  exactly like a hash mismatch. This covers a path that vanishes between scan
  and open, an ``ELOOP`` from a swapped-in link, an unreadable entry, or a
  post-copy re-walk that observes a different (path, kind) set than the first
  pass (the readdir/rename race).
* Destination-side failure raises ``SnapshotMaterializationError``: a
  verifier-side resource problem, surfaced as a clean could-not-conclude
  ERROR, never blamed on the bundle. This covers tempdir creation, ENOSPC,
  and write errors.

The snapshot lives in a fresh ``tempfile.mkdtemp`` directory, mode ``0o700``
by construction and verifier-private. Operators verifying very large bundles
should point ``TMPDIR`` at a scratch volume with capacity: the copy
transiently doubles the bundle's disk footprint. That is an
operator-capacity concern, the same scope boundary as the admission loaders'
size doctrine in SECURITY.md.
...
# os.fdopen(..., "rb"), and io.FileIO _setmode's the fd to binary on
# Windows. This bare os.open + os.write path is the only one that needs the
# flag set explicitly. O_BINARY is Windows-only, so it defaults to 0
# elsewhere.
...
class SnapshotNonQuiescent(Exception):
    ...
    read as one stable artifact — a vanished or swapped entry, an unreadable
    node, or a post-copy re-walk that saw a different path set. Fail-closed
    REJECT."""
...
class SnapshotMaterializationError(Exception):
    ...
    A verifier-side resource problem, so it surfaces as a clean
    could-not-conclude ERROR."""
...
def _scan_tree(root: Path) -> dict[str, str]:
    ...
    ``follow_symlinks=False`` throughout, since a link is an entry, never a
...
def _copy_regular(src: Path, dst: Path) -> None:
    ...
    permission-bit mask, so e.g. an opt-in re-derivation pack keeps its mode;
    all other metadata is deliberately normalized (bytes-only semantics). A
    source-side OSError raises SnapshotNonQuiescent; a destination-side
    OSError raises SnapshotMaterializationError."""
    try:
    ...
    try:
        ...
        try:
            while True:
                ...
                if not chunk:
                    break
                # os.write is the raw syscall: it may transfer fewer bytes
                # than it was handed without raising (a filling filesystem,
                # an RLIMIT_FSIZE ceiling, an interrupted transfer).
                # Discarding the count would silently truncate the snapshot
                # copy, and the strict-SHA walk would then report that
                # verifier-side disk failure as a bundle hash mismatch: a
                # REJECT blamed on the artifact, inverting this module's
                # source/destination failure split. So loop until the chunk
                # is fully on disk.
                ...
                while view:
                    ...
                    if written <= 0:
                        raise SnapshotMaterializationError(
                            f"write made no progress materializing {dst!s} "
                            f"({len(view)} bytes outstanding) — refusing to spin"
                        )
                    view = view[written:]
        finally:
            os.close(dst_fd)
...
def materialize_sealed_snapshot(bundle_dir: Path, snapshot_root: Path) -> None:
    ...
    pass without any per-file open failing, since readdir is not atomic, and
    a drifted set is exactly the non-quiescence this snapshot exists to
    refuse.
    """
    first = _scan_tree(bundle_dir)

    # Fail closed by the complement of the replicable kinds, not by
    # enumerating the known-bad ones: a denylist here goes stale the day
    # _classify learns a new kind name, and a kind the copy loop below
    # doesn't handle would then be silently dropped — a snapshot verifying a
    # different tree than the source.
    unsupported = sorted(
        rel
...
def sealed_snapshot(bundle_dir: Path) -> Iterator[Path]:
    ...
    ``bundle_dir``, with best-effort cleanup on exit. A cleanup failure must
    never mask the verdict computed from the snapshot."""
    try:
    ...
    try:
        ...
        # rmtree does not follow symlinks for the tree it removes, and FIFOs
        # are unlinked, never opened. Errors are swallowed: cleanup is
        # best-effort and must not replace the verdict.
