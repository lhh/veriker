...
manifest.schema_version selects one of two dispatch modes:
...
class BundleManifest:
    ...
    # Claim-field coverage declaration (claimset coverage gate; §C4-style
    # accounting at payload-field granularity). Shape:
    #   {"claim_files":        {payload-key: bundle-relative path, ...},
    #    "opaque_claim_files": {payload-key: bundle-relative path, ...},
    #    "residuals":          {element: reason, ...}}
    # (at least one claim-file map non-empty; keys disjoint; paths injective;
    # opaque = whole-file artifact claims, admitted only for bytes no strict
    # JSON/JSONL reader can read — see audit_bundle.claimset.DECLARATION_ADMITS)
    # When present, BundleVerifier enumerates a closed universe of claim-field
    # elements from the declared files' pinned bytes and requires every
    # element to be either reported covered by a wired plugin
    # (PluginResult.verified_claim_fields) or excused in `residuals` under the
    # committed reason enum (audit_bundle.claimset.CLAIMSET_REASON_ENUM); a
    # silent remainder is could-not-conclude, never a green verdict. None →
    # the gate is inert and the verdict carries a not-declared disclosure
    # (byte-identical behavior otherwise — the same back-compat invariant as
    # every optional key above). Distinct from the informational `payload`
    # key, which stays untouched. Shape-validated at the parse boundary
    # (claimset.validate_claimset_declaration): present-but-malformed is a
    # REJECT, never treated as absent.
...
def _safe_bundle_path(bundle_dir: Path, rel_path: str) -> Path:
    ...
    that the integrity walk does NOT perform on its own:
      ...
      2. If the on-disk object exists, it must resolve to a REGULAR FILE,
         ...
         bytes, and a symlink that ESCAPED was already rejected by check 1.
    ...
    structured failure rather than propagating. Returns the resolved Path.
    The caller still surfaces a "file missing" outcome via the usual
    ``fpath.exists()`` check: this helper fail-closes only on path-escape and
    wrong object-type, NOT absence, so a missing path passes through
    unchanged and tests that patch ``Path.exists`` for call-sequence
    assertions still hold.
    """
    # Representability FIRST, before the join is ever resolved: a lone
    # surrogate makes `.resolve()` raise UnicodeEncodeError and an embedded
    # NUL makes it raise ValueError — neither is the OSError the stat below
    # catches, so both escaped as a crash-class VERIFIER_INTERNAL_ERROR that
    # outranked every REJECT already collected (measured 2026-09-02 through
    # `decision_provenance_log`). Every path-bearing manifest field routes
    # through here, so this is the one place the class is closed for all of
    # them; the parse-boundary check covers the two keys that do not.
    try:
...
def open_regular_fd_nofollow(path: Path) -> int:
    ...
    open-time guards here are atomic with the open:
      ...
      * ``O_NOFOLLOW`` refuses a final-component symlink swapped in after the
        stat (raises ``ELOOP``).
      * ``O_NONBLOCK`` makes opening a FIFO/socket with no writer return
...
#: The longest single path component any mainstream filesystem holds — 255
#: BYTES (NAME_MAX) on Linux ext4/xfs/btrfs and APFS; 255 UTF-16 CODE UNITS on
#: NTFS. Measured in the unit the HOST filesystem counts in (`_name_length`),
#: so a 255-character CJK name — 255 units, a name NTFS holds, 765 UTF-8 bytes —
#: is admitted on Windows and refused on POSIX, where no filesystem holds it.
_NAME_MAX_UNITS = 255

#: The manifest fields whose KEYS are interpolated into filesystem paths.
#: cross_refs keys are logical names resolved by dict lookup and never touch
#: the filesystem, so they are not policed here.
_PATH_KEYED_FIELDS = frozenset({"files", "spec_files"})


#: The unit the host filesystem counts a name's length in.
_NAME_UNIT = "UTF-16 units" if os.name == "nt" else "bytes"


def _name_length(component: str) -> int:
    """A path component's length in the host filesystem's own unit: UTF-16
    code units on Windows (NTFS), bytes of the filesystem encoding elsewhere.
    Uses `surrogatepass` so a lone surrogate cannot raise out of a length
    measurement — it is representable on NTFS, and already refused by the
    encoding check on POSIX."""
    if os.name == "nt":
        return len(component.encode("utf-16-le", "surrogatepass")) // 2
    return len(component.encode("utf-8", "surrogatepass"))


def assert_fs_representable(rel_path: str, *, where: str) -> None:
    """Refuse a manifest path key the filesystem cannot hold, BEFORE any os call.

    Three things about a key make every downstream path operation raise
    something the verifier's boundary can only report as a crash
    (`VERIFIER_INTERNAL_ERROR`, a could-not-conclude that OUTRANKS every REJECT
    already collected):

      * a component longer than NAME_MAX: `Path.exists()`/`os.stat` raise
        ENAMETOOLONG (red team 2026-09-01: a 300-char `spec_files` key reached
        `spec_binding`'s `offline_copy.exists()`);
      * a code point the filesystem encoding cannot encode: a lone surrogate
        outside the `surrogateescape` range makes `Path.resolve()` raise
        UnicodeEncodeError (same red team, a `files` key);
      * an embedded NUL: `Path.resolve()` raises ValueError, which is not even
        an OSError.

    The key is the producer's, in a file that is not hash-covered, so any of
    the three is a one-string way to turn "the artifact is bad" into "the
    verifier could not conclude". They are a MALFORMED ARTIFACT, and this
    raises MalformedManifest so the parse boundary maps them to the REJECT it
    already maps every other malformed manifest to. It is the manifest-key
    copy of dispatch's `OUTPUT_ID_UNSAFE` grammar, which closed the same class
    for `output_id`.

    TWO CALL SITES, one helper. (1) The parse-boundary shape check, over
    `files` / `spec_files` KEYS: the earliest point, and the only cover for
    `spec_binding`, which joins a `spec_files` key itself instead of routing
    through the chokepoint. (2) `_safe_bundle_path`, the chokepoint every
    OTHER path-bearing field routes through (`decision_provenance_log`,
    `retrieval_trace_log`, `snapshots` values, `source_attributes` logs,
    obligation URIs, cross_refs targets, append-only paths). Site (2) was
    found by asking which of the chokepoint's callers the parse-boundary
    check did NOT cover: a lone surrogate in `decision_provenance_log` still
    crashed to ERROR with only (1) in place (measured 2026-09-02). Both call
    sites raise from this one function so they cannot drift apart; the
    chokepoint re-raises as `UnsafeBundlePath`, the structured failure its
    callers already collect.

    Deliberately NOT policed here: containment (`_safe_bundle_path`), object
    type (same), absence (the integrity walk's own MISSING_FILE). This helper
    bounds representability and nothing else. A name the filesystem CAN hold
    but which is merely absent must still be the file-integrity REJECT it
    always was, so the guard never drifts from "unrepresentable" to "unusual".

    `where` names the site for the human ("manifest.files key", "manifest
    path"); it is display only.

    PLATFORM. Both checks are stated in the HOST's terms. The encoding check
    fires on POSIX, where `os.fsencode` is UTF-8 with `surrogateescape` (only
    U+DC80..U+DCFF pass, as the escape of undecodable bytes). On Windows it is
    UTF-8 with `surrogatepass`, every code point encodes, and NTFS genuinely
    holds a lone surrogate, so the arm is inert there, correctly. The length
    check counts UTF-16 units on Windows and bytes elsewhere.
    """
    if "\x00" in rel_path:
        raise MalformedManifest(
            f"{where} {rel_path!r} contains an embedded NUL byte and cannot name a file"
        )
    try:
        os.fsencode(rel_path)  # the encode IS the check; the bytes are unused
    except UnicodeEncodeError as exc:
        raise MalformedManifest(
            f"{where} {rel_path!r} is not encodable in the filesystem encoding "
            f"({exc.reason}) and cannot name a file"
        ) from exc
    separators = {"/", os.sep}
    if os.altsep:
    ...
    for sep in separators:
        components = [part for comp in components for part in comp.split(sep)]
    longest = max((_name_length(c) for c in components), default=0)
    if longest > _NAME_MAX_UNITS:
        raise MalformedManifest(
            f"{where} has a path component of {longest} {_NAME_UNIT}; no "
            f"filesystem holds a name longer than {_NAME_MAX_UNITS}, so it "
            "cannot name a file"
        )
...
def _validate_field_shapes(
    ...
    files/spec_files KEYS are additionally required to be representable on a
    filesystem (`assert_fs_representable`): they are interpolated into paths
    by every step below, and an unrepresentable one crashes the first such
    step as a could-not-conclude that outranks every REJECT.
    """
    for name, val in (
        ("files", files),
        ...
        for k, v in val.items():
            ...
            if name in _PATH_KEYED_FIELDS:
                assert_fs_representable(k, where=f"manifest.{name} key")
...
# Fields where an EXPLICIT JSON null is refused rather than read as absent.
#
# Each declares a `BundleManifest` type that does not admit `None`, AND carries
# no documented null-means-absent contract. MEASURED 2026-09-02 (the
# null-container scoping census): on each of these the null reached a consumer
# that assumes a container and the verifier RAISED, so a determinate REJECT was
# replaced by VERIFIER_INTERNAL_ERROR and its reasons discarded. That is a
# DIAGNOSIS defect, not a fail-open — the same census found zero fields where
# null passes while deletion rejects, and no crasher reached exit 0.
#
# HAND-WRITTEN, NOT DERIVED FROM THE ANNOTATION, and `dispatch_records` is why:
# it also forbids `None`, and explicit-null there is a documented
# forward-compatible legacy marker (tests/test_legacy_bundle_compat.py). Four
# further fields tolerate null today and are undispositioned; they are LEFT
# ALONE deliberately rather than swept in. Ratcheted by
# tests/test_null_container_refused.py.
_NULL_REFUSED: frozenset[str] = frozenset(
    {
        "files",
        "spec_files",
        "cross_refs",
...
def validate_top_level_field_shapes(raw: dict) -> None:
    ...
    absent on the verdict path. bool is an int subclass, not a dict/list/str
    subclass, so JSON true/false can never satisfy any shape in the table.

    JSON null is treated as absent — matching the dataclass-default semantics
    of a missing key — EXCEPT for `_NULL_REFUSED`, where the field's own
    declared type does not admit `None` and the null crashed a downstream
    consumer instead of being read as absent. Measured, and corrected here
    rather than by guarding each consumer, because the parse boundary is the one
    place all of them are downstream of.
    """
    for name in _NULL_REFUSED:
        if name in raw and raw[name] is None:
            raise MalformedManifest(
                f"manifest.{name} is JSON null; this field is a "
                f"{_SHAPE_NAMES[_TOP_LEVEL_FIELD_SHAPES[name]]} whose type "
                f"does not admit null. Omit the key to mean absent — an "
                f"explicit null here reached a consumer expecting a container "
                f"and turned a determinate verdict into a crash"
            )
...
# confused-deputy class. The rules below reject any content that does not
# match the canonical reservation shape the legitimate producers (eidas
# pilot, test_B2) mint, so the attack vector closes without breaking the
# v0.3 "verifier ignores semantics, but accepts the marker" contract.
#
...
def _validate_manifest_deep(
    ...
    verifier unifies on the folded convention: the older bare generic path is retired, and
    the only bare-leaf producers were test/redteam/fuzz harnesses that do not run the
    generic validator.
