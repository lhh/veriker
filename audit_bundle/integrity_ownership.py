...
One positive classification function for a question every file walk in this
verifier currently answers separately, as a complement (a skip-set): "who
owns the integrity of this bundle path, and what does that ownership
guarantee?"

Today that decision is encoded five times, each as a complement of the others:
...
class has to be hand-added to every complement. The DSSE sidecar bug is the
proof: the Pass-3 sweep rejected every sealed bundle until the sidecar was
added to its skips. This module is the positive encoding instead — a single
total function from a bundle-relative path to its owner class, guarantee
floor, and authority source. It is the same closed-registry,
anchored-authority, fail-closed machine the spec-pinned dispatch layer
already uses for outputs, applied one layer down to file space.

MAP STATUS — CONSUMED. The verifier's conservation gate
(``audit_bundle/conservation.py``, run unconditionally by
``BundleVerifier.verify()``) and the file-integrity walks now derive
ownership from ``classify_path``. The complement skip-sets enumerated above
were rewired to this single source: the Pass-3 surplus sweep, strict-SHA
membership, and both ``_SEAM_EXCLUDED`` DSSE literals no longer carry their
own copies. The parity harness (``tests/test_integrity_ownership_parity.py``)
pins this map equal to the as-built walks over the real ``examples/``
corpus.
...
path receives exactly one ``OwnerKind`` — a partition property, where
``UNOWNED`` is the explicit default, never an exception. Inputs:

* ``rel_path`` — a canonical relative-POSIX string under the bundle root
...
* ``manifest`` — a ``BundleManifest`` or any duck-typed stand-in. Fields
  ...
  getattr-defensive for pre-v0.4 test stubs, while everything else assumes
  the field exists. ``typed_checks`` is no longer read, since D3 dropped the
...
Precedence (first match wins; design rationale in parentheses):
  ...
  2. APPEND_ONLY   rel_path is declared in ``append_only_files`` (the
                   ...
                   time).
  3. PLUGIN        rel_path is in both ``manifest.files`` and
                   ``plugin_files`` (an exact-path plugin ``applies_to_files``
                   entry). ``files`` membership is required: an on-disk path
                   covered by a plugin but absent from ``files`` is surplus
                   to the Pass-3 sweep as-built, since no plugin skip exists
                   there — de facto UNOWNED. D3 (ratified) removed the dead
                   second leg (typed_checks-as-paths, see the as-built quirk
                   note below), so PLUGIN is now files ∩ exact-path
                   plugin_files only.
  4. STRICT_SHA    rel_path is in ``manifest.files`` and no rule above fired.
  ...
  5. SNAPSHOT      rel_path appears among ``manifest.snapshots`` values
                   ...
                   or its first path segment is ``snapshots`` — a tree
                   convention the Pass-3 sweep skips wholesale. Declared
                   values carry the cid_recompute guarantee; tree-only
                   membership carries none, since a surplus file inside
                   ``snapshots/`` is skipped unchecked as-built.
  6. SPEC          first path segment is ``spec``. Guarantee pinned_blob_hash
                   applies only to ``spec/<basename>`` paths whose basename
                   matches a declared ``spec_files`` key's basename (the
                   pinning walk checks exactly ``spec/<Path(key).name>``,
                   flat — a deeper ``spec/sub/x.md`` is never pinned).
                   Tree-only membership carries none, since surplus inside
                   ``spec/`` is skipped unchecked as-built, same as SNAPSHOT.
  7. SCAFFOLD      rel_path is exactly ``pilot.json`` or ``README.md`` at the
                   top level (D4, ratified as amended: narrowed from the
                   prior any-depth basename match, so a deeper
                   ``deep/dir/README.md`` is now UNOWNED). A quiet tolerance
                   with a committed-removal direction; the Pass-3 sweep
                   surfaces every tolerated undeclared scaffold to the
                   auditor.
...
* The sealed set-closure walk is stricter than the unsealed Pass-3 sweep:
  under seal, every on-disk file must be payload-listed or envelope, and
  the SCAFFOLD allowance and the spec/snapshots tree skips do not exist
  there. The map does not erase per-walk differences; each walk derives its
  own membership predicate from the one classification.
...
# The seam spec's structural envelope files. The sidecar is written after
# the manifest and signs its hash, so neither can appear in manifest.files
# of a verifiable bundle. Top-level names only.
...
def require_canonical_rel_path(
    ...
    (``_safe_bundle_path``, ``resolve_within``). This is the write-side rule
    the emitter applies to ``BundleContent.files`` / ``spec_files`` keys
    before any byte hits disk. It is deliberately stricter than the read
    side — lexical canonical form, not resolve-then-check — because a
    manifest key is an identity the digests and the DSSE sidecar bind to, so
    two spellings of one file (``a/b`` vs ``a/../a/b``) or a per-OS reading
    (backslash) must never exist. Being strictly stricter keeps "emit-green"
    implying "verifier-green".
    ...
    Rejected (raises :class:`UnsafeBundleRelPath`):
      ...
      * backslashes: a separator on Windows, a filename byte on POSIX, so the
        same key would name different on-disk trees per OS;
      * any ``.``, ``..``, or empty segment — traversal plus non-canonical
        forms like ``a//b``, ``./a``, ``a/``;
...
def append_only_declared_paths(manifest: Any) -> frozenset[str]:
    ...
    filter, and ``spec.get("path", "")``. That includes the consequence that
    a malformed path-less spec dict contributes ``""`` to the set (the §C9.1
    well-formedness validator rejects that shape for loader-parsed
    manifests, so it is reachable only via directly constructed ones).
    """
    return frozenset(
        spec.get("path", "")
