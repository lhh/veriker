...
* Integrity rests on the per-file sha256 digests recorded in manifest.files /
  ...
  manifest cannot hash itself, and the orchestrator re-canonicalizes it
  anyway). So write_bundle emits a single canonical manifest form, and the
  conformance bar is "veriker/cli/verify.py passes", not byte-identity with a pilot's
  old hand-written manifest.
...
* A deployment that needs a stronger witness on any seam (an RFC 3161 TSA plus
  a Roughtime quorum plus a BLS timestamp, a cross-host causal chain, or TEE
...
* Optional DSSE sealing: pass ``dsse_signing_key`` to emit a ``bundle.dsse.json``
  ...
  pass no key are completely unaffected, and manifest.json bytes stay
  identical. The seal path imports ``sign_envelope`` / ``rfc8785`` lazily
  (inside the ``if dsse_signing_key is not None`` block), so the default path
  stays import-light. The seal path does NOT import
...
class BundleSelfCheckFailed(Exception):
    ...
    pass the verifier's own orchestration (BundleVerifier.verify with the
    CLI's default plugin set). The message carries the verdict state and
    every reason."""
...
class BundleContent:
    ...
    files       : bundle-relative path -> bytes for every non-spec artifact
                  ...
                  may not name manifest.json / bundle.dsse.json:
                  write_bundle rejects violations fail-closed before writing
                  anything.
    spec_files  : path-relative-to-spec/ -> bytes for pinned spec documents
                  ...
                  canonical-form discipline as files keys, except envelope
                  names are permitted here: they are ordinary doc names under
                  spec/.
    ...
    extra_manifest_fields: any additional top-level manifest keys this pilot
                  carries that are not produced by a hook, e.g.
                  fragment_anchors, snapshots, snapshot_policy,
                  source_attributes, per_output_manifests. Merged into the
                  manifest last.
    """
...
def _write_file(out_dir: Path, rel_path: str, data: bytes) -> str:
    ...
    already present under ``out_dir`` that redirects the write out of tree.
    The resolve-then-``relative_to`` assert below is the same last-line
    containment rule the verifier's read side applies (``_safe_bundle_path`` /
...
def assemble_manifest(
    ...
    *,
    ...
    Pure (no I/O) given precomputed digests. Kept separate from write_bundle
    so callers that need the dict before writing (or that write files
    themselves) can reuse the assembly. Hooks default to the open
    production-standard set.
    ...
    # Pilot-carried optional fields, then hook-produced fields. A one-level
    # deep merge, not a shallow replace, so multiple sources can contribute
    # distinct sub-keys of the same composite manifest key. Notably
    # `causal_chain`, to which a stronger TimestampProvider adds
    # `layer_b_anchors` and a cross-org CausalChainEmitter adds
    # `cross_host_authenticators`. A shallow `manifest.update` would let the
    # later source clobber the earlier source's sub-keys (dropping
    # layer_b_anchors). For non-dict values, or a sub-key present in both, the
    # later source still wins (last-writer, as before).
    for extra in (
...
def write_manifest(out_dir: Path, manifest: dict) -> Path:
    ...
    Windows (the CRT default), so a Windows producer would emit a CRLF
    manifest for the same logical content a Linux producer writes as LF. The
    DSSE hash is taken over these on-disk bytes on both emit and verify, so
    today that divergence would only be self-consistent-but-non-canonical.
    Pinning LF here forecloses it before any future canonical-bytes comparison
    can trip on it.
    """
...
def write_bundle(
    ...
    *,
    ...
    runs the three hooks, and assembles and writes manifest.json.
    ...
    drift. It raises BundleSelfCheckFailed on any non-OK verdict, including
    ...
    sha256 and every content+spec file digest under a single Ed25519
    signature.
    ...
    If not supplied, 0 is used (deterministic for tests; non-load-bearing per
    D1).
    ...
    When ``dsse_signing_key is None`` (the default), behavior is unchanged:
    no sidecar is written and manifest.json is as before.
    ...
    and can never write outside ``out_dir``. This is the write-side
    counterpart of the verifier's read-side containment. Keys are validated
    up front even though they may be honest pass-throughs of upstream data
    (e.g. dataset filenames): the emitter is the last point where a hostile
    name is still just a string and not yet a write.
    ...
    * Builders legitimately write sibling artifacts straight into the bundle
      ...
      files). Those are owned by other integrity classes, not by
    ...
    * The authoritative surplus decision already exists and is fail-closed:
      ...
      ``BundleVerifier.verify()`` in every lane (an unowned on-disk path
      triggers an ``EXTRA_FILE_NOT_IN_MANIFEST`` reject; the sealed
      set-closure walk is stricter still). A stale file can therefore never
      ride a green verdict. Trust comes from the verdict path, never from
      "the emitter produced it". A consumer that packages a bundle without
      verifying it is outside the trust model.
    * A caller that wants the stale-root check at emit time already has it:
...
def _self_check(out_dir: Path, dsse_signing_key: "Ed25519PrivateKey | None") -> None:
    ...
    loop had no typed-check/plugin/append-only skip set, whose spec_files
    check was presence-only, and which never executed plugins. So
    emitter-green and verifier-green could diverge on the same bundle in
    either direction.
