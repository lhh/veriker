...
Core module for the verdict contract, stdlib-only (importable on the
...
class ErrorKind(str, Enum):
    ...
    `state == ERROR` and is consulted only by composition and telemetry. It is NOT a
    ...
    so safety never needs to distinguish the two ERROR kinds).
      ...
      * CRASH       — an unexpected exception / OOM / stack-exhaustion / verifier-state
                      ...
                      cannot be trusted: this is the BI-4 plugin-abort. It is GLOBAL,
                      dominant, and short-circuits the run
...
def exit_code(verdict: "Verdict") -> int:
    """The ONE verdict -> process exit-code mapping (ADR BI-1 tri-state).

        OK -> 0    ran the logic, good on this dimension
        REJECT -> 1    ran the logic, the ARTIFACT is bad
        ERROR -> 2    could NOT conclude -- a statement about the VERIFIER

    Exists because it was re-implemented per entry point and the copies lost a
    state. Measured 2026-08-31: 104 of 107 `examples/<pilot>/verify.py` scripts
    branched on `result.ok` alone and returned 1 for everything non-OK, so a
    could-not-conclude (wrong spec anchored, a stale committed copy, the same
    JSON with CRLF bytes, a primitive that raised, nothing re-derived) was
    reported to the operator as "the artifact is bad" -- an accusation against
    the producer for something the verifier never established. `veriker/cli/verify.py`
    got this right; the demo wrappers everyone actually runs did not.

    Callers keying on `exit != 0` are unaffected: REJECT and ERROR are both
    non-zero, and neither is certified.

    NOTE -- deliberate difference from veriker.cli/verify.py. That entry point escalates
    to 2 from separately-tracked flags (verifier_errored / rederivation_not_run /
    extension_not_evaluated), so a verdict carrying BOTH a REJECT leg and a
    clean-ERROR leg exits 2 there and 1 here: `compose` ranks
    crash-ERROR > REJECT > clean-ERROR, so `state` is REJECT. This maps `state`,
    which ADR BI-1 calls authoritative. The two agree wherever a verdict has a
    single dominant class, which is every pilot fixture in the tree today.
    """
    if verdict.state is VerdictState.OK:
        return 0
    if verdict.state is VerdictState.REJECT:
        return 1
    return 2
...
def compose(
    ...
    *,
    ...
    be trusted. This IS the BI-4 plugin-abort, restated as the composition rule for the
    crash class.) Concretely, over the GATING legs:
