...
Protocol only, no ABC, using field mirroring rather than import coupling.

Plugin tracking has two surfaces:
...
class PluginResult:
    ...
    # strings a PASSING check must still surface machine-readably. These name
    # guarantees the green result does NOT include (e.g. C19's v0.3
    # reference-implementation limitation; shape-checked-only edge
    # timestamps). BundleVerifier.verify() accumulates these (getattr,
    # default ()) into Completeness.disclosures, the verdict-face channel the
    # conservation gate and append-only floor already use, so the residual
    # reaches LIBRARY consumers, not just the CLI detail printout. Surfacing
    # only: contributes to no pass/fail decision. Prefix idiom:
    # "<check_name>: <residual>".
    ...
    # Per-field claim coverage (claimset coverage gate — the cross-host /
    # fragment-anchor / stamp discipline applied to the fields the payload
    # claim files actually carry): the set of claim-field elements (rendered
    # by audit_bundle.claimset.render_element, "<payload-key>:a.b[].c") this
    # plugin actually READ AND COMPARED against independent data. When a
    # bundle declares `claimset` in its manifest, BundleVerifier accumulates
    # the union and _step_claimset_coverage_guard requires every enumerated
    # claim-field element to be either in it or excused in the bundle's
    # committed residual map; a silent remainder is could-not-conclude
    # (clean-ERROR), never a green verdict over fields nothing read. Report a
    # field ONLY when this check binds its VALUE — a field that is merely
    # present, byte-hashed as part of the file, or copied into a detail
    # string is NOT covered (presence gate ≠ coverage gate). Empty for
    # plugins that do not compare payload claim fields.
    #   HARD RULE (CLAIMSET_COVERED_FILE_UNAUDITED): a result that reports a
    #   covered element MUST list that element's claim file in its own
    #   `files_audited` (bundle-relative or absolute under the bundle) —
    #   otherwise the verdict is REJECT. Whole-file (opaque) elements render
    #   as the payload key alone; report them through
    #   audit_bundle.claimset.opaque_keys_for_paths over the set of files the
    #   check ACTUALLY byte-compared (ideally the set the pack itself emits),
    #   never a constant key. Report ONLY on the full-compare success branch:
    #   NO_PACK / NO_PAYLOAD early-outs report nothing and surface as
    #   could-not-conclude under a declaration, never as a silent pass.
    verified_claim_fields: frozenset[str] = frozenset()
    # Bundle-supplied re-derivation pack EXECUTION (re-derivation-surface
    # disclosure, 2026-08-16): the set of pack filenames this plugin actually
    # RAN to a clean exit (re_derivation_invocation's RE_DERIVED path).
    #
    # REPORTING ONLY — this is deliberately NOT coverage.
    # _step_rederivation_surface_guard NAMES these packs in its disclosure but
    # never lets them SUPPRESS it. RE_DERIVED is decided on the subprocess exit
    # status and nothing else, and re_derivation_invocation's own docstring
    # names the forgery: a pack can "simply `exit(0)` without re-deriving
    # anything (the producer would be grading its own homework)". Measured: a
    # pack whose entire body is `sys.exit(0)` satisfied --require-rederivation
    # at exit 0 with a clean verdict face while this fed the coverage set
    # (adversarial audit, 2026-08-16). Only verifier-distribution code counts as
    # having re-derived something. Empty for every other plugin.
    verified_rederivation_packs: frozenset[str] = frozenset()
    # Generic verifier-side re-derivation coverage (2026-08-16, adversarial
    # audit round 3): the set of labels this plugin recomputed-and-compared
    # using VERIFIER-DISTRIBUTION code. Report `frozenset({self.name})` from a
    # success arm that really re-derived; report NOTHING from a
    # NO_PACK / NO_PAYLOAD / NO_TRACES / mismatch arm.
    #
    # Exists because spec-pinned dispatch and fragment attestation are NOT the
    # only mechanisms: the reference domain checks (span / sensor / control /
    # aigov) recompute and compare too. _step_rederivation_surface_guard
    # measured only the first two, so the mesh pilot — whose
    # sensor_re_derivation genuinely re-derived — was measured returning
    # exit 2 under --require-rederivation: a FALSE FAILURE, which is worse
    # for a relying party than the false pass the guard exists to prevent.
    #
    # This is the guard's open extension point. A new re-derivation plugin that
    # does not report here is invisible to it; tests/test_no_rederivation_
    # disclosure.py ratchets over the in-tree plugin set so the omission is
    # caught rather than shipped (the asymmetric-hardening failure mode: a
    # guard added at the triggering site and never swept across its siblings).
    verified_rederivations: frozenset[str] = frozenset()
