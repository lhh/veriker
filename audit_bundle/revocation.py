...
def _b64url_nopad_decode(s: str) -> bytes:
    ...
    Strict: rejects any character outside the base64url alphabet, and any
    non-canonical encoding (unused trailing bits set). The stdlib
    ``urlsafe_b64decode`` does NOT validate — it silently DISCARDS non-alphabet
    characters, so distinct sidecar strings decode to identical bytes and this
    function's strict-parse promise would be a lie. Measured before this guard
    existed: ``"aGVs!!!!bG8gd29ybGQ"`` and ``"aGVs\n \n bG8gd29ybGQ"`` both
    decoded to ``b"hello world"``, aliasing the canonical encoding.

    The ``remainder == 1`` length check alone does NOT cover this: it rejects
    stray characters only when they happen to change ``len(s) % 4``. Injected
    in multiples of four they pass straight through — accidentally safe, not
    safe.

    This mirrors ``audit_bundle.dsse.pae.b64url_nopad_decode``, deliberately
    duplicated rather than imported to preserve this module's documented
    standalone-auditability contract (see the module docstring). The two copies
    MUST stay in agreement; the drift that lost this guard is exactly what the
    contract costs.
    """
    remainder = len(s) % 4
    if remainder == 1:
    ...
    if _B64URL_ALPHABET_RE.fullmatch(s) is None:
    ...
    # Canonical-encoding check. base64 leaves unused low bits in the final
    # group: a 32-byte value encodes to 43 chars with 2 slack bits (4 distinct
    # strings per value) and a 64-byte Ed25519 signature to 86 chars with 4
    # slack bits (16 strings per value). Every such alias decodes to identical
    # bytes, so a signature verifies under all 16 spellings while the document
    # they arrive in hashes differently each time — which breaks any use of a
    # raw-bytes digest as a stable identifier for the decision. Re-encoding is
    # canonical iff it reproduces the input.
    if base64.urlsafe_b64encode(decoded).rstrip(b"=").decode("ascii") != s:
        raise binascii.Error(
            "Invalid base64url-nopad string: non-canonical encoding "
            "(unused trailing bits are set); this value has an alias that "
